...
// src/lib/billing.ts
var BILLING_CURRENCY = "INR";
var BILLING_GST_RATE = 0.18;
var BILLING_DEFAULT_TRIAL_DAYS = 7;
var BILLING_FREE_TRIAL_HOURS = 168;
var BILLING_HOUR_MS = 60 * 60 * 1e3;
var BILLING_DAY_MS = 24 * BILLING_HOUR_MS;
var BILLING_PLANS = [
  {
    code: "starter",
    name: "Starter",
    headline: "For teams launching their first production inbox.",
    description: "Shared inbox, automation baseline, business profile controls, and channel readiness for a lean operating team.",
    monthlyBaseAmount: 99900,
    annualBaseAmount: 999e3,
    featured: true,
    features: [
  ...
  },
  {
    ...
    description: "Everything in Starter, plus faster support, higher-touch operations, and a plan designed for scale-up teams.",
    monthlyBaseAmount: 199900,
    annualBaseAmount: 1999e3,
    features: [
...
function mapPlatformPricingPlan(plan) {
...
function mapPlatformPricingPlans(plans) {
  const mapped = plans.map(mapPlatformPricingPlan).filter((plan) => Boolean(plan));
  return mapped.length > 0 ? mapped : BILLING_PLANS;
}
function getBillingPlan(planCode, plans = BILLING_PLANS) {
...
function normalizeBillingStatus(value) {
  ...
  return normalized === "trialing" || normalized === "active" || normalized === "inactive" || normalized === "past_due" ? normalized : null;
}
...
function buildBillingSummary(args) {
...
function normalizeConversationThreadStatus(value) {
...
var graphVersion = process.env.META_GRAPH_VERSION || "v24.0";
var instagramGraphVersion = process.env.INSTAGRAM_GRAPH_VERSION || "v25.0";
var officialBusinessAccountGraphVersion = process.env.META_OBA_GRAPH_VERSION || "v25.0";
var obaStatusCacheTtlMs = 5 * 60 * 1e3;
...
var MAX_TEMPLATE_URL_BUTTONS = 1;
var MAX_TEMPLATE_QUICK_REPLY_BUTTONS = 2;
var MAX_TEMPLATE_FLOW_BUTTONS = 1;
var MAX_TEMPLATE_FLOW_BUTTON_TEXT_LENGTH = 25;
var CUSTOMER_SERVICE_WINDOW_MS = 24 * 60 * 60 * 1e3;
var TEMPLATE_HEADER_MEDIA_PREVIEW_KEY = "__connektly_header_media_preview";
var TEMPLATE_HEADER_MEDIA_ROUTE = "/template-header-media";
var TEMPLATE_HEADER_MEDIA_API_ROUTE = "/api/template-header-media";
var TEMPLATE_HEADER_MEDIA_BUCKET = normalizeOptionalString(process.env.TEMPLATE_HEADER_MEDIA_BUCKET) || "app-profile-pictures";
var templateHeaderMediaDir = path.join(__dirname, "storage", "template-header-media");
var transparentPng = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=",
  "base64"
);
var RECOMMENDED_WHATSAPP_FLOW_JSON_VERSION = "7.3";
var RECOMMENDED_WHATSAPP_FLOW_DATA_API_VERSION = "4.0";
var RECOMMENDED_WHATSAPP_FLOW_MESSAGE_VERSION = "3";
var SUPPORTED_WHATSAPP_FLOW_JSON_PUBLISHING_VERSIONS = /* @__PURE__ */ new Set([
  "5.1",
  "6.0",
  "6.1",
  "6.2",
...
var SUPPORTED_WHATSAPP_FLOW_DATA_API_VERSIONS = /* @__PURE__ */ new Set(["3.0", "4.0"]);
var SUPPORTED_WHATSAPP_FLOW_MESSAGE_VERSIONS = /* @__PURE__ */ new Set(["3"]);
var whatsAppFlowJsonVersion = resolveConfiguredMetaVersion(
  "META_FLOW_JSON_VERSION",
  RECOMMENDED_WHATSAPP_FLOW_JSON_VERSION,
  SUPPORTED_WHATSAPP_FLOW_JSON_PUBLISHING_VERSIONS
);
var whatsAppFlowDataApiVersion = resolveConfiguredMetaVersion(
  "META_FLOW_DATA_API_VERSION",
  RECOMMENDED_WHATSAPP_FLOW_DATA_API_VERSION,
  SUPPORTED_WHATSAPP_FLOW_DATA_API_VERSIONS
);
var whatsAppFlowMessageVersion = resolveConfiguredMetaVersion(
  "META_FLOW_MESSAGE_VERSION",
  RECOMMENDED_WHATSAPP_FLOW_MESSAGE_VERSION,
  SUPPORTED_WHATSAPP_FLOW_MESSAGE_VERSIONS
);
var WHATSAPP_FLOW_CATEGORIES = /* @__PURE__ */ new Set([
  "SIGN_UP",
  "SIGN_IN",
  "APPOINTMENT_BOOKING",
  "LEAD_GENERATION",
...
var WHATSAPP_FLOW_FIELD_TYPES = /* @__PURE__ */ new Set(["text", "number", "email", "phone", "date", "select"]);
var MAX_FLOW_FIELDS = 20;
...
var metaAppId = process.env.META_APP_ID || process.env.VITE_META_APP_ID || "";
var metaAppSecret = process.env.META_APP_SECRET || "";
var instagramAppId = process.env.INSTAGRAM_APP_ID || process.env.VITE_INSTAGRAM_APP_ID || "1364707755710909";
var instagramAppSecret = process.env.INSTAGRAM_APP_SECRET || "";
var metaRedirectUri = process.env.META_REDIRECT_URI || "";
var metaWebhookVerifyToken = process.env.META_WEBHOOK_VERIFY_TOKEN || "";
var messengerWebhookVerifyToken = process.env.MESSENGER_WEBHOOK_VERIFY_TOKEN || metaWebhookVerifyToken;
var instagramWebhookVerifyToken = process.env.INSTAGRAM_WEBHOOK_VERIFY_TOKEN || messengerWebhookVerifyToken;
var tokenEncryptionSecret = process.env.META_TOKEN_ENCRYPTION_KEY || "";
var frontendOrigin = process.env.FRONTEND_ORIGIN || "http://localhost:3000";
...
var razorpayBusinessName = process.env.RAZORPAY_BRAND_NAME || "Connektly";
var razorpayBusinessLogoUrl = process.env.RAZORPAY_BRAND_LOGO_URL || "";
var razorpayTrialDays = Number(process.env.RAZORPAY_TRIAL_DAYS || BILLING_DEFAULT_TRIAL_DAYS);
var razorpayMonthlyTotalCount = Number(process.env.RAZORPAY_SUBSCRIPTION_TOTAL_COUNT_MONTHLY || 120);
var razorpayAnnualTotalCount = Number(process.env.RAZORPAY_SUBSCRIPTION_TOTAL_COUNT_ANNUAL || 10);
var enablePlatformWallet = parseBooleanEnv(process.env.ENABLE_PLATFORM_WALLET, true);
var enableCampaignEstimator = parseBooleanEnv(process.env.ENABLE_CAMPAIGN_ESTIMATOR, true);
var enableWabaCreditBilling = parseBooleanEnv(process.env.ENABLE_WABA_CREDIT_BILLING, false);
var enablePartnerBillingMode = parseBooleanEnv(process.env.ENABLE_PARTNER_BILLING_MODE, false);
var walletPricingOverviewUrl = process.env.WALLET_PRICING_OVERVIEW_URL || "https://pricing.connektly.in";
var APP_PROFILE_PICTURE_BUCKET = "app-profile-pictures";
var MAX_APP_PROFILE_PHOTO_BYTES = 5 * 1024 * 1024;
var MESSENGER_PROFILE_ENRICHMENT_RETRY_COOLDOWN_MS = 60 * 60 * 1e3;
var META_USER_PROFILE_CACHE_TTL_MS = 24 * 60 * 60 * 1e3;
var META_USER_PROFILE_ERROR_TTL_MS = 6 * 60 * 60 * 1e3;
var DEFAULT_MESSENGER_WEBHOOK_FIELDS = [
  "messages",
  "messaging_postbacks",
  "message_reads",
  "message_deliveries",
...
var DEFAULT_INSTAGRAM_WEBHOOK_FIELDS = [
  "messages",
  "messaging_postbacks",
  "message_reactions",
  "message_reads"
];
var DEFAULT_INSTAGRAM_LOGIN_WEBHOOK_FIELDS = [
  "messages",
  "messaging_seen",
  "messaging_postbacks",
  "message_reactions"
];
var DEVELOPER_API_SCOPES = [
  "messages:read",
  "messages:write",
  "contacts:read",
  "contacts:write",
...
var RAZORPAY_TRACKED_WEBHOOK_EVENTS = [
  "payment.authorized",
  "payment.pending",
  "payment.failed",
  "payment.captured",
...
var authSupabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    autoRefreshToken: false,
    persistSession: false
  }
});
var adminSupabase = createClient(supabaseUrl, supabaseServiceRoleKey, {
  auth: {
    autoRefreshToken: false,
    persistSession: false
  }
});
var encryptionKey = tokenEncryptionSecret ? crypto.createHash("sha256").update(tokenEncryptionSecret).digest() : null;
var app = express();
var upload = multer({
...
app.get([`${TEMPLATE_HEADER_MEDIA_ROUTE}/*`, `${TEMPLATE_HEADER_MEDIA_API_ROUTE}/*`], (_req, res) => {
  res.status(200).type("image/png").set("Cache-Control", "public, max-age=300").send(transparentPng);
});
...
function parseRazorpayCoupons(raw) {
  const coupons = /* @__PURE__ */ new Map();
  if (!raw?.trim()) {
    return coupons;
  }
  try {
    ...
    for (const [couponCode, value] of Object.entries(parsed || {})) {
      ...
      const kind = record.kind === "percent" || record.type === "percent" ? "percent" : record.kind === "flat" || record.type === "flat" ? "flat" : null;
      ...
      coupons.set(normalizedCode, {
        ...
        kind,
        value: parsedValue,
        description: typeof record.description === "string" && record.description.trim() ? record.description.trim() : void 0
      });
    }
  } catch (error) {
...
var razorpayCoupons = parseRazorpayCoupons(process.env.RAZORPAY_COUPONS_JSON);
function getBillingTrialDays() {
  return Number.isFinite(razorpayTrialDays) && razorpayTrialDays >= 0 ? Math.round(razorpayTrialDays) : BILLING_DEFAULT_TRIAL_DAYS;
}
function getRazorpaySubscriptionTotalCount(cycle) {
...
function resolveBillingCoupon(couponCode) {
  const normalizedCode = normalizeCouponCode(couponCode);
  if (!normalizedCode) {
    return null;
  }
...
async function loadBillingPlans() {
  try {
    const { data, error } = await adminSupabase.from("user_platform_settings").select("settings").eq("section", "pricing_plans").maybeSingle();
    if (error) {
    ...
    const plans = settings && typeof settings === "object" && "plans" in settings ? settings.plans : [];
...
async function getBillingQuote(input) {
  const plans = await loadBillingPlans();
  const plan = getBillingPlan(input.planCode, plans);
  if (!plan) {
...
function buildRazorpayCatalogKey(input) {
...
async function razorpayRequest(pathname, init) {
  ...
  if (!response.ok) {
    const errorMessage = payload?.error?.description || payload?.description || payload?.message || `Razorpay request failed (${response.status}).`;
...
async function getOrCreateRazorpayPlan(input) {
  ...
  return razorpayRequest("/v1/plans", {
    method: "POST",
    body: JSON.stringify({
...
async function createRazorpaySubscription(input) {
  const { quote } = await getBillingQuote({
    planCode: input.planCode,
    billingCycle: input.billingCycle,
    couponCode: input.couponCode || void 0
  });
  ...
  const subscription = await razorpayRequest("/v1/subscriptions", {
    ...
    body: JSON.stringify({
      ...
      customer_notify: 0,
      start_at: Math.floor(new Date(quote.trialEndsAt).getTime() / 1e3),
      notes: {
...
async function fetchRazorpaySubscription(subscriptionId) {
  return razorpayRequest(`/v1/subscriptions/${subscriptionId}`, {
    method: "GET"
  });
}
...
function verifyRazorpaySubscriptionSignature(args) {
  requireRazorpayCredentials();
  const expectedSignature = crypto.createHmac("sha256", razorpayKeySecret).update(`${args.paymentId}|${args.subscriptionId}`).digest("hex");
  if (expectedSignature !== args.signature) {
    throw new Error("Razorpay payment signature verification failed.");
  }
}
function resolvePersistedBillingStatus(subscription, trialEndsAt) {
...
function majorAmountToMinorUnits(value) {
  return Math.max(Math.round(value * 100), 0);
}
...
function normalizeWalletType(value) {
  return value === "campaign_estimate" || value === "partner_managed_waba" || value === "platform" ? value : getDefaultWalletType();
}
function normalizeWalletTransactionType(value) {
  return value === "credit" || value === "debit" || value === "refund" || value === "adjustment" ? value : "adjustment";
}
function normalizeWalletTransactionSource(value) {
  return value === "razorpay" || value === "stripe" || value === "manual" || value === "system" ? value : "system";
}
function normalizeWalletTransactionPurpose(value) {
  return value === "subscription" || value === "addon" || value === "campaign_estimate" || value === "waba_billing" ? value : "addon";
}
function normalizeWalletTransactionStatus(value) {
  return value === "pending" || value === "successful" || value === "failed" || value === "refunded" ? value : "pending";
}
function isWalletRechargeEnabled() {
  return enablePlatformWallet && Boolean(razorpayKeyId && razorpayKeySecret);
}
function buildEmptyWalletSummary(preferredCurrency) {
...
function mapWalletSummary(row, preferredCurrency, transactions) {
  if (!row) {
...
async function resolveWorkspaceOwnerUserId(userId) {
  const membershipResult = await adminSupabase.from("workspace_team_members").select("workspace_owner_user_id").eq("member_user_id", userId).eq("status", "active").maybeSingle();
...
async function getProfilePreferredCurrency(userId) {
  const profileResult = await adminSupabase.from("app_profiles").select("preferred_currency").eq("user_id", userId).maybeSingle();
  if (profileResult.error && !isMissingSchemaError(profileResult.error)) {
    throw profileResult.error;
  }
  return normalizeOptionalString(profileResult.data?.preferred_currency) ? normalizeCurrencyCode(profileResult.data?.preferred_currency) : null;
}
async function backfillWalletFromLegacyLedger(orgId, preferredCurrency) {
  const legacyLedgerResult = await adminSupabase.from("credit_ledger").select("*").eq("user_id", orgId).order("created_at", { ascending: true });
  if (legacyLedgerResult.error && !isMissingSchemaError(legacyLedgerResult.error)) {
    throw legacyLedgerResult.error;
  }
  const legacyRows = legacyLedgerResult.data || [];
  ...
  const createdWalletResult = await adminSupabase.from("wallets").insert({
    user_id: orgId,
    org_id: orgId,
  ...
  const createdWallet = createdWalletResult.data;
...
async function ensureWalletForUser(userId, preferredCurrency) {
  ...
  const existingWalletResult = await adminSupabase.from("wallets").select("*").eq("org_id", orgId).eq("wallet_type", walletType).maybeSingle();
  ...
  const existingWallet = existingWalletResult.data || null;
  if (existingWallet) {
    ...
    if (currentCurrency !== targetCurrency && currentAvailable === 0 && currentLocked === 0) {
      const updatedWalletResult = await adminSupabase.from("wallets").update({
        currency: targetCurrency,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("id", existingWallet.id).select("*").single();
      if (updatedWalletResult.error) {
        throw updatedWalletResult.error;
      }
      return updatedWalletResult.data;
    }
  ...
  return createdWalletResult.data;
}
async function getWalletForUser(user) {
  ...
  const transactionsResult = await adminSupabase.from("transactions").select("*").eq("wallet_id", walletRow.id).order("created_at", { ascending: false }).limit(50);
...
function verifyRazorpayOrderSignature(args) {
  requireRazorpayCredentials();
  const expectedSignature = crypto.createHmac("sha256", razorpayKeySecret).update(`${args.orderId}|${args.paymentId}`).digest("hex");
  if (expectedSignature !== args.signature) {
    throw new Error("Razorpay payment signature verification failed.");
  }
}
...
function normalizeRazorpayWebhookTimestamp(value) {
  const numeric = typeof value === "number" && Number.isFinite(value) ? value : typeof value === "string" && /^\d+$/.test(value.trim()) ? Number(value.trim()) : null;
...
function getRazorpayPayloadRecord(payload) {
  return isRecord(payload.payload) ? payload.payload : {};
}
function getRazorpayPayloadEntity(payload, key) {
...
function getPrimaryRazorpayEntity(payload, eventName) {
...
function getRazorpayEntityNotes(entity) {
  return entity && isRecord(entity.notes) ? entity.notes : null;
}
function getRazorpayWebhookNotes(payload) {
  const payloadRecord = getRazorpayPayloadRecord(payload);
  const notes = [];
  for (const wrapper of Object.values(payloadRecord)) {
    ...
    const entityNotes = getRazorpayEntityNotes(wrapper.entity);
    if (entityNotes) {
...
function getRazorpayWebhookIdentifiers(payload, eventName) {
  ...
  return {
    ...
    paymentId: normalizeOptionalIdentifier(payment?.id) || normalizeOptionalIdentifier(entity?.payment_id) || (primary.type === "payment" ? normalizeOptionalIdentifier(entity?.id) : null),
    orderId: normalizeOptionalIdentifier(order?.id) || normalizeOptionalIdentifier(entity?.order_id) || (primary.type === "order" ? normalizeOptionalIdentifier(entity?.id) : null),
    subscriptionId: normalizeOptionalIdentifier(subscription?.id) || normalizeOptionalIdentifier(entity?.subscription_id) || (primary.type === "subscription" ? normalizeOptionalIdentifier(entity?.id) : null),
    invoiceId: normalizeOptionalIdentifier(invoice?.id) || normalizeOptionalIdentifier(entity?.invoice_id) || (primary.type === "invoice" ? normalizeOptionalIdentifier(entity?.id) : null),
    refundId: normalizeOptionalIdentifier(refund?.id) || normalizeOptionalIdentifier(entity?.refund_id) || (primary.type === "refund" ? normalizeOptionalIdentifier(entity?.id) : null),
    settlementId: normalizeOptionalIdentifier(settlement?.id) || normalizeOptionalIdentifier(entity?.settlement_id) || (primary.type === "settlement" ? normalizeOptionalIdentifier(entity?.id) : null)
  };
}
async function resolveRazorpayWebhookUserId(payload, identifiers) {
  const notesUserId = getRazorpayWebhookNotes(payload).map((notes) => normalizeOptionalIdentifier(notes.user_id)).find((userId) => Boolean(userId));
  if (notesUserId) {
    return notesUserId;
  }
  if (identifiers.subscriptionId) {
    const { data, error } = await adminSupabase.from("app_profiles").select("user_id").eq("razorpay_subscription_id", identifiers.subscriptionId).maybeSingle();
  ...
  for (const externalReference of externalReferences) {
    const { data: transaction, error: transactionError } = await adminSupabase.from("transactions").select("wallet_id").eq("external_reference", externalReference).maybeSingle();
    ...
    const { data: wallet, error: walletError } = await adminSupabase.from("wallets").select("user_id, org_id").eq("id", walletId).maybeSingle();
...
async function insertRazorpayWebhookEvent(req, payload) {
  ...
  const eventId = normalizeOptionalString(req.header("x-razorpay-event-id")) || normalizeOptionalString(payload.id);
  ...
  const { error } = await adminSupabase.from("razorpay_webhook_events").insert({
    user_id: userId,
    event_id: eventId,
...
async function startFreeTrial(user) {
  const existing = await adminSupabase.from("app_profiles").select("*").eq("user_id", user.id).maybeSingle();
  if (existing.error) {
    throw existing.error;
  }
  const existingProfile = existing.data ? mapProfile(existing.data) : null;
...
function mapDbError(error) {
  const message = error instanceof Error ? error.message : typeof error === "string" ? error : error && typeof error === "object" && "message" in error && typeof error.message === "string" ? error.message : error && typeof error === "object" && "error_description" in error && typeof error.error_description === "string" ? error.error_description : error && typeof error === "object" && "details" in error && typeof error.details === "string" ? error.details : JSON.stringify(error);
  const missingSchema = typeof message === "string" && (message.includes("does not exist") && message.includes("relation") || message.includes("Could not find the table"));
  if (missingSchema) {
...
function isMissingSchemaError(error) {
  ...
  return message === "Supabase tables are missing. Apply supabase/schema.sql before starting the API server.";
}
...
async function authenticate(req, res, next) {
  ...
  if (apiKey || apiSecret) {
    try {
      ...
      const { data, error: error2 } = await adminSupabase.from("developer_api_credentials").select("*").eq("api_key", apiKey).eq("status", "active").maybeSingle();
      if (error2 || !data) {
        throw error2 || new Error("Invalid API key credentials.");
      }
      const credential = mapDeveloperApiCredential(data);
      const secretHash = String(data.secret_hash || "");
      ...
      const lastUsedResult = await adminSupabase.from("developer_api_credentials").update({ last_used_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", credential.id);
      if (lastUsedResult.error) {
        console.error("Failed to update API key last_used_at:", lastUsedResult.error);
      }
      req.authedUser = user2;
      req.developerApiCredential = credential;
      next();
      return;
    } catch (error2) {
      res.status(401).json({
        error: mapDbError(error2)
      });
      return;
    }
  }
...
async function activatePendingWorkspaceMembership(userId) {
  const { data, error } = await adminSupabase.from("workspace_team_members").update({
    status: "active",
    accepted_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("member_user_id", userId).eq("status", "invited").select("*");
  if (error && !isMissingSchemaError(error)) {
    throw error;
  }
  for (const row of data || []) {
    ...
    const memberName = normalizeOptionalString(membership.full_name) || normalizeOptionalString(membership.invited_email) || "A team member";
...
function toIsoTimestamp(raw) {
  ...
  const numeric = typeof raw === "string" && /^\d+$/.test(raw) ? Number(raw) * 1e3 : Number(raw);
...
function normalizeDeveloperWebhookEvents(value) {
  if (!Array.isArray(value)) {
...
function normalizeDeveloperWebhookUrl(value) {
...
function normalizeDeveloperWebhookStatus(value, fallback) {
...
function getDeveloperApiRequiredScope(req) {
...
function resolveConfiguredMetaVersion(envKey, recommendedVersion, supportedVersions) {
...
function getGraphPictureUrl(value) {
  if (!isRecord(value)) {
    return null;
  }
...
function isSupportedAppProfilePhotoMimeType(value) {
  return value === "image/png" || value === "image/jpeg";
}
function getAppProfilePhotoExtension(mimeType) {
  return mimeType === "image/png" ? "png" : "jpg";
}
function getStoragePublicUrl(bucket, objectPath) {
  const { data } = adminSupabase.storage.from(bucket).getPublicUrl(objectPath);
  return normalizeOptionalString(data.publicUrl);
}
function getAppProfilePhotoStoragePathFromUrl(value) {
...
async function deleteStoredAppProfilePhoto(value) {
...
async function uploadAppProfilePhoto(args) {
  ...
  const { error } = await adminSupabase.storage.from(APP_PROFILE_PICTURE_BUCKET).upload(objectPath, args.buffer, {
...
function normalizeWorkspaceUserRole(value) {
  if (value === "Owner" || value === "Admin" || value === "Manager") {
    return value;
  }
  return "Agent";
}
function normalizeWorkspaceUserStatus(value) {
...
function normalizeNotificationSoundPreset(value) {
...
function normalizeNotificationVolume(value, fallback = 0.8) {
...
function notificationTypeToPreferenceKey(type) {
  switch (type) {
    case "incoming_message":
...
function shouldCreateNotification(preferences, type) {
...
async function getAppNotificationEmailConfig() {
  try {
    const { data, error } = await adminSupabase.from("user_platform_settings").select("settings").eq("section", "email_templates").maybeSingle();
    if (!error && data && typeof data === "object" && data.settings && typeof data.settings === "object") {
      ...
      if (provider && provider.enabled) {
        ...
        const customTriggers = Array.isArray(data.settings.customTriggers) ? data.settings.customTriggers : [];
...
async function resolveNotificationRecipientEmail(userId) {
  ...
  return normalizeEmailAddress(authResult.data.user?.email) || normalizeEmailAddress(profileResult.data?.email) || null;
}
function escapeHtml(value) {
  return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
}
function renderEmailTemplate(value, placeholders) {
  return value.replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}/g, (_match, key) => placeholders[key] ?? "");
}
async function sendTemplateApprovedNotificationEmail(args) {
  ...
  const targetUrl = args.targetPath ? `${appOrigin.replace(/\/+$/, "")}/${args.targetPath.replace(/^\/+/, "")}` : appOrigin;
  ...
  const text = customTrigger?.textBody ? renderEmailTemplate(String(customTrigger.textBody), placeholders) : [
    args.body,
    "",
    ...detailLines,
  ...
  const html = customTrigger?.html ? renderEmailTemplate(String(customTrigger.html), placeholders) : `
      <div style="font-family: Arial, sans-serif; color: #111827; line-height: 1.6;">
        <h2 style="margin: 0 0 12px;">${escapeHtml(args.title)}</h2>
        <p style="margin: 0 0 16px;">${escapeHtml(args.body)}</p>
        ${detailLines.length > 0 ? `<ul style="margin: 0 0 20px; padding-left: 20px;">${detailLines.map((line) => `<li>${escapeHtml(line)}</li>`).join("")}</ul>` : ""}
        <p style="margin: 0;">
  ...
  await transporter.sendMail({
    ...
    to: recipientEmail,
    replyTo: config.replyToEmail || void 0,
    subject,
    text,
    html
  });
...
async function verifyPasswordResetUserExists(email) {
  const perPage = 1e3;
...
function sendPasswordResetEmailInBackground(args) {
  setImmediate(() => {
    authSupabase.auth.resetPasswordForEmail(args.email, {
...
function normalizeEmailPort(value, label) {
...
function normalizeSmtpSecure(port2, value) {
  if (port2 === 465) {
    return true;
  }
  if (port2 === 587 || port2 === 25) {
    return false;
  }
  return value !== false;
}
function normalizeImapSecure(port2, value) {
  if (port2 === 993) {
    return true;
  }
  if (port2 === 143) {
...
function normalizeEmailConnectionInput(input) {
...
function normalizeEmailConnectionIdentityInput(input) {
  const displayName = normalizeEditableString(input.displayName);
  const emailAddress = normalizeEmailAddress(input.emailAddress);
  const authUser = normalizeEditableString(input.authUser);
  const password = typeof input.password === "string" ? input.password.trim() : "";
...
function normalizeEmailSmtpVerificationInput(input) {
...
function normalizeEmailImapVerificationInput(input) {
  const identity = normalizeEmailConnectionIdentityInput(input);
  const imapHost = normalizeEmailHost(input.imapHost);
  if (!imapHost) {
    throw new Error("IMAP host is required.");
...
function normalizeEmailTemplateEditorMode(value) {
...
function normalizeEmailRecipients(value) {
  ...
  for (const entry of value) {
    ...
    const name = normalizeEditableString(entry.name) || null;
...
function normalizeEmailCampaignInput(input) {
  ...
  const audienceSource = input.audienceSource === "custom" ? "custom" : "contacts";
  if (!templateId) {
...
function normalizeInsightsChannel(value) {
...
function parseInsightsDateInput(value) {
...
function resolveInsightsDateRange(startDate, endDate) {
...
function getMessagingLimitCapacity(tier) {
  if (!tier) {
    return null;
  }
  ...
  if (shorthandMatch) {
    ...
    const multiplier = shorthandMatch[2] === "M" ? 1e6 : 1e3;
    return numeric * multiplier;
  }
  const numericMatch = normalized.match(/(\d+)/);
  return numericMatch ? Number(numericMatch[1]) : null;
}
function getNormalizedMessagingLimitTier(value) {
  return normalizeOptionalString(value?.whatsapp_business_manager_messaging_limit) || normalizeOptionalString(value?.messaging_limit_tier);
}
function countRepliedOutboundMessages(rows) {
  const rowsByThread = /* @__PURE__ */ new Map();
...
function normalizeEditableString(value) {
  if (typeof value !== "string") {
    return void 0;
  }
  return value.trim();
}
function normalizeWhatsAppBusinessVerticalToken(value) {
  return (value || "").trim().toUpperCase().replace(/[^A-Z0-9]+/g, "");
}
function normalizeWhatsAppBusinessVertical(value) {
...
function normalizeStringArray(value, options) {
  ...
  const normalized = value.map((entry) => normalizeOptionalString(entry)).filter((entry) => Boolean(entry)).map((entry) => options?.uppercase ? entry.toUpperCase() : entry);
  return Array.from(new Set(normalized));
}
function normalizeConversationalAutomationPrompt(value) {
  if (typeof value !== "string") {
    return null;
  }
  const trimmed = value.trim();
...
function normalizeConversationalAutomationCommandName(value) {
  if (typeof value !== "string") {
    return null;
  }
  const normalized = value.trim().replace(/^\/+/, "").replace(/\s+/g, "_").replace(/[^a-zA-Z0-9_]/g, "").toLowerCase();
  return normalized || null;
}
function normalizeConversationalAutomationCommandDescription(value) {
  if (typeof value !== "string") {
    return null;
  }
  const trimmed = value.trim();
...
function normalizeConversationalAutomationCommands(value) {
  if (!Array.isArray(value)) {
    return [];
  }
...
function normalizeConversationalAutomationInput(input) {
  const prompts = Array.isArray(input?.prompts) ? Array.from(
    new Set(
      input.prompts.map((prompt) => normalizeConversationalAutomationPrompt(prompt)).filter((prompt) => Boolean(prompt))
    )
  ) : [];
  return {
...
function normalizeAutomationRuleName(value, keyword) {
...
function normalizeAutomationRuleKeyword(value, matchMode) {
...
function normalizeAutomationRuleMatchMode(value) {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
  if (normalized === "any") {
    return "any";
  }
  if (normalized === "equals" || normalized === "starts_with" || normalized === "ends_with" || normalized === "fuzzy") {
    return normalized;
  }
  return "contains";
}
function normalizeAutomationRuleTriggerType(value) {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
  if (!normalized || normalized === "incoming_message_keyword") {
    return "incoming_message_keyword";
  }
  if (normalized === "whatsapp_message_received" || normalized === "instagram_message_received" || normalized === "contact_attribute_added" || normalized === "contact_attribute_changed" || normalized === "lead_created") {
    return normalized;
  }
  throw new Error(`Unsupported rule trigger: ${normalized}.`);
}
function normalizeAutomationRuleFilterCondition(value, index) {
  if (!isRecord(value)) {
  ...
  const values = Array.isArray(value.values) ? value.values.map((entry) => normalizeOptionalString(entry)).filter((entry) => Boolean(entry)) : [];
  const normalized = {
    id: normalizeOptionalString(value.id) || void 0,
    type: type === "contact_initiates_chat" || type === "timestamp" || type === "no_keyword_matches" || type === "contact_exists" || type === "contact_attribute" ? type : "message_contains_keywords",
    operator: operator === "equals" || operator === "contains" || operator === "starts_with" || operator === "ends_with" || operator === "fuzzy" || operator === "does_not_equal" || operator === "between" || operator === "outside" || operator === "is_true" || operator === "is_false" ? operator : "contains_any",
    field: normalizeOptionalString(value.field) || void 0,
    value: normalizeOptionalString(value.value) || void 0,
    values,
    startTime: normalizeOptionalString(value.startTime ?? value.start_time) || void 0,
    endTime: normalizeOptionalString(value.endTime ?? value.end_time) || void 0
  };
  ...
  if (normalized.type === "contact_attribute" && (!normalized.field || !normalized.value)) {
...
function buildLegacyAutomationFilterGroup(keyword, matchMode) {
  if (matchMode === "any") {
    return {
      operator: "AND",
      conditions: []
...
function normalizeAutomationRuleFilters(value) {
  if (!isRecord(value)) {
    return null;
  }
...
function normalizeAutomationRuleAction(value) {
  if (!isRecord(value)) {
  ...
  if (actionType === "send_flow") {
    ...
    const flowActionData = isRecord(value.flowActionData) ? value.flowActionData : isRecord(value.flow_action_data) ? value.flow_action_data : null;
...
function normalizeAutomationRuleInput(input, index) {
  if (!isRecord(input)) {
  ...
  const filters = normalizeAutomationRuleFilters(input.filters) || normalizeAutomationRuleFilters(inputAction?.filters) || buildLegacyAutomationFilterGroup(keyword, keywordMatchMode);
...
function normalizeAutomationRulesInput(value) {
  if (!Array.isArray(value)) {
  ...
  return value.map((entry, index) => {
    return normalizeAutomationRuleInput(entry, index);
  });
}
...
function formatContactIdentity(value) {
...
function buildContactIdentityVariants(value) {
  const normalizedPhone = normalizePhoneLike(value);
  if (normalizedPhone) {
    return Array.from(/* @__PURE__ */ new Set([normalizedPhone, `+${normalizedPhone}`]));
  }
...
function normalizeCallDirection(value) {
  if (typeof value === "string") {
    const normalized = value.trim().toLowerCase();
    if (normalized === "incoming" || normalized === "user_initiated" || normalized === "user-initiated" || normalized === "user initiated") {
      return "incoming";
    }
  }
  return "outgoing";
}
function normalizeCallState(value, fallback = "dialing") {
  if (typeof value !== "string") {
    return fallback;
  }
  const normalized = value.trim().toLowerCase();
...
function extractPhoneLike(value) {
  ...
  return normalizePhoneLike(value.wa_id) || normalizePhoneLike(value.user_wa_id) || normalizePhoneLike(value.phone) || normalizePhoneLike(value.phone_number) || normalizePhoneLike(value.id) || null;
}
...
function getCallLogTypeFromSession(session) {
  if (session.direction === "incoming") {
    return session.state === "missed" || session.state === "rejected" ? "missed" : "incoming";
  }
  return "outgoing";
}
function formatCallStateLabel(state) {
  switch (state) {
    case "incoming":
      return "Incoming";
    case "dialing":
...
function normalizeLabels(value) {
  ...
  const labels = value.map((entry) => typeof entry === "string" ? entry.trim() : "").filter(Boolean);
...
function normalizeWebsites(value) {
  ...
  const websites = value.map((entry) => typeof entry === "string" ? entry.trim() : "").filter(Boolean);
...
function buildTemplateExamples(value) {
...
function guessMediaTypeFromMime(mimeType) {
  if (!mimeType) {
    return "document";
  }
...
function getMessageText(message) {
  ...
  switch (type) {
    ...
    case "image":
      return message.image?.caption || "Image attachment";
    case "video":
      return message.video?.caption || "Video attachment";
    case "document":
      return message.document?.caption || message.document?.filename || "Document attachment";
    case "audio":
    ...
    case "interactive": {
      ...
      return normalizeOptionalString(body?.text) || normalizeOptionalString(header?.text) || normalizeOptionalString(footer?.text) || normalizeOptionalString(nfmReply?.body) || normalizeOptionalString(buttonReply?.title) || normalizeOptionalString(listReply?.title) || normalizeOptionalString(listReply?.description) || normalizeOptionalString(parameters?.display_text) || normalizeOptionalString(parameters?.text) || normalizeOptionalString(parameters?.body) || normalizeOptionalString(parameters?.title) || null;
    }
    case "system": {
    ...
    case "unsupported": {
      ...
      const details = normalizeOptionalString(errorData?.details) || normalizeOptionalString(firstError?.message) || normalizeOptionalString(firstError?.title);
...
function getMessengerThreadIdentity(senderId) {
  return `messenger:${senderId}`;
}
function getInstagramThreadIdentity(senderId) {
  return `instagram:${senderId}`;
}
function getMessengerAttachmentPreviewLabel(type, count) {
  if (count > 1) {
...
function parseMessengerInboundEvent(event) {
  ...
  const createdAt = toIsoTimestamp(
    typeof event.timestamp === "string" || typeof event.timestamp === "number" ? event.timestamp : null
  ...
  if (message) {
    ...
    const attachments = Array.isArray(message.attachments) ? message.attachments.filter((entry) => isRecord(entry)) : [];
    const primaryAttachmentType = attachments.length > 0 ? normalizeOptionalString(attachments[0].type) : null;
    const messageId = normalizeOptionalString(message.mid) || `messenger:${senderId}:${String(event.timestamp || Date.now())}`;
    ...
    return {
      ...
      messageId,
      createdAt,
      messageType: quickReplyPayload && !text ? "quick_reply" : primaryAttachmentType === "image" || primaryAttachmentType === "video" || primaryAttachmentType === "audio" ? primaryAttachmentType : primaryAttachmentType === "file" ? "document" : text ? "text" : "attachment",
      body: text || quickReplyPayload || getMessengerAttachmentPreviewLabel(primaryAttachmentType, attachments.length),
      raw: {
        source: "messenger",
        event_type: "message",
        ...event
...
function isMessengerPageMessagingEvent(event, pageId) {
  const recipient = isRecord(event.recipient) ? event.recipient : null;
  const recipientId = normalizeOptionalIdentifier(recipient?.id);
  return Boolean(recipientId && recipientId === pageId);
}
function parseInstagramInboundEvent(event, instagramAccountId) {
  const sender = isRecord(event.sender) ? event.sender : null;
  const recipient = isRecord(event.recipient) ? event.recipient : null;
  const senderId = normalizeOptionalIdentifier(sender?.id);
  const recipientId = normalizeOptionalIdentifier(recipient?.id);
  ...
  if (message) {
    ...
    const attachments = Array.isArray(message.attachments) ? message.attachments.filter((entry) => isRecord(entry)) : [];
    const primaryAttachmentType = attachments.length > 0 ? normalizeOptionalString(attachments[0].type) : null;
    const messageId = normalizeOptionalString(message.mid) || `instagram:${senderId}:${String(event.timestamp || Date.now())}`;
    if (!text && attachments.length === 0 && !quickReplyPayload) {
      return null;
    }
    return {
...
function buildMetaApiError(response, payload) {
  const code = payload?.error?.code;
  let message = payload?.error?.error_data?.details || payload?.error?.error_user_msg || payload?.error?.message || `Meta API request failed with status ${response.status}`;
  if (code === 138006) {
...
async function metaRequestDetailed({
  accessToken,
  path: graphPath,
  method = "GET",
  query,
...
function normalizeMetaRedirectUriCandidate(value) {
  if (typeof value !== "string") {
    return null;
  }
  const trimmed = value.trim();
...
function expandMetaRedirectUriCandidates(value) {
  ...
  try {
    ...
    const pathnameWithTrailingSlash = pathnameWithoutTrailingSlash === "/" ? "/" : `${pathnameWithoutTrailingSlash}/`;
...
async function exchangeEmbeddedSignupCode(code, options) {
  ...
  for (let index = 0; index < attempts.length; index += 1) {
    ...
    try {
      ...
      const isRedirectUriMismatch = payload.error?.code === 100 && payload.error?.error_subcode === 36008;
...
async function exchangeInstagramLongLivedAccessToken(accessToken) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      message = payload2.error?.message || message;
    } catch {
...
async function exchangeInstagramLoginCode(code, options) {
  ...
  if (!response.ok) {
    ...
    try {
      const payload2 = await response.json();
      message = payload2.error_message || payload2.error_description || message;
    } catch {
    }
    throw new Error(message);
  }
  ...
  return {
    ...
    permissions: Array.isArray(payload.permissions) ? payload.permissions.filter((entry) => typeof entry === "string") : []
  };
}
var WHATSAPP_PHONE_NUMBER_BASE_FIELDS = "display_phone_number,verified_name,quality_rating,whatsapp_business_manager_messaging_limit,name_status";
var WHATSAPP_PHONE_NUMBER_DISPLAY_NAME_FIELDS = `${WHATSAPP_PHONE_NUMBER_BASE_FIELDS},new_display_name,new_name_status`;
var WHATSAPP_PHONE_NUMBER_LIVE_STATUS_FIELDS = `${WHATSAPP_PHONE_NUMBER_DISPLAY_NAME_FIELDS},code_verification_status,is_pin_enabled`;
var WHATSAPP_PHONE_NUMBER_LEGACY_LIVE_STATUS_FIELDS = `${WHATSAPP_PHONE_NUMBER_BASE_FIELDS},code_verification_status,is_pin_enabled`;
function isUnsupportedWhatsAppPhoneNumberFieldError(error, fields) {
  const message = error instanceof Error ? error.message.toLowerCase() : String(error || "").toLowerCase();
  return fields.some((field) => message.includes(field.toLowerCase()));
}
async function fetchPhoneNumber(accessToken, phoneNumberId) {
  try {
...
async function submitWhatsAppDisplayNameUpdate(accessToken, phoneNumberId, displayName) {
...
function normalizeMetaRegistrationPin(value) {
...
function normalizeMetaVerificationCodeMethod(value) {
  if (value === "SMS" || value === "VOICE") {
    return value;
  }
  throw new Error("Verification code method must be SMS or VOICE.");
}
function normalizeMetaVerificationCodeLanguage(value) {
...
function normalizeMetaVerificationCode(value) {
...
function normalizeWhatsAppDisplayName(value) {
...
function getMetaChannelMetadataRecord(row) {
  return isRecord(row.metadata) ? row.metadata : {};
}
function getTwoStepVerificationEnabledFromPhoneSnapshot(phone) {
  ...
  if (normalizedStatus === "UNVERIFIED" || normalizedStatus === "NOT_VERIFIED" || normalizedStatus === "EXPIRED" || normalizedStatus === "PENDING") {
    return false;
  }
  return null;
}
function mergeLiveTwoStepVerificationMetadata(metadata, phone, checkedAt) {
  const currentTwoStepVerification = isRecord(metadata.twoStepVerification) ? metadata.twoStepVerification : {};
  ...
  if (isEnabled === null) {
    return {
      ...
      twoStepVerification: {
        ...
        isPinEnabled: typeof phone.is_pin_enabled === "boolean" ? phone.is_pin_enabled : currentTwoStepVerification.isPinEnabled ?? null,
  ...
  return {
    ...
    twoStepVerification: {
      ...
      liveStatusCheckedAt: checkedAt,
      enabledAt: isEnabled ? normalizeOptionalString(currentTwoStepVerification.enabledAt) || checkedAt : normalizeOptionalString(currentTwoStepVerification.enabledAt),
      disabledAt: isEnabled ? null : checkedAt
    }
  };
}
function mapStoredTwoStepVerificationStatus(metadata) {
  if (!isRecord(metadata) || !isRecord(metadata.twoStepVerification)) {
    return null;
  }
  const twoStepVerification = metadata.twoStepVerification;
  const isPinEnabled = typeof twoStepVerification.isPinEnabled === "boolean" ? twoStepVerification.isPinEnabled : null;
  return {
...
function getStoredWhatsAppRegistrationPin(metadata) {
  const twoStepVerification = isRecord(metadata.twoStepVerification) ? metadata.twoStepVerification : {};
...
function resolveWhatsAppRegistrationPin(metadata, rawPin) {
  const hasProvidedPin = rawPin !== void 0 && rawPin !== null && !(typeof rawPin === "string" && !rawPin.trim());
  if (hasProvidedPin) {
    return {
      pin: normalizeMetaRegistrationPin(rawPin),
      pinMode: "user_provided",
      generated: false
    };
  }
  ...
  return {
    ...
    pinMode: "system_generated",
    generated: true
  };
}
async function registerWhatsAppBusinessPhoneNumber(accessToken, phoneNumberId, pin) {
  const url = new URL(
  ...
  if (!response.ok) {
    const errorMessage = payload?.error?.error_data?.details || payload?.error?.error_user_msg || payload?.error?.message || "";
...
function getWhatsAppRegistrationErrorMessage(error) {
  if (error instanceof Error && error.message) {
    return error.message;
  }
  return typeof error === "string" && error.trim() ? error : "Failed to register the WhatsApp sender with Meta.";
}
function isWhatsAppRegistrationPinMismatchError(error) {
  const message = error instanceof Error ? error.message : String(error || "");
  const normalized = message.toLowerCase();
  return normalized.includes("133005") || normalized.includes("pin mismatch") || normalized.includes("wrong pin");
}
function shouldAutoRegisterApprovedDisplayName(row, phoneSnapshot) {
  const metadata = getMetaChannelMetadataRecord(row);
  const autoRegistration = isRecord(metadata.autoRegistration) ? metadata.autoRegistration : {};
  const displayNameApproval = isRecord(metadata.displayNameApproval) ? metadata.displayNameApproval : {};
  const displayNameRequest = isRecord(metadata.displayNameRequest) ? metadata.displayNameRequest : {};
  const requestedName = normalizeOptionalString(displayNameRequest.requestedName) || normalizeOptionalString(phoneSnapshot.verified_name);
  const verifiedName2 = normalizeOptionalString(phoneSnapshot.verified_name);
  ...
  const requestMatchesCurrentName = Boolean(
    requestedName && verifiedName2 && requestedName === verifiedName2 && phoneStatus === "APPROVED"
  );
  const status = requestedName ? requestStatus === "APPROVED" || requestMatchesCurrentName ? "APPROVED" : requestStatus || phoneStatus || approvalStatus : approvalStatus === "APPROVED" || phoneStatus === "APPROVED" ? "APPROVED" : phoneStatus || approvalStatus || requestStatus;
  ...
  if (normalizeOptionalString(autoRegistration.status) === "failed" && normalizeOptionalString(autoRegistration.reason) === "display_name_approved") {
  ...
  const registrationReference = normalizeOptionalString(displayNameApproval.approvedAt) || normalizeOptionalString(displayNameRequest.approvedAt) || normalizeOptionalString(displayNameRequest.requestedAt) || normalizeOptionalString(displayNameApproval.lastCheckedAt);
  if (!registeredAt) {
    return true;
  }
...
async function autoRegisterWhatsAppSenderForChannel(args) {
  ...
  const currentAutoRegistration = isRecord(currentMetadata.autoRegistration) ? currentMetadata.autoRegistration : {};
  const currentTwoStepVerification = isRecord(currentMetadata.twoStepVerification) ? currentMetadata.twoStepVerification : {};
  const currentDisplayNameApproval = isRecord(currentMetadata.displayNameApproval) ? currentMetadata.displayNameApproval : {};
  const currentDisplayNameRequest = isRecord(currentMetadata.displayNameRequest) ? currentMetadata.displayNameRequest : null;
  try {
    ...
    try {
      ...
      if (!isWhatsAppRegistrationPinMismatchError(error2)) {
        throw error2;
      }
    ...
    const registeredName = normalizeOptionalString(currentDisplayNameRequest?.requestedName) || normalizeOptionalString(phone.verified_name);
    const nextMetadata = {
      ...currentMetadata,
      autoRegistration: {
      ...
      twoStepVerification: {
        ...
        lastPinUpdatedAt: pinMode === "stored" && !pinResetBeforeRegistration ? normalizeOptionalString(currentTwoStepVerification.lastPinUpdatedAt) || attemptedAt : attemptedAt,
        pinMode,
        pinCiphertext,
        systemGeneratedAt: generated ? attemptedAt : normalizeOptionalString(currentTwoStepVerification.systemGeneratedAt),
        disabledAt: null
      }
    };
    ...
    const { data, error } = await adminSupabase.from("meta_channels").update({
      status: "connected",
    ...
    }).eq("user_id", args.userId).eq("id", args.row.id).select("*").single();
    if (error) {
      throw error;
    }
    return {
    ...
    const { data, error: persistError } = await adminSupabase.from("meta_channels").update({
      metadata: nextMetadata,
      updated_at: attemptedAt
    }).eq("user_id", args.userId).eq("id", args.row.id).select("*").single();
    if (persistError) {
...
async function updateWhatsAppBusinessPhoneNumberTwoStepVerification(accessToken, phoneNumberId, pin) {
...
async function requestWhatsAppBusinessPhoneNumberVerificationCode(accessToken, phoneNumberId, input) {
  const url = new URL(
...
async function verifyWhatsAppBusinessPhoneNumberVerificationCode(accessToken, phoneNumberId, code) {
  const url = new URL(
...
async function deregisterWhatsAppBusinessPhoneNumber(accessToken, phoneNumberId) {
  const url = new URL(
  ...
  if (!response.ok) {
    const errorMessage = payload?.error?.error_data?.details || payload?.error?.error_user_msg || payload?.error?.message || "";
    if (typeof errorMessage === "string" && (errorMessage.toLowerCase().includes("already deregistered") || errorMessage.toLowerCase().includes("not registered"))) {
      return {
...
function normalizeCallPermissionStatus(value) {
...
function normalizeCallActionName(value) {
  return typeof value === "string" && value.trim() ? value.trim().toLowerCase() : "unknown";
}
async function fetchCallPermissions(accessToken, phoneNumberId, userWaId) {
  ...
  return {
    ...
    permission: {
      ...
      expirationTime: typeof response.permission?.expiration_time === "number" ? response.permission.expiration_time : null
    },
    actions: Array.isArray(response.actions) ? response.actions.map((action) => ({
      ...
      limits: Array.isArray(action.limits) ? action.limits.map((limit) => ({
        ...
        limitExpirationTime: typeof limit.limit_expiration_time === "number" ? limit.limit_expiration_time : null
      })) : []
    })) : []
  };
}
function normalizeCallSettingStatusValue(value, fallback = "disabled") {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
...
function normalizeCallIconVisibilityValue(value, fallback = "hidden") {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
  if (!normalized) {
    return fallback;
  }
...
function toMetaCallSettingStatus(value, fallback = "disabled") {
  return normalizeCallSettingStatusValue(value, fallback).toUpperCase();
}
function toMetaCallIconVisibilityValue(value, fallback = "hidden") {
...
function normalizeMetaCallTime(value, fieldName) {
...
function normalizeMetaCallDate(value, fieldName) {
...
function mapCallHoursWindow(entry) {
  if (!isRecord(entry)) {
    return null;
  }
...
function mapCallHolidaySchedule(entry) {
  if (!isRecord(entry)) {
    return null;
  }
...
function mapCallHoursSettings(raw) {
  if (!isRecord(raw)) {
    return null;
  }
  return {
    ...
    weeklyOperatingHours: Array.isArray(raw.weekly_operating_hours ?? raw.weeklyOperatingHours) ? (raw.weekly_operating_hours ?? raw.weeklyOperatingHours).map((entry) => mapCallHoursWindow(entry)).filter((entry) => Boolean(entry)) : [],
    holidaySchedule: Array.isArray(raw.holiday_schedule ?? raw.holidaySchedule) ? (raw.holiday_schedule ?? raw.holidaySchedule).map((entry) => mapCallHolidaySchedule(entry)).filter((entry) => Boolean(entry)) : [],
    raw
  };
}
function getCallingSettingsRecord(raw) {
  const firstDataRow = Array.isArray(raw.data) && isRecord(raw.data[0]) ? raw.data[0] : null;
  const container = firstDataRow || raw;
  return isRecord(container.calling) ? container.calling : {};
}
function mapCallSettings(raw, channelRow) {
...
async function fetchCallSettings(accessToken, phoneNumberId, channelRow) {
  const response = await metaRequestDetailed({
    accessToken,
...
function normalizeCallHoursUpdateInput(input) {
  const status = normalizeCallSettingStatusValue(input.status);
  const timezoneId = normalizeOptionalString(input.timezoneId);
  const weeklyOperatingHours = Array.isArray(input.weeklyOperatingHours) ? input.weeklyOperatingHours.map((entry) => {
  ...
  const holidaySchedule = Array.isArray(input.holidaySchedule) ? input.holidaySchedule.map((entry) => ({
...
function normalizeCallSettingsUpdateInput(input, current) {
  ...
  if ("callHours" in input) {
    payload.call_hours = input.callHours ? normalizeCallHoursUpdateInput(input.callHours) : null;
  }
...
async function updateCallSettings(accessToken, phoneNumberId, channelRow, input) {
...
function normalizeBlockedUsersPayload(users) {
  if (!Array.isArray(users)) {
    throw new Error("At least one WhatsApp user is required.");
  }
  const normalizedUsers = Array.from(
    new Set(
      users.map((value) => normalizePhoneLike(value)).filter((value) => Boolean(value))
    )
  );
...
function mapBlockedUserRecord(entry) {
...
function mapBlockedUserOperationRecord(entry) {
  return {
    input: normalizeOptionalString(entry.input),
    waId: normalizePhoneLike(entry.wa_id)
  };
}
async function fetchBlockedUsers(accessToken, phoneNumberId, after) {
  const response = await metaRequestDetailed({
    accessToken,
    path: `${phoneNumberId}/block_users`,
    query: {
      after: normalizeOptionalString(after) || void 0
    }
  });
  const data = Array.isArray(response.data) ? response.data.map((entry) => mapBlockedUserRecord(entry)).filter((entry) => Boolean(entry)) : [];
  return {
    data,
...
async function fetchAllBlockedUsers(accessToken, phoneNumberId) {
  const users = [];
...
async function blockUsers(accessToken, phoneNumberId, users) {
  ...
  return {
    ...
    users: Array.isArray(response.block_users?.added_users) ? response.block_users.added_users.map((entry) => mapBlockedUserOperationRecord(entry)) : []
  };
}
async function unblockUsers(accessToken, phoneNumberId, users) {
  const normalizedUsers = normalizeBlockedUsersPayload(users);
  const response = await metaRequestDetailed({
    accessToken,
    path: `${phoneNumberId}/block_users`,
  ...
  return {
    messagingProduct: normalizeOptionalString(response.messaging_product),
    users: Array.isArray(response.block_users?.removed_users) ? response.block_users.removed_users.map((entry) => mapBlockedUserOperationRecord(entry)) : []
  };
}
function normalizeActivitiesLimit(value) {
...
function normalizeActivitiesListFilter(value) {
  if (Array.isArray(value)) {
    return Array.from(
      new Set(
...
function normalizeWhatsAppBusinessActivitiesFilters(input) {
  return {
    limit: normalizeActivitiesLimit(input.limit),
    after: normalizeOptionalString(input.after) || void 0,
    before: normalizeOptionalString(input.before) || void 0,
    since: normalizeOptionalString(input.since) || void 0,
    until: normalizeOptionalString(input.until) || void 0,
    activityType: normalizeActivitiesListFilter(input.activityType)
  };
}
function mapWhatsAppBusinessActivityRecord(entry) {
...
async function fetchWhatsAppBusinessActivities(accessToken, wabaId, filters) {
  const normalizedFilters = normalizeWhatsAppBusinessActivitiesFilters(filters);
  const response = await metaRequestDetailed({
    accessToken,
    path: `${wabaId}/activities`,
    query: {
      fields: "id,activity_type,timestamp,actor_type,actor_id,actor_name,description,details,ip_address,user_agent",
      ...
      activity_type: normalizedFilters.activityType && normalizedFilters.activityType.length > 0 ? normalizedFilters.activityType.join(",") : void 0
    }
  });
  return {
    wabaId,
    activities: Array.isArray(response.data) ? response.data.map((entry) => mapWhatsAppBusinessActivityRecord(entry)).filter((entry) => Boolean(entry)) : [],
    paging: {
      before: normalizeOptionalString(response.paging?.cursors?.before),
      after: normalizeOptionalString(response.paging?.cursors?.after),
...
async function manageRemoteCall(accessToken, phoneNumberId, input) {
  ...
  const callIds = Array.isArray(response.calls) ? response.calls.map((entry) => normalizeOptionalString(entry.id)).filter((value) => Boolean(value)) : [];
  return {
    messagingProduct: normalizeOptionalString(response.messaging_product),
...
async function fetchOfficialBusinessAccountStatus(accessToken, phoneNumberId) {
  ...
  return {
    ...
    oba_status: response.official_business_account?.oba_status || (response.is_official_business_account ? "APPROVED" : void 0),
    status_message: response.official_business_account?.status_message || (response.is_official_business_account ? "This phone number is an approved Official Business Account." : void 0)
  };
}
async function submitOfficialBusinessAccountUpdate(accessToken, phoneNumberId, payload) {
  return metaRequest({
    accessToken,
...
function normalizeWhatsAppBusinessAppealStatus(value) {
  const status = normalizeOptionalString(value);
  return status ? status.toUpperCase() : null;
}
function normalizeUriInput(value, label) {
...
function normalizeOfficialBusinessAccountUpdateInput(input) {
  if (!input || typeof input !== "object") {
  ...
  const supportingLinks = Array.isArray(supportingLinksInput) ? supportingLinksInput.map((link) => normalizeUriInput(link, "supportingLinks")).filter((link) => Boolean(link)) : [];
...
function mapOfficialBusinessAccountStatus(raw, phoneNumberId, checkedAt, lastError) {
  return {
...
function mapOfficialBusinessAccountUpdateResponse(raw, phoneNumberId, checkedAt) {
  const updatedStatus = isRecord(raw.updated_status) ? mapOfficialBusinessAccountStatus(raw.updated_status, phoneNumberId, checkedAt) : null;
  return {
...
function getStoredOfficialBusinessAccountStatus(metadata, phoneNumberId) {
  if (!isRecord(metadata)) {
    return null;
  }
  const rawStatus = isRecord(metadata.officialBusinessAccountStatus) && metadata.officialBusinessAccountStatus || isRecord(metadata.officialBusinessAccount) && metadata.officialBusinessAccount || null;
  if (!rawStatus) {
    return null;
  }
  ...
  if (status.lastError && isUnsupportedOfficialBusinessAccountStatusError(status.lastError)) {
    return {
      ...status,
      statusMessage: status.statusMessage || "Official Business Account status is not available from Meta for this phone number.",
      lastError: null
    };
  }
  return status;
}
function isOfficialBusinessAccountStatusFresh(status) {
...
function isUnsupportedOfficialBusinessAccountStatusError(error) {
...
async function persistOfficialBusinessAccountStatus(userId, channelId, status) {
  const { data, error } = await adminSupabase.from("meta_channels").select("metadata").eq("user_id", userId).eq("id", channelId).maybeSingle();
  if (error) {
    throw error;
  }
  const metadata = isRecord(data?.metadata) ? { ...data.metadata } : {};
  const { error: updateError } = await adminSupabase.from("meta_channels").update({
    metadata: {
      ...metadata,
    ...
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("user_id", userId).eq("id", channelId);
  if (updateError) {
    throw updateError;
  }
}
  ...
  try {
    ...
    if (isUnsupportedOfficialBusinessAccountStatusError(error)) {
      const status2 = {
        ...
        statusMessage: cachedStatus?.statusMessage || "Official Business Account status is not available from Meta for this phone number.",
        lastCheckedAt: checkedAt,
        lastError: null
      };
      await persistOfficialBusinessAccountStatus(args.userId, args.row.id, status2).catch((persistError) => {
...
async function updateWhatsAppDisplayNameForChannel(args) {
  ...
  const { data, error } = await adminSupabase.from("meta_channels").update({
    metadata: {
      ...
      displayNameApproval: {
        ...isRecord(currentMetadata.displayNameApproval) ? currentMetadata.displayNameApproval : {},
        status: "PENDING_REVIEW",
...
async function updateBusinessProfile(accessToken, phoneNumberId, input) {
  const payload = {
    messaging_product: "whatsapp"
  };
...
async function fetchCommerceSettings(accessToken, phoneNumberId) {
  const response = await metaRequest({
    accessToken,
...
function normalizeCommerceSettingsInput(input) {
  const normalized = {};
...
async function updateCommerceSettings(accessToken, phoneNumberId, input) {
...
async function configureConversationalAutomation(accessToken, phoneNumberId, input) {
  const response = await metaRequestDetailed({
    accessToken,
...
function isSupportedBusinessProfilePhotoMimeType(value) {
  const normalized = (value || "").trim().toLowerCase();
  return normalized === "image/jpeg" || normalized === "image/png";
}
async function createBusinessProfilePhotoUploadSession(accessToken, file) {
  requireMetaAppCredentials();
  const response = await metaRequest({
    accessToken,
...
async function uploadBusinessProfilePhotoHandle(accessToken, file) {
...
async function fetchWaba(accessToken, wabaId) {
  return metaRequest({
    accessToken,
    path: wabaId,
    query: {
...
var META_SETUP_SENSITIVE_KEYS = /* @__PURE__ */ new Set([
  "accesstoken",
  "token",
  "secret",
  "clientsecret",
...
function isSensitiveMetaSetupKey(key) {
  const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, "");
  return META_SETUP_SENSITIVE_KEYS.has(normalizedKey) || normalizedKey.includes("token") || normalizedKey.includes("secret") || normalizedKey === "code";
}
function sanitizeReusableMetaSetupValue(value, depth = 0) {
...
function addIdentifierValue(target, value) {
  if (Array.isArray(value)) {
...
function collectReusableMetaIdentifiers(value) {
  const identifiers = {
    businessIds: /* @__PURE__ */ new Set(),
    wabaIds: /* @__PURE__ */ new Set(),
    phoneNumberIds: /* @__PURE__ */ new Set(),
    pageIds: /* @__PURE__ */ new Set(),
    instagramAccountIds: /* @__PURE__ */ new Set(),
    adAccountIds: /* @__PURE__ */ new Set(),
    formIds: /* @__PURE__ */ new Set(),
    catalogIds: /* @__PURE__ */ new Set()
  };
  const visit = (entry) => {
    ...
    for (const [key, nestedValue] of Object.entries(entry)) {
      const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, "");
      if (normalizedKey === "businessid" || normalizedKey === "businessmanagerid" || normalizedKey === "metabusinessid") {
        addIdentifierValue(identifiers.businessIds, nestedValue);
      } else if (normalizedKey === "wabaid" || normalizedKey === "whatsappbusinessaccountid") {
        addIdentifierValue(identifiers.wabaIds, nestedValue);
      } else if (normalizedKey === "phonenumberid" || normalizedKey === "whatsappphonenumberid") {
      ...
      } else if (normalizedKey === "instagrambusinessaccountid" || normalizedKey === "instagramaccountid" || normalizedKey === "iguserid") {
      ...
      } else if (normalizedKey === "formid" || normalizedKey === "formids" || normalizedKey === "leadgenformid") {
...
async function fetchReusableMetaSetupAssets(accessToken) {
  try {
    const response = await metaRequestDetailed({
      accessToken,
      path: "me/accounts",
...
function buildReusableMetaSetupContext(args) {
  ...
  const instagramAccountIds = pageAssets.map((page) => page.instagramBusinessAccountId).filter(Boolean);
  return {
    source: args.source,
    flowState,
    oauthState,
    ...
    updatedAt: timestamp,
    app: {
      appId: normalizeOptionalString(args.setupContext?.appId) || metaAppId || null,
      configId: normalizeOptionalString(args.setupContext?.configId),
      graphVersion: normalizeOptionalString(args.setupContext?.graphVersion) || graphVersion
    },
    identifiers: {
      businessIds: identifiers.businessIds,
      wabaIds: Array.from(/* @__PURE__ */ new Set([args.wabaId, ...identifiers.wabaIds])),
      phoneNumberIds: Array.from(/* @__PURE__ */ new Set([args.phoneNumberId, ...identifiers.phoneNumberIds])),
      pageIds: Array.from(/* @__PURE__ */ new Set([...identifiers.pageIds, ...pageIds])),
      instagramAccountIds: Array.from(
        /* @__PURE__ */ new Set([...identifiers.instagramAccountIds, ...instagramAccountIds])
      ),
    ...
    rawEmbeddedSignupContext: args.source === "waba_embedded_signup" ? sanitizedSetupContext : null
  };
}
...
function getRawMetaAdAccountNumber(value) {
  const normalized = normalizeOptionalIdentifier(value);
  if (!normalized) {
    return null;
  }
...
function isSameMetaAdAccountId(option, value) {
  const normalized = normalizeMetaAdAccountId(value);
  if (!normalized) {
    return false;
  }
  return option.adAccountId === normalized || normalizeMetaAdAccountId(option.accountId) === normalized || normalizeMetaAdAccountId(option.adAccountId) === normalized;
}
function normalizeMetaAdAccountStatus(value) {
  if (typeof value === "number" && Number.isFinite(value)) {
    return value;
  }
  if (typeof value === "string") {
...
function normalizeMetaOAuthFlowState(value) {
  const normalized = normalizeOptionalString(value);
  if (normalized === "core_onboarding" || normalized === "ads_flow" || normalized === "catalog_flow" || normalized === "lead_capture_flow" || normalized === "instagram_flow" || normalized === "messenger_flow") {
    return normalized;
  }
  return null;
}
function assertMetaOAuthFlowState(value, expected) {
...
async function fetchMetaAdsAssets(accessToken) {
  ...
  const permissions = (permissionsResponse.data || []).filter((permission) => (permission.status || "").toLowerCase() === "granted").map((permission) => normalizeOptionalString(permission.permission)).filter((permission) => Boolean(permission));
  return {
    pages,
    adAccounts,
    permissions,
...
function mapMetaAdsIntegration(row) {
  if (!row) {
    return null;
  }
  return {
...
function getMetaAdsSelectedAssets(config) {
  return {
    pages: config?.pageId ? [
      {
...
async function getMetaAdsIntegrationSetup(userId, options = {}) {
  const result = await adminSupabase.from("meta_ads_integrations").select("*").eq("user_id", userId).maybeSingle();
  if (result.error) {
  ...
  const config = mapMetaAdsIntegration(result.data);
...
async function saveMetaAdsIntegration(userId, input) {
  ...
  const existingResult = await adminSupabase.from("meta_ads_integrations").select("*").eq("user_id", userId).maybeSingle();
  ...
  const accessToken = providedToken || (storedTokenCiphertext ? decryptAccessToken(storedTokenCiphertext) : null);
  if (!accessToken) {
  ...
  const selectedAdAccount = assets.adAccounts.find(
    (account) => isSameMetaAdAccountId(account, selectedAdAccountId)
  );
  if (!selectedPage) {
  ...
  const { error } = await adminSupabase.from("meta_ads_integrations").upsert(payload, { onConflict: "user_id" });
  if (error) {
    throw error;
  }
  return getMetaAdsIntegrationSetup(userId, { includeLiveAssets: true });
}
async function disconnectMetaAdsIntegration(userId) {
  const { error } = await adminSupabase.from("meta_ads_integrations").delete().eq("user_id", userId);
  if (error && !isMissingSchemaError(error)) {
    throw error;
  }
  return { config: null, pages: [], adAccounts: [] };
}
async function fetchMetaAdsLeadForms(pageAccessToken, pageId) {
  const response = await metaRequestDetailed({
  ...
  return (response.data || []).map((form) => {
    ...
    const questions = Array.isArray(form.questions) ? form.questions.map((question) => {
      ...
      return normalizeOptionalString(question.label) || normalizeOptionalString(question.key) || normalizeOptionalString(question.type);
    }).filter((entry) => Boolean(entry)) : [];
    return {
      formId,
      pageId,
...
async function fetchMetaAdsPixels(accessToken, adAccountId) {
  const response = await metaRequestDetailed({
    accessToken,
...
async function fetchMetaAdsWhatsAppAccounts(userId) {
  const result = await adminSupabase.from("meta_channels").select(
...
async function getMetaAdsCreationSetup(userId) {
...
function normalizeMetaAdsCampaignPeriod(value) {
  const normalized = normalizeOptionalString(value);
  if (normalized === "last_7d" || normalized === "last_30d" || normalized === "this_month" || normalized === "last_month" || normalized === "maximum" || normalized === "custom") {
    return normalized;
  }
  return "last_30d";
}
function normalizeMetaAdsDate(value) {
  const normalized = normalizeOptionalString(value);
  if (!normalized || !/^\d{4}-\d{2}-\d{2}$/.test(normalized)) {
    return null;
  }
  const date = /* @__PURE__ */ new Date(`${normalized}T00:00:00.000Z`);
...
function parseMetaBudgetAmount(value) {
...
function getMetaActionLabel(actionType) {
...
function getMetaAdsResult(insights) {
...
function findMetaCreativeText(value) {
  if (typeof value === "string") {
    return normalizeOptionalString(value);
  }
  if (Array.isArray(value)) {
...
function findMetaCreativeImageUrl(value) {
  if (!isRecord(value)) {
    return null;
  }
  ...
  for (const entry of Object.values(value)) {
    if (Array.isArray(entry)) {
      for (const item of entry) {
        const found2 = findMetaCreativeImageUrl(item);
        if (found2) {
          return found2;
        }
      }
...
function mapMetaManagedAd(rawAd) {
...
function getMetaAdsCampaignBudget(campaign, ads) {
  ...
  const adsetBudgets = /* @__PURE__ */ new Map();
  for (const ad of ads) {
    const adset = isRecord(ad.raw.adset) ? ad.raw.adset : null;
    const adsetId = ad.adsetId || ad.id;
    const adsetBudget = parseMetaBudgetAmount(adset?.daily_budget) ?? parseMetaBudgetAmount(adset?.lifetime_budget);
  ...
  return totalAdsetBudget > 0 ? { budgetAllocated: totalAdsetBudget, budgetAllocatedType: "adset" } : { budgetAllocated: null, budgetAllocatedType: null };
}
function getMetaAdsInsightsQuery(period, since, until) {
  const query = {
...
async function getStoredMetaAdsIntegrationWithToken(userId) {
  const result = await adminSupabase.from("meta_ads_integrations").select("*").eq("user_id", userId).maybeSingle();
  if (result.error) {
    if (isMissingSchemaError(result.error)) {
...
async function fetchMetaAdsCampaignRows(accessToken, adAccountId) {
  const campaigns = [];
  let after;
  do {
    const response = await metaRequestDetailed({
      ...
      query: {
        fields: "id,name,objective,status,effective_status,daily_budget,lifetime_budget,budget_remaining,spend_cap,start_time,stop_time,created_time,updated_time,buying_type",
        limit: 100,
        after
      }
    });
    campaigns.push(...(response.data || []).filter(isRecord));
    after = normalizeOptionalString(response.paging?.cursors?.after) || void 0;
...
function mapMetaAdsMediaAsset(rawAsset) {
  ...
  return {
    ...
    thumbnailUrl: normalizeOptionalString(rawAsset.url_128) || normalizeOptionalString(rawAsset.url) || normalizeOptionalString(rawAsset.permalink_url),
...
async function fetchMetaAdsMediaLibrary(userId) {
  ...
  do {
    ...
    assets.push(
      ...(response.data || []).filter(isRecord).map(mapMetaAdsMediaAsset).filter((asset) => Boolean(asset))
    );
    after = normalizeOptionalString(response.paging?.cursors?.after) || void 0;
...
async function fetchMetaAdsCampaignAds(accessToken, campaignId) {
  const ads = [];
  let after;
  do {
    const response = await metaRequestDetailed({
      ...
      path: `${campaignId}/ads`,
      query: {
        fields: "id,name,status,effective_status,created_time,updated_time,adset{id,name,daily_budget,lifetime_budget,budget_remaining},creative{id,name,thumbnail_url,object_story_spec}",
        limit: 100,
        after
      }
    });
...
async function fetchMetaAdsCampaignInsight(accessToken, campaignId, period, since, until) {
  const response = await metaRequestDetailed({
    accessToken,
...
async function fetchMetaAdsCampaigns(userId, query) {
  const { config, accessToken } = await getStoredMetaAdsIntegrationWithToken(userId);
  const adAccountId = config.adAccountId;
  if (!adAccountId) {
  ...
  return {
    config,
    campaigns: campaigns.filter((campaign) => Boolean(campaign)),
    period,
    since,
    until
  };
}
function normalizeMetaAdsCampaignStatusInput(value) {
...
async function updateMetaAdsCampaignStatus(userId, campaignId, input) {
...
async function fetchMessengerPage(accessToken, pageId) {
  return metaRequestDetailed({
    accessToken,
    path: pageId,
    query: {
...
async function listWhatsAppSubscribedApps(accessToken, wabaId) {
  return metaRequestDetailed({
    accessToken,
    path: `${wabaId}/subscribed_apps`
  });
}
async function unsubscribeWhatsAppWebhookApp(accessToken, wabaId) {
  return metaRequestDetailed({
    accessToken,
    path: `${wabaId}/subscribed_apps`,
    method: "DELETE"
...
function getWhatsAppSubscribedAppData(entry) {
  const nestedApiData = isRecord(entry.whatsapp_business_api_data) ? entry.whatsapp_business_api_data : null;
  return nestedApiData || entry;
}
function getMetaChannelMetadata(row) {
  return isRecord(row.metadata) ? row.metadata : {};
}
function getNormalizedIdentifierList(value) {
  const entries = Array.isArray(value) ? value : [value];
  return Array.from(
    new Set(
      entries.map((entry) => normalizeOptionalIdentifier(entry)).filter((entry) => Boolean(entry))
    )
  );
}
function hasSharedIdentifiers(left, right) {
...
function getCatalogWebhookMetadata(metadata) {
  return isRecord(metadata.catalogWebhook) ? metadata.catalogWebhook : {};
}
function getCatalogConnectionMetadata(metadata) {
  return isRecord(metadata.catalogConnection) ? metadata.catalogConnection : {};
}
function buildMetaCatalogWebhookSetupResponse(req, row) {
  const metadata = row ? getMetaChannelMetadata(row) : {};
  const catalogWebhook = getCatalogWebhookMetadata(metadata);
  const reusableMetaSetup = isRecord(metadata.reusableMetaSetup) ? metadata.reusableMetaSetup : {};
  const reusableIdentifiers = isRecord(reusableMetaSetup.identifiers) ? reusableMetaSetup.identifiers : {};
  return {
...
function getMetaCatalogSelectionMetadata(metadata) {
  return isRecord(metadata.catalogSelection) ? metadata.catalogSelection : {};
}
function getMetaCatalogConnectionContext(row) {
  ...
  return {
    metadata,
    businessIds: Array.from(/* @__PURE__ */ new Set([...businessIds, ...connectedBusinessIds])),
    catalogIds: Array.from(/* @__PURE__ */ new Set([...embeddedCatalogIds, ...webhookCatalogIds, ...connectedCatalogIds])),
    selectedCatalogId: normalizeOptionalIdentifier(catalogSelection.selectedCatalogId)
  };
}
function resolveMetaCatalogAccessToken(row, fallbackAccessToken) {
  const metadata = getMetaChannelMetadata(row);
  const catalogConnection = getCatalogConnectionMetadata(metadata);
...
function parseMetaOptionalNumber(value) {
  if (typeof value === "number" && Number.isFinite(value)) {
    return value;
  }
  const normalized = normalizeOptionalString(value);
...
function mapMetaCatalogSummary(record) {
...
async function fetchMetaCatalog(accessToken, catalogId) {
  const response = await metaRequestDetailed({
    accessToken,
    path: catalogId,
    query: {
      fields: "id,name,vertical,product_count,feed_count,default_image_url,is_catalog_segment,is_local_catalog,business{id,name}"
    }
  });
...
async function listOwnedMetaCatalogs(accessToken, businessId) {
  const response = await metaRequestDetailed({
    accessToken,
  ...
  return Array.isArray(response.data) ? response.data.map((entry) => mapMetaCatalogSummary(entry)).filter((entry) => Boolean(entry)) : [];
}
async function listMetaBusinessesForCatalogConnection(accessToken) {
  const response = await metaRequestDetailed({
    accessToken,
    path: "me/businesses",
    query: {
  ...
  return Array.isArray(response.data) ? response.data.map((entry) => normalizeOptionalIdentifier(entry.id)).filter((entry) => Boolean(entry)) : [];
}
...
async function listMetaCatalogsForChannel(row, accessToken) {
  ...
  return {
    ...
    selectedCatalogId: context.selectedCatalogId && catalogMap.has(context.selectedCatalogId) ? context.selectedCatalogId : null,
    catalogs: Array.from(catalogMap.values()).sort(
      (left, right) => (left.name || left.id).localeCompare(right.name || right.id)
    )
  };
}
...
function mapMetaCatalogProduct(record) {
  const id = normalizeOptionalIdentifier(record.id);
  if (!id) {
    return null;
  }
  return {
    ...
    imageUrl: normalizeOptionalString(record.image_url) || normalizeOptionalString(record.image_link),
...
async function listMetaCatalogProducts(args) {
  ...
  return {
    ...
    catalog,
    products: Array.isArray(response.data) ? response.data.map((entry) => mapMetaCatalogProduct(entry)).filter((entry) => Boolean(entry)) : []
  };
}
...
async function findMetaChannelRowsForCatalogWebhookPayload(payload) {
  const identifiers = collectReusableMetaIdentifiers(payload);
  const hasAnyIdentifiers = identifiers.businessIds.length > 0 || identifiers.wabaIds.length > 0 || identifiers.phoneNumberIds.length > 0 || identifiers.catalogIds.length > 0;
  ...
  const { data, error } = await adminSupabase.from("meta_channels").select("id,user_id,waba_id,phone_number_id,business_account_name,metadata");
  if (error) {
    throw error;
  }
  const rows = (data || []).filter((row) => {
    ...
    return hasSharedIdentifiers(identifiers.businessIds, rowBusinessIds) || hasSharedIdentifiers(identifiers.wabaIds, rowWabaIds) || hasSharedIdentifiers(identifiers.phoneNumberIds, rowPhoneNumberIds) || hasSharedIdentifiers(identifiers.catalogIds, rowCatalogIds);
  });
  return { identifiers, rows };
}
async function getMetaChannelRow(userId) {
...
function resolveWhatsAppWebhookSubscriptionEntry(entries) {
  if (!entries.length) {
    return null;
  }
  if (metaAppId) {
...
async function persistWhatsAppWebhookSubscriptionStatus(args) {
  ...
  const existingSubscription = isRecord(existingMetadata.webhookSubscription) ? existingMetadata.webhookSubscription : {};
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  const matchedEntry = args.isSubscribed ? resolveWhatsAppWebhookSubscriptionEntry(args.entries) : null;
  ...
  const { data, error } = await adminSupabase.from("meta_channels").update({
    metadata: {
      ...
      webhookSubscription: {
        ...
        appId: normalizeOptionalIdentifier(apiData?.id) || (args.isSubscribed ? metaAppId || null : metaAppId || null),
        ...
        subscribedAt: args.isSubscribed ? normalizeOptionalString(existingSubscription.subscribedAt) || timestamp : null,
...
async function syncWhatsAppWebhookSubscription(args) {
  try {
    ...
    const entries = Array.isArray(subscriptions.data) ? subscriptions.data.filter((entry) => isRecord(entry)) : [];
...
async function subscribeMessengerPageToWebhook(accessToken, pageId) {
  return metaRequestDetailed({
    accessToken,
...
async function subscribeInstagramPageToWebhook(accessToken, pageId) {
...
async function subscribeMetaAppToInstagramWebhook(fields) {
  if (!instagramAppId || !instagramAppSecret) {
...
async function subscribeInstagramAccountToWebhook(accessToken, instagramAccountId) {
  let appWebhookLastError = null;
  try {
...
async function fetchInstagramLoginProfile(accessToken) {
  return metaRequestDetailed({
    accessToken,
    graphHost: "graph.instagram.com",
    path: "me",
...
var messengerUserProfileCache = /* @__PURE__ */ new Map();
var instagramMessagingUserProfileCache = /* @__PURE__ */ new Map();
function getMetaAccessTokenCacheKey(accessToken) {
  return crypto.createHash("sha256").update(accessToken).digest("hex").slice(0, 16);
}
function getCachedMetaUserProfile(cache, key) {
...
function setCachedMetaUserProfile(cache, key, value) {
  cache.set(key, {
    ok: true,
    value,
...
function setCachedMetaUserProfileError(cache, key, error) {
  cache.set(key, {
    ok: false,
...
async function fetchMessengerUserProfile(accessToken, senderId) {
  ...
  try {
    try {
      response = await metaRequestDetailed({
        accessToken,
        path: senderId,
        query: {
  ...
  const profile = {
    ...
    profilePictureUrl: normalizeOptionalString(response.profile_pic) || normalizeOptionalString(response.profile_picture_url)
  };
  setCachedMetaUserProfile(messengerUserProfileCache, cacheKey, profile);
  return profile;
}
async function fetchMessengerPageConversations(accessToken, pageId) {
  const response = await metaRequestDetailed({
    accessToken,
...
async function fetchInstagramConversations(accessToken, instagramAccountId) {
  const response = await metaRequestDetailed({
    accessToken,
    graphHost: "graph.instagram.com",
    version: instagramGraphVersion,
...
async function fetchInstagramConversationMessages(accessToken, conversationId) {
  const response = await metaRequestDetailed({
    accessToken,
    graphHost: "graph.instagram.com",
    version: instagramGraphVersion,
...
async function fetchInstagramConversationMessageDetail(accessToken, messageId) {
  return metaRequestDetailed({
    accessToken,
    graphHost: "graph.instagram.com",
    version: instagramGraphVersion,
...
function mapInstagramMessagingUserProfile(response, senderId) {
  const profilePictureUrl = normalizeOptionalString(response.profile_pic) || normalizeOptionalString(response.profile_picture_url);
...
async function fetchInstagramMessagingUserProfile(args) {
  const tokenCacheKeyParts = [args.pageAccessToken, args.userAccessToken].map((token) => token ? getMetaAccessTokenCacheKey(token) : "none").join(":");
  ...
  const addAttempt = (accessToken, graphHost, fields) => {
...
async function fetchInstagramMessagingUserProfileForChannel(channelRow, senderId) {
...
async function listTemplates(accessToken, wabaId) {
  const templates = [];
...
async function createRemoteTemplate(accessToken, wabaId, input) {
  return metaRequest({
    accessToken,
    path: `${wabaId}/message_templates`,
    method: "POST",
...
async function deleteRemoteTemplate(accessToken, wabaId, templateName) {
  return metaRequest({
    accessToken,
    path: `${wabaId}/message_templates`,
    method: "DELETE",
...
function normalizeFlowCategory(value) {
...
function normalizeFlowCategories(input) {
  const source = Array.isArray(input.categories) ? input.categories : [input.category];
  const categories = source.map((value) => normalizeFlowCategory(value)).filter((value) => Boolean(value));
  const uniqueCategories = Array.from(new Set(categories));
  return uniqueCategories.length > 0 ? uniqueCategories.slice(0, 3) : ["OTHER"];
}
function normalizeFlowFieldType(value) {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
...
function normalizeFlowFieldId(value, label, index, seenIds) {
  const source = normalizeOptionalString(value) || label || `field_${index + 1}`;
  let base = source.toLowerCase().replace(/[^a-z0-9_]+/g, "_").replace(/^_+|_+$/g, "");
  if (!base) {
    base = `field_${index + 1}`;
  }
  if (/^\d/.test(base)) {
...
function normalizeFlowSelectOptions(value, fieldLabel) {
  const options = Array.isArray(value) ? value.map((entry) => normalizeOptionalString(entry)).filter((entry) => Boolean(entry)) : [];
...
function normalizeFlowSchema(value, options) {
  if (!Array.isArray(value)) {
...
function normalizeStoredFlowSchema(value) {
  try {
...
function normalizeFlowCreateInput(input) {
  if (!isRecord(input)) {
...
function normalizeFlowUpdateInput(input) {
  if (!isRecord(input)) {
    throw new Error("Flow details are required.");
  }
  const update = {};
...
function buildWhatsAppFlowFieldComponent(field) {
  if (field.type === "date") {
    return {
      type: "DatePicker",
      name: field.id,
  ...
  if (field.type === "select") {
    return {
      ...
      "data-source": (field.options || []).map((option) => ({
        id: option.toLowerCase().replace(/[^a-z0-9_]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 80) || option.slice(0, 80),
        title: option
      }))
    };
  }
  const inputType = field.type === "email" || field.type === "phone" || field.type === "number" ? field.type : "text";
  return {
    type: "TextInput",
    name: field.id,
    label: field.label,
...
function buildWhatsAppFlowJson(input) {
  const payload = input.schema.reduce((current, field) => {
...
function getFlowValidationMessage(raw) {
  const validationErrors = Array.isArray(raw?.validation_errors) ? raw.validation_errors : [];
  const firstError = validationErrors.find((entry) => isRecord(entry));
  if (!firstError) {
    return null;
  }
  return normalizeOptionalString(firstError.message) || normalizeOptionalString(firstError.error) || "Flow JSON has validation errors.";
}
async function listRemoteFlows(accessToken, wabaId) {
  const response = await metaRequest({
    accessToken,
    path: `${wabaId}/flows`,
    query: {
...
async function createRemoteFlow(accessToken, wabaId, input) {
  return metaRequestDetailed({
    accessToken,
    path: `${wabaId}/flows`,
    method: "POST",
...
async function updateRemoteFlowMetadata(accessToken, flowId, input) {
  const body = {};
  if (input.name) {
...
async function updateRemoteFlowJson(accessToken, flowId, flowJson) {
...
async function publishRemoteFlow(accessToken, flowId) {
  return metaRequestDetailed({
    accessToken,
    path: `${flowId}/publish`,
    method: "POST"
...
async function deleteRemoteFlow(accessToken, flowId) {
  return metaRequestDetailed({
    accessToken,
    path: flowId,
    method: "DELETE"
...
async function fetchRemoteFlowPreview(accessToken, flowId) {
  return metaRequestDetailed({
    accessToken,
    path: flowId,
    query: {
      fields: "id,name,categories,preview.invalidate(false),status,validation_errors,json_version,data_api_version,endpoint_uri,health_status"
    }
  });
}
function getRemoteFlowPreview(raw) {
...
function getRemoteFlowCategories(raw) {
  return Array.isArray(raw.categories) ? raw.categories.map((entry) => normalizeFlowCategory(entry)).filter((entry) => Boolean(entry)) : [];
}
function mapFlow(row) {
  const raw = isRecord(row.raw) ? row.raw : {};
  const preview = getRemoteFlowPreview(raw);
  const categories = Array.isArray(row.categories) ? row.categories.map((entry) => normalizeFlowCategory(entry)).filter((entry) => Boolean(entry)) : [];
  return {
    id: String(row.id),
    userId: String(row.user_id),
...
function getFlowJsonFromRaw(raw) {
...
function normalizeTemplateFlowJson(value) {
  if (typeof value === "string") {
    ...
    try {
      ...
      return isRecord(parsed) ? { text: trimmed, raw: parsed } : null;
    } catch {
...
async function getStoredFlows(userId) {
  const { data, error } = await adminSupabase.from("meta_flows").select("*").eq("user_id", userId).order("updated_at", { ascending: false });
  if (error) {
    throw error;
  }
  return (data || []).map((row) => mapFlow(row));
}
async function getStoredFlowRow(userId, flowId) {
  const { data, error } = await adminSupabase.from("meta_flows").select("*").eq("user_id", userId).eq("id", flowId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function findStoredFlowRowByLocalOrMetaId(userId, flowId) {
  const { data: localData, error: localError } = await adminSupabase.from("meta_flows").select("*").eq("user_id", userId).eq("id", flowId).maybeSingle();
  if (localError) {
    throw localError;
  }
  if (localData) {
  ...
  const { data: metaData, error: metaError } = await adminSupabase.from("meta_flows").select("*").eq("user_id", userId).eq("meta_flow_id", flowId).maybeSingle();
  if (metaError) {
    throw metaError;
  }
  return metaData ? metaData : null;
}
async function findStoredFlowRowByName(userId, flowName) {
  const { data, error } = await adminSupabase.from("meta_flows").select("*").eq("user_id", userId).eq("flow_name", flowName).order("updated_at", { ascending: false }).limit(1).maybeSingle();
  if (error) {
    throw error;
  }
  return data ? data : null;
}
function parseFlowResponseJson(value) {
  const raw = normalizeOptionalString(value);
  if (!raw) {
    return null;
  }
...
function getFlowReplyFromMessage(message) {
  ...
  const reply = isRecord(interactive?.nfm_reply) ? interactive.nfm_reply : null;
...
function extractFlowIdentifierFromToken(flowToken) {
  ...
  if (localMatch?.[1]) {
    return { type: "local_or_meta_id", value: localMatch[1] };
  }
  const metaMatch = token.match(/(?:^|:)meta-flow:([^:]+)(?:$|:)/i);
  if (metaMatch?.[1]) {
    return { type: "local_or_meta_id", value: metaMatch[1] };
  }
  const nameMatch = token.match(/(?:^|:)flow-name:([^:]+)(?:$|:)/i);
  if (nameMatch?.[1]) {
    return { type: "name", value: decodeURIComponent(nameMatch[1]) };
  }
  return null;
}
async function resolveFlowRowForSubmission(userId, responses) {
...
function toContactAttributeKey(value) {
  return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 80);
}
function getFlowResponseFields(responses) {
...
async function mergeFlowResponseAttributes(args) {
  const { data, error } = await adminSupabase.from("conversation_threads").select("attributes").eq("user_id", args.userId).eq("id", args.threadId).maybeSingle();
  if (error) {
    throw error;
  }
  const existingAttributes = isRecord(data?.attributes) ? data?.attributes : {};
  ...
  const update = await adminSupabase.from("conversation_threads").update({
    attributes: nextAttributes,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("user_id", args.userId).eq("id", args.threadId);
  if (update.error) {
    throw update.error;
  }
}
async function recordFlowSubmission(args) {
  ...
  if (flow) {
    const { error: updateError } = await adminSupabase.from("meta_flows").update({
      submission_count: flow.submissionCount + 1,
      last_submitted_at: args.submittedAt,
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("user_id", args.userId).eq("id", flow.id);
    if (updateError) {
      throw updateError;
    }
  }
...
async function syncFlows(userId) {
  ...
  const existingResult = await adminSupabase.from("meta_flows").select("*").eq("user_id", userId);
  if (existingResult.error) {
    throw existingResult.error;
  }
  const existingByMetaFlowId = /* @__PURE__ */ new Map();
  for (const existingFlow of existingResult.data || []) {
    const metaFlowId = normalizeOptionalIdentifier(existingFlow.meta_flow_id);
    if (metaFlowId) {
      existingByMetaFlowId.set(metaFlowId, existingFlow);
    }
  }
  ...
  for (const remoteFlow of remoteFlows) {
    ...
    const existingRaw = isRecord(existingFlow?.raw) ? existingFlow?.raw : {};
    const raw = {
      ...existingRaw,
      ...remoteFlow,
    ...
    const categories = remoteCategories.length > 0 ? remoteCategories : Array.isArray(existingFlow?.categories) ? existingFlow.categories.map((entry) => normalizeFlowCategory(entry)).filter((entry) => Boolean(entry)) : ["OTHER"];
    const { error } = await adminSupabase.from("meta_flows").upsert(
      {
        user_id: userId,
        meta_channel_id: channelId,
        meta_flow_id: metaFlowId,
        flow_name: normalizeOptionalString(remoteFlow.name) || normalizeOptionalString(existingFlow?.flow_name) || "Untitled Flow",
        status: normalizeOptionalString(remoteFlow.status)?.toUpperCase() || normalizeOptionalString(existingFlow?.status)?.toUpperCase() || "DRAFT",
        categories,
...
function normalizeMarketingMessageProductPolicy(value) {
  if (value === "CLOUD_API_FALLBACK" || value === "STRICT") {
    return value;
  }
  return null;
}
function normalizeMarketingCampaignRecipients(value) {
  if (!Array.isArray(value)) {
    throw new Error("At least one campaign recipient is required.");
  }
  const dedupedRecipients = /* @__PURE__ */ new Map();
  for (const entry of value) {
    if (!isRecord(entry)) {
      continue;
    }
...
function normalizeMarketingMessageStatus(value) {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
  if (!normalized) {
    return null;
  }
  if (normalized === "accepted" || normalized === "held_for_quality_assessment" || normalized === "paused") {
    return normalized;
  }
  return normalized;
}
function normalizeOutgoingWhatsAppRecipient(value) {
  return normalizePhoneLike(value) || normalizeOptionalString(value);
}
function normalizeOutgoingMessageContext(value) {
  if (!isRecord(value)) {
    return void 0;
  }
  const messageId = normalizeOptionalString(value.message_id);
  return messageId ? { message_id: messageId } : void 0;
}
function normalizeOutgoingMediaObject(value, options) {
  if (!isRecord(value)) {
...
function normalizeOutgoingInteractiveHeader(value) {
  if (!isRecord(value)) {
    return void 0;
  }
...
function normalizeOutgoingInteractiveObject(value) {
  if (!isRecord(value)) {
...
function normalizeOutgoingFlowInteractiveAction(value) {
  if (!isRecord(value)) {
  ...
  if (normalizedParameters.flow_action === "navigate") {
    const payload = isRecord(parameters.flow_action_payload) ? parameters.flow_action_payload : null;
...
function normalizeOutgoingContacts(value) {
  if (!Array.isArray(value)) {
...
function normalizeOutgoingLocation(value) {
  if (!isRecord(value)) {
...
function normalizeOutgoingWhatsAppMessagePayload(payload) {
  if (!isRecord(payload)) {
  ...
  switch (type) {
    ...
    case "template": {
      ...
      const languageCode = isRecord(payload.template.language) ? normalizeOptionalString(payload.template.language.code) : null;
...
async function sendRemoteWhatsAppMessage(accessToken, phoneNumberId, payload) {
  return metaRequest({
    accessToken,
...
async function sendRemoteInstagramTextMessage(args) {
  ...
  throw new Error(
    errors.length > 0 ? `Failed to send the Instagram message. ${errors.join(" | ")}` : "Failed to send the Instagram message."
  );
}
async function sendRemoteWhatsAppMarketingMessage(accessToken, phoneNumberId, payload, options) {
...
async function uploadRemoteMedia(accessToken, phoneNumberId, file) {
  const formData = new FormData();
...
async function uploadTemplateHeaderMedia(accessToken, file) {
  ...
  const uploadSession = await metaRequestDetailed({
    accessToken,
    path: `${metaAppId}/uploads`,
    method: "POST",
    query: {
  ...
  const headerMediaHandle = normalizeOptionalString(uploaded.h) || normalizeOptionalString(uploaded.handle) || normalizeOptionalString(uploaded.id);
...
function getTemplateHeaderMediaExtension(fileName, mimeType) {
...
async function saveTemplateHeaderMediaPreview(userId, file) {
  ...
  try {
    const { error } = await adminSupabase.storage.from(TEMPLATE_HEADER_MEDIA_BUCKET).upload(storageObjectPath, file.buffer, {
...
async function saveTemplateHeaderMediaPreviewWithFallback(req, userId, file) {
  try {
...
function normalizeTemplateHeaderMediaPreview(value) {
  if (!isRecord(value)) {
    return null;
  }
...
function mergeTemplateHeaderMediaPreview(raw, preview) {
  if (!preview) {
    return raw;
  }
  return {
...
function normalizeTemplateHeaderMediaPreviewUrlForSend(value) {
  try {
...
function resolveTemplateHeaderMediaPreviewLocalFile(value) {
  try {
    const parsed = new URL(value, frontendOrigin);
...
async function ensureTemplateHeaderMediaPreviewIsAvailable(preview) {
  const localFile = resolveTemplateHeaderMediaPreviewLocalFile(preview.url);
  if (!localFile) {
    return;
  }
  if (!await fs.promises.stat(localFile.resolvedFilePath).then((stats) => stats.isFile()).catch(() => false)) {
    throw new Error(
...
function getTemplateHeaderMediaPreviewFromRaw(raw) {
...
async function saveTemplateHeaderMediaPreviewMetadata(userId, templateName, language, preview) {
  const existingResult = await adminSupabase.from("meta_templates").select("*").eq("user_id", userId).eq("template_name", templateName).eq("language", language).maybeSingle();
  if (existingResult.error) {
    throw existingResult.error;
  }
  ...
  const { data, error } = await adminSupabase.from("meta_templates").update({
    raw: nextRaw
...
async function migrateTemplateHeaderMediaPreviewToDurableUrl(userId, templateName, language, preview) {
  const localFile = resolveTemplateHeaderMediaPreviewLocalFile(preview.url);
  if (!localFile) {
    return preview;
  }
  const exists = await fs.promises.stat(localFile.resolvedFilePath).then((stats) => stats.isFile()).catch(() => false);
  if (!exists) {
    return preview;
  }
  try {
...
async function fetchRemoteMediaMetadata(accessToken, mediaId) {
  return metaRequest({
    accessToken,
    path: mediaId
  });
...
function mapProfile(row) {
  ...
  return {
    ...
    preferredCurrency: normalizeOptionalString(row.preferred_currency) ? normalizeCurrencyCode(row.preferred_currency) : null,
...
function mapWorkspaceTeamMember(row, profileRow) {
  ...
  return {
    ...
    invitedAt: String(row.invite_sent_at || row.created_at || (/* @__PURE__ */ new Date()).toISOString()),
...
function mapNotificationPreferences(row, userId) {
...
function mapChannel(row) {
  ...
  if (isRecord(metadata.catalogConnection)) {
    const catalogConnection = { ...metadata.catalogConnection };
...
function mapInstagramChannel(row) {
  if (!row) {
    return null;
  }
  if (row.status === "disconnected") {
  ...
  const webhookSubscription = isRecord(metadata.webhookSubscription) ? metadata.webhookSubscription : {};
  return {
    id: String(row.id),
    userId: String(row.user_id),
    connectionMethod: row.connection_method,
...
function mapMessengerChannel(row) {
  if (!row) {
    return null;
  }
  if (row.status === "disconnected") {
...
function mapEmailConnection(row) {
  if (!row) {
    return null;
  }
  return {
    ...
    status: normalizeOptionalString(row.status) || "connected",
...
function mapEmailCampaign(row) {
  return {
    ...
    status: row.status === "partial" || row.status === "failed" ? row.status : "sent",
...
function getThreadSource(row) {
  const source = normalizeOptionalString(row.source)?.toLowerCase();
  return source || null;
}
function getThreadContactIdentity(row) {
  return normalizeOptionalString(row.contact_wa_id) || "";
}
function getPrefixedThreadIdentity(row, prefix) {
...
function isMessengerThreadRow(row) {
...
function isInstagramThreadRow(row) {
  const source = getThreadSource(row);
  const contactIdentity = getThreadContactIdentity(row).toLowerCase();
  return source === "instagram" || contactIdentity.startsWith("instagram:");
}
function isExternalMessagingThreadRow(row) {
  return isMessengerThreadRow(row) || isInstagramThreadRow(row);
}
function getExternalThreadDisplayIdentity(row, prefix) {
...
function getThreadDisplayPhone(row) {
  ...
  return formatContactIdentity(row.display_phone) || (normalizePhoneLike(row.contact_wa_id) ? formatContactIdentity(row.contact_wa_id) : null) || null;
}
...
function mapMessage(row) {
  ...
  return {
    ...
    senderWaId: direction === "inbound" ? formatContactIdentity(row.sender_wa_id) || normalizeOptionalString(row.sender_wa_id) : normalizeOptionalString(row.sender_wa_id),
    recipientWaId: direction === "outbound" ? formatContactIdentity(row.recipient_wa_id) || normalizeOptionalString(row.recipient_wa_id) : normalizeOptionalString(row.recipient_wa_id),
...
function mapCallSession(row) {
  return {
    ...
    bizOpaqueCallbackData: row.biz_opaque_callback_data || null,
    lastEvent: row.last_event || null,
    raw: row.raw || {}
  };
}
function mapMetaLeadCaptureConfig(row, callbackUrl) {
  ...
  return {
    ...
    pageIds: Array.isArray(row.page_ids) ? row.page_ids.filter(Boolean) : [],
    formIds: Array.isArray(row.form_ids) ? row.form_ids.filter(Boolean) : [],
    accessTokenLast4: normalizeOptionalString(row.access_token_last4),
    ...
    defaultLabels: Array.isArray(row.default_labels) ? row.default_labels.filter(Boolean) : [],
...
function mapMetaLeadCaptureEvent(row) {
  ...
  return {
    ...
    processingStatus: processingStatus === "processed" || processingStatus === "skipped" || processingStatus === "error" ? processingStatus : "received",
    errorMessage: normalizeOptionalString(row.error_message),
    raw: row.raw || {},
    createdAt: String(row.created_at)
  };
}
function normalizeMetaSubscribedFields(value) {
  if (Array.isArray(value)) {
    return Array.from(
      new Set(
        value.map((entry) => normalizeOptionalString(entry)?.toLowerCase()).filter((entry) => Boolean(entry))
      )
    );
  }
  if (typeof value === "string") {
    return Array.from(
      new Set(
        value.split(/[,\s]+/).map((entry) => entry.trim().toLowerCase()).filter(Boolean)
      )
    );
  }
  return [];
...
async function insertCallLog(args) {
  ...
  const result = payload.call_id ? await query.upsert(payload, { onConflict: "user_id,call_id" }).select("*").single() : await query.insert(payload).select("*").single();
...
async function syncCallLogFromSession(userId, session) {
  ...
  const durationSeconds = Number.isFinite(startedAtMs) && Number.isFinite(endedAtMs) && endedAtMs >= startedAtMs ? Math.round((endedAtMs - startedAtMs) / 1e3) : 0;
...
function isWhatsAppCloudMessageId(value) {
  const messageId = normalizeOptionalString(value);
  return Boolean(messageId && messageId.startsWith("wamid."));
}
async function markRemoteWhatsAppMessageRead(accessToken, phoneNumberId, messageId) {
  const response = await metaRequestDetailed({
    accessToken,
    path: `${phoneNumberId}/messages`,
    method: "POST",
...
async function upsertCallSummaryMessage(args) {
  ...
  const createdAt = session.endedAt || session.updatedAt || (/* @__PURE__ */ new Date()).toISOString();
  const connectedAtMs = session.connectedAt ? Date.parse(session.connectedAt) : Number.NaN;
  const endedAtMs = session.endedAt ? Date.parse(session.endedAt) : Number.NaN;
  const durationSeconds = Number.isFinite(connectedAtMs) && Number.isFinite(endedAtMs) && endedAtMs >= connectedAtMs ? Math.round((endedAtMs - connectedAtMs) / 1e3) : 0;
...
async function maybeCreateMissedCallNotification(session, userId) {
  if (session.state !== "missed") {
    return;
  }
  const callerLabel = session.contactName || session.displayPhone || session.contactWaId || "a WhatsApp caller";
  await createUserNotification({
    userId,
    type: "missed_call",
...
async function handleCallWebhookEntry(args) {
  const callId = normalizeOptionalString(args.callRecord.id) || normalizeOptionalString(args.callRecord.call_id) || null;
  if (!callId) {
    return null;
  }
  ...
  const eventName = normalizeOptionalString(args.callRecord.event) || normalizeOptionalString(args.callRecord.status) || normalizeOptionalString(args.callRecord.call_status);
  const statusName = normalizeOptionalString(args.callRecord.status);
  const startedAt = toIsoTimestamp(
    args.callRecord.timestamp || args.callRecord.created_at || args.callRecord.updated_at
  );
  const contactWaId = direction === "incoming" ? extractPhoneLike(args.callRecord.from) || extractPhoneLike(args.callRecord.caller) || extractPhoneLike(args.callRecord.user) : extractPhoneLike(args.callRecord.to) || extractPhoneLike(args.callRecord.callee) || extractPhoneLike(args.callRecord.user);
  ...
  const endedAt = isTerminalCallState(nextState) ? (/* @__PURE__ */ new Date()).toISOString() : void 0;
  const connectedAt = nextState === "ongoing" ? (/* @__PURE__ */ new Date()).toISOString() : void 0;
  const sessionRecord = await upsertCallSession({
    userId: args.userId,
    metaChannelId: args.metaChannelId,
    callId,
    contactWaId: contactWaId || void 0,
    contactName: normalizeOptionalString(args.callRecord.contact_name) || normalizeOptionalString(args.callRecord.contactName) || args.fallbackContactName || void 0,
    displayPhone: contactWaId || void 0,
    direction,
    state: nextState,
    startedAt: startedAt || void 0,
    connectedAt,
    endedAt,
    offerSdp: sessionSdpType === "offer" ? sessionSdp : void 0,
    answerSdp: sessionSdpType === "answer" ? sessionSdp : void 0,
    bizOpaqueCallbackData: normalizeOptionalString(args.callRecord.biz_opaque_callback_data) || normalizeOptionalString(args.callRecord.bizOpaqueCallbackData) || void 0,
    lastEvent: eventName || statusName || void 0,
    raw: args.callRecord
  });
...
async function handleCallWebhookStatus(args) {
  ...
  const callId = normalizeOptionalString(args.statusRecord.call_id) || (explicitType === "call" ? normalizeOptionalString(args.statusRecord.id) : null);
  if (!callId) {
    return null;
  }
  ...
  const contactWaId = extractPhoneLike(args.statusRecord.from) || extractPhoneLike(args.statusRecord.to) || extractPhoneLike(args.statusRecord.user);
  const direction = normalizeCallDirection(
  ...
  const sessionRecord = await upsertCallSession({
    ...
    connectedAt,
    endedAt: isTerminalCallState(nextState || "connecting") ? (/* @__PURE__ */ new Date()).toISOString() : void 0,
    lastEvent: normalizeOptionalString(args.statusRecord.event) || normalizeOptionalString(args.statusRecord.status) || void 0,
...
function mapBusinessProfile(raw, channelRow, phoneSnapshot, officialBusinessAccountStatus) {
  return {
    ...
    websites: Array.isArray(raw.websites) ? raw.websites.map((website) => typeof website === "string" ? website : "").filter(Boolean) : [],
    vertical: normalizeOptionalString(raw.vertical),
    displayPhoneNumber: normalizeOptionalString(phoneSnapshot?.display_phone_number) || normalizeOptionalString(channelRow.display_phone_number),
    verifiedName: normalizeOptionalString(phoneSnapshot?.verified_name) || normalizeOptionalString(channelRow.verified_name),
    qualityRating: normalizeOptionalString(phoneSnapshot?.quality_rating) || normalizeOptionalString(channelRow.quality_rating),
...
function mapCommerceSettings(raw, channelRow) {
  return {
...
function mapConversationalAutomationConfig(row, args) {
  ...
  return {
    ...
    phoneNumberId: args.channelRow ? String(args.channelRow.phone_number_id) : normalizeOptionalString(row.phone_number_id),
    enableWelcomeMessage: Boolean(row.enable_welcome_message),
    prompts: Array.isArray(row.prompts) ? row.prompts.map((prompt) => normalizeConversationalAutomationPrompt(prompt)).filter((prompt) => Boolean(prompt)) : [],
...
function mapAutomationRule(row) {
  ...
  const filters = normalizeAutomationRuleFilters(action.filters) || buildLegacyAutomationFilterGroup(String(row.keyword || ""), normalizeAutomationRuleMatchMode(row.keyword_match_mode));
  return {
    id: String(row.id),
    userId: String(row.user_id),
    metaChannelId: normalizeOptionalString(row.meta_channel_id),
...
function getMessengerConversationCustomer(conversation, pageId) {
...
function getMessengerConversationMessageDirection(message, pageId) {
...
async function syncMessengerPageConversations(args) {
  ...
  for (const conversation of conversations) {
    ...
    const messages = (Array.isArray(conversation.messages?.data) ? conversation.messages.data : []).filter((message) => normalizeOptionalString(message.id) || normalizeOptionalString(message.message)).sort((left, right) => {
    ...
    const contactName = profile?.name || normalizeOptionalString(customer?.name) || null;
    const profilePictureUrl = profile?.profilePictureUrl || null;
    const latestMessageBody = normalizeOptionalString(latestMessage?.message) || (messages.length > 0 ? "Messenger conversation" : null);
    const latestMessageAt = toIsoTimestamp(latestMessage?.created_time || null) || (/* @__PURE__ */ new Date()).toISOString();
    const thread = await upsertThread({
      userId: args.userId,
      metaChannelId: null,
    ...
    for (const message of messages) {
      ...
      const existingMessageResult = await adminSupabase.from("conversation_messages").select("id").eq("user_id", args.userId).eq("wa_message_id", messageId).maybeSingle();
...
function getInstagramConversationCounterparty(message, instagramAccountId) {
...
function getInstagramConversationMessageDirection(message, instagramAccountId) {
  return normalizeOptionalIdentifier(message.from?.id) === instagramAccountId ? "outbound" : "inbound";
}
function getInstagramConversationMessageBody(message) {
...
async function syncInstagramConversations(args) {
  ...
  for (const conversation of conversations) {
    ...
    const messages = messageDetails.filter((message) => normalizeOptionalString(message.id)).sort((left, right) => {
    ...
    const latestMessageAt = toIsoTimestamp(latestMessage?.created_time) || toIsoTimestamp(conversation.updated_time) || (/* @__PURE__ */ new Date()).toISOString();
    const thread = await upsertThread({
      userId: args.userId,
      metaChannelId: null,
...
async function getBootstrap(user) {
  const [
    ...
    adminSupabase.from("conversation_threads").select("*").eq("user_id", user.id).order("last_message_at", { ascending: false, nullsFirst: false }),
  ...
  const mappedProfile = mapProfile(profileResult.data);
  let threadRows = threadsResult.data || [];
  if (threadRows.length > 0) {
    const changed = await ensureConversationThreadPhoneConsistency(user.id, threadRows);
    if (changed) {
      const refreshedThreadsResult = await adminSupabase.from("conversation_threads").select("*").eq("user_id", user.id).order("last_message_at", { ascending: false, nullsFirst: false });
      if (refreshedThreadsResult.error) {
        throw refreshedThreadsResult.error;
      }
      threadRows = refreshedThreadsResult.data || [];
    }
  }
  const connectedInstagramChannelRow = getConnectedChannelRow(
    instagramChannelResult.data || null
  );
  const connectedMessengerChannelRow = getConnectedChannelRow(
    messengerChannelResult.data || null
  );
  return {
    ...
    channel: mapChannel(channelResult.data),
    ...
    adsIntegration: mapMetaAdsIntegration(
      adsIntegrationResult.data
    ),
    templates: (templatesResult.data || []).map((row) => mapTemplate(row)),
    conversations: threadRows.map((row) => mapThread(row)),
    notifications: (notificationsResult.data || []).map(
      (row) => mapNotification(row)
    ),
    ...
    wallet,
    callHistory: (callHistoryResult.data || []).map((row) => mapCall(row)),
    callSessions: (callSessionsResult.data || []).map((row) => mapCallSession(row))
  };
}
async function getNotificationPreferencesForUser(userId) {
  const { data, error } = await adminSupabase.from("user_notification_preferences").select("*").eq("user_id", userId).maybeSingle();
  if (error && !isMissingSchemaError(error)) {
    throw error;
  }
  return mapNotificationPreferences(data || null, userId);
}
async function saveNotificationPreferences(userId, input) {
  ...
  const payload = {
    ...
    display_name_approved_enabled: input.displayNameApprovedEnabled ?? current.displayNameApprovedEnabled,
    team_joined_enabled: input.teamJoinedEnabled ?? current.teamJoinedEnabled,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  const { data, error } = await adminSupabase.from("user_notification_preferences").upsert(payload, { onConflict: "user_id" }).select("*").single();
  if (error) {
    throw error;
  }
  return mapNotificationPreferences(data, userId);
}
function mapDeveloperApiCredential(row) {
  const scopes = Array.isArray(row.scopes) ? row.scopes.filter((scope) => {
    return typeof scope === "string" && developerApiScopeSet.has(scope);
  }) : [];
  return {
    id: String(row.id),
...
function mapDeveloperWebhookEndpoint(row) {
  const events = Array.isArray(row.events) ? row.events.filter((event) => {
    return typeof event === "string" && developerWebhookEventSet.has(event);
  }) : [];
  return {
    id: String(row.id),
...
async function dispatchDeveloperWebhookEvent(userId, eventName, data) {
  const endpointsResult = await adminSupabase.from("developer_webhook_endpoints").select("*").eq("user_id", userId).eq("status", "active").contains("events", [eventName]);
  ...
  const endpoints = (endpointsResult.data || []).map((row) => ({
    ...
    signingSecret: normalizeOptionalString(row.signing_secret_ciphertext) ? decryptSecretValue(String(row.signing_secret_ciphertext)) : null
  ...
  for (const { endpoint, signingSecret } of endpoints) {
    ...
    try {
      ...
      await adminSupabase.from("developer_webhook_endpoints").update(deliveryPayload).eq("user_id", userId).eq("id", endpoint.id);
    } catch (error) {
      await adminSupabase.from("developer_webhook_endpoints").update({
        last_delivery_at: (/* @__PURE__ */ new Date()).toISOString(),
        last_error: error instanceof Error ? error.message : "Webhook delivery failed."
      }).eq("user_id", userId).eq("id", endpoint.id);
    }
  }
}
async function listDeveloperApiCredentials(userId) {
  const { data, error } = await adminSupabase.from("developer_api_credentials").select("*").eq("user_id", userId).order("created_at", { ascending: false });
  if (error) {
    throw error;
  }
  return (data || []).map((row) => mapDeveloperApiCredential(row));
}
async function createDeveloperApiCredential(userId, input) {
  ...
  const { data, error } = await adminSupabase.from("developer_api_credentials").insert({
    user_id: userId,
    name,
    api_key: apiKey,
  ...
  return {
    credential: mapDeveloperApiCredential(data),
    secret
  };
}
async function regenerateDeveloperApiCredentialSecret(userId, credentialId) {
  const id = normalizeOptionalIdentifier(credentialId);
  if (!id) {
    throw new Error("API credential ID is required.");
  }
  const secret = generateDeveloperToken("cnkt_secret", 32);
  const { data, error } = await adminSupabase.from("developer_api_credentials").update({
    secret_hash: hashDeveloperSecret(secret),
    secret_last4: last4(secret),
    status: "active"
...
async function deleteDeveloperApiCredential(userId, credentialId) {
  const id = normalizeOptionalIdentifier(credentialId);
  if (!id) {
    throw new Error("API credential ID is required.");
  }
  const { error } = await adminSupabase.from("developer_api_credentials").delete().eq("user_id", userId).eq("id", id);
  if (error) {
    throw error;
  }
  return { ok: true };
}
async function listDeveloperWebhooks(userId) {
  const { data, error } = await adminSupabase.from("developer_webhook_endpoints").select("*").eq("user_id", userId).order("created_at", { ascending: false });
  if (error) {
    throw error;
  }
  return (data || []).map((row) => mapDeveloperWebhookEndpoint(row));
}
async function createDeveloperWebhook(userId, input) {
  ...
  const { data, error } = await adminSupabase.from("developer_webhook_endpoints").insert({
    user_id: userId,
    name,
    url,
    events,
  ...
  return {
    webhook: mapDeveloperWebhookEndpoint(data),
    signingSecret
  };
}
async function updateDeveloperWebhook(userId, webhookId, input) {
  ...
  const { data, error } = await adminSupabase.from("developer_webhook_endpoints").update(payload).eq("user_id", userId).eq("id", id).select("*").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  return mapDeveloperWebhookEndpoint(data);
}
async function deleteDeveloperWebhook(userId, webhookId) {
  const id = normalizeOptionalIdentifier(webhookId);
  if (!id) {
    throw new Error("Webhook ID is required.");
  }
  const { error } = await adminSupabase.from("developer_webhook_endpoints").delete().eq("user_id", userId).eq("id", id);
  if (error) {
    throw error;
  }
  return { ok: true };
...
async function createUserNotification(args) {
  ...
  if (dedupeKey) {
    const existing = await adminSupabase.from("user_notifications").select("*").eq("user_id", args.userId).eq("dedupe_key", dedupeKey).maybeSingle();
    if (existing.error && !isMissingSchemaError(existing.error)) {
      throw existing.error;
    }
    if (existing.data) {
      return mapNotification(existing.data);
    }
  }
  ...
  const insertNotification = (payload) => adminSupabase.from("user_notifications").insert(payload).select("*").single();
  let { data, error } = await insertNotification(notificationPayload);
  if (error) {
    const rawErrorMessage = isRecord(error) && typeof error.message === "string" ? error.message : "";
    const rawErrorDetails = isRecord(error) && typeof error.details === "string" ? error.details : "";
    const isLegacyTypeConstraint = isRecord(error) && error.code === "23514" && (rawErrorMessage.includes("user_notifications_type_check") || rawErrorDetails.includes("user_notifications_type_check"));
...
function getDisplayNameStatus(value) {
  const normalized = normalizeOptionalString(value);
  return normalized ? normalized.toUpperCase() : null;
}
function getStoredDisplayNameApprovalStatus(metadata) {
  if (!isRecord(metadata) || !isRecord(metadata.displayNameApproval)) {
    return null;
  }
  return getDisplayNameStatus(metadata.displayNameApproval.status);
}
function getStoredDisplayNameRequest(metadata) {
  if (!isRecord(metadata) || !isRecord(metadata.displayNameRequest)) {
    return null;
  }
  const request = metadata.displayNameRequest;
...
function getDisplayNameRequestStatusFromPhoneSnapshot(displayNameRequest, phoneSnapshot) {
  ...
  if (requestedName && verifiedName2 && requestedName === verifiedName2 && currentNameStatus === "APPROVED") {
    return currentNameStatus;
  }
  return null;
}
function isDisplayNameReviewStatus(status) {
  const normalized = (status || "").toUpperCase();
  return normalized.includes("SUBMITTED") || normalized.includes("PENDING") || normalized.includes("REVIEW") || normalized.includes("DEFERRED");
}
function getBusinessProfileDisplayNameStatus(metadata, phoneSnapshot) {
  const request = getStoredDisplayNameRequest(metadata);
  const liveRequestStatus = isRecord(metadata) && isRecord(metadata.displayNameRequest) ? getDisplayNameRequestStatusFromPhoneSnapshot(
  ...
  if (requestStatus && (isDisplayNameReviewStatus(requestStatus) || requestStatus.includes("REJECT") || requestStatus.includes("DECLIN"))) {
    return requestStatus;
  }
  return getDisplayNameStatus(phoneSnapshot?.name_status) || requestStatus;
}
function isRejectedDisplayNameStatus(status) {
  const normalized = (status || "").toUpperCase();
...
async function createDisplayNameReviewNotification(args) {
  ...
  const body = rejected ? `${args.verifiedName} was rejected for ${args.displayPhoneNumber}.${args.rejectionReason ? ` Reason: ${args.rejectionReason}` : ""}` : `${args.verifiedName} is now approved for ${args.displayPhoneNumber}.`;
  await createUserNotification({
    userId: args.userId,
...
async function createIncomingMessageNotification(args) {
  ...
  const senderLabel = normalizeOptionalString(args.contactName) || normalizeOptionalString(args.contactValue) || `${args.source} contact`;
...
var LIVE_EMAIL_NOTIFICATION_WINDOW_MS = 30 * 60 * 1e3;
async function syncIncomingEmailNotifications(userId, messages) {
  const now = Date.now();
  for (const message of messages) {
    if (!message.isUnread) {
      continue;
    ...
    if (!Number.isFinite(receivedAtTimestamp) || now - receivedAtTimestamp > LIVE_EMAIL_NOTIFICATION_WINDOW_MS) {
      continue;
    }
    const senderLabel = normalizeOptionalString(message.fromName) || normalizeOptionalString(message.fromEmail) || "New sender";
    await createUserNotification({
      userId,
      type: "incoming_email",
...
async function markNotificationsRead(userId, options) {
  let query = adminSupabase.from("user_notifications").update({
    is_read: true,
...
async function getInboxInsights(userId, query) {
  ...
  const channelResult = await adminSupabase.from("meta_channels").select("*").eq("user_id", userId).maybeSingle();
  if (channelResult.error) {
    throw channelResult.error;
  }
  let rows = [];
  if (isChannelSupported) {
    const messagesResult = await adminSupabase.from("conversation_messages").select("thread_id,direction,status,recipient_wa_id,created_at").eq("user_id", userId).gte("created_at", startAtIso).lt("created_at", endExclusiveIso).order("thread_id", { ascending: true }).order("created_at", { ascending: true });
    if (messagesResult.error) {
      throw messagesResult.error;
    }
    rows = messagesResult.data || [];
  }
  ...
  const uniqueOutboundRecipients = new Set(
    outboundRows.map((row) => normalizePhoneLike(row.recipient_wa_id) || (row.thread_id ? `thread:${row.thread_id}` : null)).filter(Boolean)
  ).size;
  const channelRow = channelResult.data;
  ...
  if (channelRow?.phone_number_id) {
    try {
      ...
      if (nextMessagingLimitTier !== normalizeOptionalString(channelRow.messaging_limit_tier) || nextMessagingQuality !== normalizeOptionalString(channelRow.quality_rating)) {
        await adminSupabase.from("meta_channels").update({
...
async function seedMetaLeadCaptureFromReusableSetup(args) {
  ...
  const nextAccessTokenCiphertext = shouldUseReusableToken ? encryptAccessToken(args.accessToken) : existingAccessToken;
  const nextAccessTokenLast4 = shouldUseReusableToken ? last4(args.accessToken) : normalizeOptionalString(existing.access_token_last4);
  const nextLastError = shouldUseReusablePages || shouldUseReusableToken ? null : normalizeOptionalString(existing.last_error);
  const status = resolveMetaLeadCaptureStatus({
    pageIds: nextPageIds,
    hasAccessToken: Boolean(nextAccessTokenCiphertext),
    lastError: nextLastError
  });
  const { error } = await adminSupabase.from("meta_lead_capture_configs").upsert(
    {
      user_id: args.userId,
...
async function ensureMetaLeadCaptureConfig(userId, metaChannelId) {
  const existingResult = await adminSupabase.from("meta_lead_capture_configs").select("*").eq("user_id", userId).maybeSingle();
  if (existingResult.error) {
    throw existingResult.error;
  }
  if (existingResult.data) {
    if (!existingResult.data.meta_channel_id && metaChannelId) {
      const { data: data2, error: error2 } = await adminSupabase.from("meta_lead_capture_configs").update({
  ...
  const { data, error } = await adminSupabase.from("meta_lead_capture_configs").insert({
    user_id: userId,
    meta_channel_id: metaChannelId,
...
async function getMetaLeadCaptureSetup(userId, req) {
  const channelResult = await adminSupabase.from("meta_channels").select("id").eq("user_id", userId).maybeSingle();
  if (channelResult.error) {
    throw channelResult.error;
  }
...
async function fetchMetaLeadCapturePageSubscription(accessToken, pageId, appId) {
  try {
    const response = await metaRequest({
      accessToken,
      path: `${pageId}/subscribed_apps`,
    ...
    const apps = Array.isArray(response.data) ? response.data.filter((entry) => isRecord(entry)) : [];
    const normalizedAppId = normalizeOptionalIdentifier(appId);
    const exactApp = normalizedAppId ? apps.find((entry) => normalizeOptionalIdentifier(entry.id) === normalizedAppId) || null : null;
    const leadgenApp = apps.find((entry) => normalizeMetaSubscribedFields(entry.subscribed_fields).includes("leadgen")) || null;
    ...
    const mismatchMessage = normalizedAppId && leadgenApp && normalizeOptionalIdentifier(leadgenApp.id) !== normalizedAppId ? `Leadgen is active on this Page, but the subscribed app ID (${normalizeOptionalIdentifier(leadgenApp.id) || "unknown"}) does not match the saved Meta App ID (${normalizedAppId}).` : null;
    return {
      pageId,
...
async function getMetaLeadCapturePageSubscriptions(configRow) {
...
async function buildMetaLeadCaptureSetupResponse(userId, req, configRow) {
  const [eventsResult, pageSubscriptions] = await Promise.all([
    adminSupabase.from("meta_lead_capture_events").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(12),
  ...
  return {
    ...
    recentEvents: (eventsResult.data || []).map(
      (row) => mapMetaLeadCaptureEvent(row)
    ),
    pageSubscriptions
  };
}
async function connectMetaLeadCaptureSetup(userId, input, req) {
  ...
  if (!reusableAssets.pages.length) {
    throw new Error(
      reusableAssets.collectionError || "No Facebook Pages were returned. Confirm the user granted Pages and Leads permissions."
    );
  }
  const requestedPageIds = normalizeStringArray(input.pageIds);
  const requestedPageIdSet = new Set(requestedPageIds);
  const matchedPages = requestedPageIds.length ? reusableAssets.pages.filter((page) => requestedPageIdSet.has(page.pageId)) : reusableAssets.pages;
  if (!matchedPages.length) {
    throw new Error("The selected Facebook Page was not returned by Meta for this login.");
  }
  const primaryPage = matchedPages.find((page) => normalizeOptionalString(page.pageAccessToken)) || matchedPages[0];
  ...
  for (const page of selectedPages) {
    ...
    try {
      ...
      subscriptionFailures.push(`${page.pageId}: ${mapDbError(error2)}`);
    }
  }
  ...
  const { data, error } = await adminSupabase.from("meta_lead_capture_configs").upsert(
    {
      user_id: userId,
      ...
      verify_token: normalizeOptionalString(existing.verify_token) || generateVerifyToken(),
      default_owner_name: normalizeOptionalString(existing.default_owner_name),
      default_labels: normalizeStringArray(existing.default_labels).length ? normalizeStringArray(existing.default_labels) : ["meta lead"],
  ...
  return buildMetaLeadCaptureSetupResponse(userId, req, data);
}
async function saveMetaLeadCaptureSetup(userId, input, req) {
  const channelResult = await adminSupabase.from("meta_channels").select("id").eq("user_id", userId).maybeSingle();
  if (channelResult.error) {
    throw channelResult.error;
  }
  ...
  const nextPageIds = input.pageIds !== void 0 ? normalizeStringArray(input.pageIds) : normalizeStringArray(existing.page_ids);
  const nextFormIds = input.formIds !== void 0 ? normalizeStringArray(input.formIds) : normalizeStringArray(existing.form_ids);
  const nextDefaultLabels = input.defaultLabels !== void 0 ? normalizeStringArray(input.defaultLabels) : normalizeStringArray(existing.default_labels);
  const nextVerifyToken = input.regenerateVerifyToken || !normalizeOptionalString(existing.verify_token) ? generateVerifyToken() : String(existing.verify_token);
  const accessTokenCiphertext = input.accessToken !== void 0 ? nextAccessToken ? encryptAccessToken(nextAccessToken) : null : normalizeOptionalString(existing.access_token_ciphertext);
  const accessTokenLast4 = input.accessToken !== void 0 ? nextAccessToken ? last4(nextAccessToken) : null : normalizeOptionalString(existing.access_token_last4);
  const nextLastError = input.accessToken !== void 0 || input.pageIds !== void 0 ? null : normalizeOptionalString(existing.last_error);
  const status = resolveMetaLeadCaptureStatus({
    pageIds: nextPageIds,
  ...
  const { data, error } = await adminSupabase.from("meta_lead_capture_configs").upsert(
    {
      ...
      meta_channel_id: channelResult.data ? String(channelResult.data.id) : normalizeOptionalString(existing.meta_channel_id),
      status,
      app_id: "appId" in input ? normalizeOptionalString(input.appId) : normalizeOptionalString(existing.app_id),
      page_ids: nextPageIds,
      form_ids: nextFormIds,
      access_token_ciphertext: accessTokenCiphertext,
      access_token_last4: accessTokenLast4,
      verify_token: nextVerifyToken,
      default_owner_name: "defaultOwnerName" in input ? normalizeOptionalString(input.defaultOwnerName) : normalizeOptionalString(existing.default_owner_name),
      default_labels: nextDefaultLabels,
      auto_create_leads: input.autoCreateLeads !== void 0 ? Boolean(input.autoCreateLeads) : Boolean(existing.auto_create_leads),
      last_error: nextLastError
    },
    {
      onConflict: "user_id"
...
async function subscribeMetaLeadCapturePage(accessToken, pageId) {
...
async function activateMetaLeadCapturePageSubscriptions(userId, req) {
  const channelResult = await adminSupabase.from("meta_channels").select("id").eq("user_id", userId).maybeSingle();
  if (channelResult.error) {
    throw channelResult.error;
  }
  ...
  for (const pageId of pageIds) {
    try {
      ...
      failures.push(`${pageId}: ${mapDbError(error2)}`);
    }
  }
  const nextLastError = failures.length ? failures.join(" | ") : null;
  const { data, error } = await adminSupabase.from("meta_lead_capture_configs").update({
...
async function fetchMetaLeadCaptureLead(accessToken, leadId) {
  return metaRequest({
    accessToken,
    path: leadId,
    query: {
...
function getMetaLeadCaptureFieldMap(fieldData) {
  if (!Array.isArray(fieldData)) {
    return {};
  }
  return fieldData.reduce((accumulator, entry) => {
    if (!isRecord(entry)) {
      return accumulator;
    }
    const name = normalizeOptionalString(entry.name)?.toLowerCase();
    const values = Array.isArray(entry.values) ? entry.values.map((value) => normalizeOptionalString(value)).filter((value) => Boolean(value)) : [];
...
function getMetaLeadCaptureFieldValue(fieldMap, candidates) {
  for (const candidate of candidates) {
...
async function upsertMetaLeadCaptureEvent(args) {
  if (args.leadId) {
    const existing = await adminSupabase.from("meta_lead_capture_events").select("*").eq("user_id", args.userId).eq("lead_id", args.leadId).maybeSingle();
    if (existing.error) {
      throw existing.error;
    }
    if (existing.data) {
      const { data: data2, error: error2 } = await adminSupabase.from("meta_lead_capture_events").update({
        page_id: args.pageId,
        form_id: args.formId,
  ...
  const { data, error } = await adminSupabase.from("meta_lead_capture_events").insert({
    user_id: args.userId,
    page_id: args.pageId,
    form_id: args.formId,
    lead_id: args.leadId,
...
async function processMetaLeadCaptureChange(change, pageIdFallback) {
  ...
  const eventTime = toIsoTimestamp(
    typeof value.created_time === "string" || typeof value.created_time === "number" ? value.created_time : null
  ) || (/* @__PURE__ */ new Date()).toISOString();
  if (!pageId) {
    return;
  }
  const configResult = await adminSupabase.from("meta_lead_capture_configs").select("*").contains("page_ids", [pageId]).limit(1).maybeSingle();
  ...
  await adminSupabase.from("meta_lead_capture_configs").update({
    last_webhook_at: (/* @__PURE__ */ new Date()).toISOString(),
    last_error: null
  }).eq("user_id", userId);
  ...
  try {
    ...
    const fullName = getMetaLeadCaptureFieldValue(fieldMap, ["full_name", "full name", "name"]) || [firstName, lastName].filter(Boolean).join(" ") || "Meta lead";
    const phone = normalizePhoneLike(
    ...
    const displayPhone2 = phone || email || `Lead ${leadId.slice(-6)}`;
    const previewText = `New Meta lead${formId ? ` from form ${formId}` : ""}`;
    const leadLabels = Array.from(/* @__PURE__ */ new Set(["meta lead", ...configLabels]));
    const leadMessageId = `meta-lead:${leadId}`;
    const existingLeadMessage = await adminSupabase.from("conversation_messages").select("id").eq("user_id", userId).eq("wa_message_id", leadMessageId).maybeSingle();
    ...
    await adminSupabase.from("meta_lead_capture_configs").update({
      ...
      last_error: null,
      last_lead_synced_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("user_id", userId);
  } catch (error) {
    const message = mapDbError(error);
    await upsertMetaLeadCaptureEvent({
...
async function upsertProfile(user, input) {
  ...
  const nextProfilePictureUrl = "profilePictureUrl" in input ? normalizeOptionalString(input.profilePictureUrl) : previousProfilePictureUrl;
  const nextCompanyLogoUrl = "companyLogoUrl" in input ? normalizeOptionalString(input.companyLogoUrl) : previousCompanyLogoUrl;
  const payload = {
    user_id: user.id,
    email: user.email || existing.data?.email || null,
    full_name: "fullName" in input ? normalizeOptionalString(input.fullName) : normalizeOptionalString(existing.data?.full_name),
    profile_picture_url: nextProfilePictureUrl,
    company_logo_url: nextCompanyLogoUrl,
    country_code: "countryCode" in input ? normalizeOptionalString(input.countryCode) : normalizeOptionalString(existing.data?.country_code),
    phone: "phone" in input ? normalizeOptionalString(input.phone) : normalizeOptionalString(existing.data?.phone),
    preferred_currency: "preferredCurrency" in input ? normalizeOptionalString(input.preferredCurrency) ? normalizeCurrencyCode(input.preferredCurrency) : null : normalizeOptionalString(existing.data?.preferred_currency) ? normalizeCurrencyCode(existing.data?.preferred_currency) : null,
    company_name: "companyName" in input ? normalizeOptionalString(input.companyName) : normalizeOptionalString(existing.data?.company_name),
    company_website: "companyWebsite" in input ? normalizeOptionalString(input.companyWebsite) : normalizeOptionalString(existing.data?.company_website),
    industry: "industry" in input ? normalizeOptionalString(input.industry) : normalizeOptionalString(existing.data?.industry),
    selected_plan: "selectedPlan" in input ? input.selectedPlan ?? null : existing.data?.selected_plan ?? null,
    billing_cycle: "billingCycle" in input ? input.billingCycle ?? null : existing.data?.billing_cycle ?? null,
    billing_status: "billingStatus" in input ? input.billingStatus ?? null : existing.data?.billing_status ?? null,
    trial_ends_at: "trialEndsAt" in input ? input.trialEndsAt ?? null : existing.data?.trial_ends_at ?? null,
    free_trial_started_at: "freeTrialStartedAt" in input ? input.freeTrialStartedAt ?? null : existing.data?.free_trial_started_at ?? null,
    coupon_code: "couponCode" in input ? input.couponCode ?? null : existing.data?.coupon_code ?? null,
    razorpay_subscription_id: "razorpaySubscriptionId" in input ? input.razorpaySubscriptionId ?? null : existing.data?.razorpay_subscription_id ?? null,
    onboarding_completed: "onboardingCompleted" in input ? input.onboardingCompleted ?? false : existing.data?.onboarding_completed ?? false,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  const { data, error } = await adminSupabase.from("app_profiles").upsert(payload, {
    onConflict: "user_id"
  }).select("*").single();
  if (error) {
    throw error;
...
async function deleteAccount(user) {
  const profileResult = await adminSupabase.from("app_profiles").select("profile_picture_url,company_logo_url").eq("user_id", user.id).maybeSingle();
  if (profileResult.error && !isMissingSchemaError(profileResult.error)) {
    throw profileResult.error;
  }
  const membershipResult = await adminSupabase.from("workspace_team_members").select("id").eq("member_user_id", user.id);
  if (membershipResult.error && !isMissingSchemaError(membershipResult.error)) {
    throw membershipResult.error;
  }
  const profileRow = profileResult.data;
  const membershipIds = (membershipResult.data || []).map((row) => normalizeOptionalIdentifier(row.id)).filter((value) => Boolean(value));
  ...
  if (membershipIds.length > 0) {
    try {
      const { error: cleanupError } = await adminSupabase.from("workspace_team_members").delete().in("id", membershipIds);
...
async function ensureInvitedProfile(userId, email, fullName) {
  const payload = {
    user_id: userId,
    email,
    full_name: fullName,
...
async function getWorkspaceTeamMembers(user) {
  const [profileResult, membersResult] = await Promise.all([
    adminSupabase.from("app_profiles").select("*").eq("user_id", user.id).maybeSingle(),
    adminSupabase.from("workspace_team_members").select("*").eq("workspace_owner_user_id", user.id).order("created_at", { ascending: false })
  ]);
  ...
  const memberUserIds = memberRows.map((row) => normalizeOptionalIdentifier(row.member_user_id)).filter((value) => Boolean(value));
  const profileRowsByUserId = /* @__PURE__ */ new Map();
  if (memberUserIds.length > 0) {
    const profilesResult = await adminSupabase.from("app_profiles").select("user_id,email,full_name,profile_picture_url").in("user_id", memberUserIds);
  ...
  const ownerName = normalizeOptionalString(ownerProfile?.full_name) || normalizeOptionalString(user.user_metadata?.full_name) || normalizeOptionalString(user.user_metadata?.name) || normalizeOptionalString(ownerEmail.split("@")[0]) || "Workspace Owner";
  const ownerRecord = {
    id: `owner-${user.id}`,
    ...
    role: "Owner",
    status: "active",
    invitedAt: String(ownerProfile?.created_at || user.created_at || (/* @__PURE__ */ new Date()).toISOString()),
    acceptedAt: String(ownerProfile?.created_at || user.created_at || (/* @__PURE__ */ new Date()).toISOString()),
    isOwner: true
  };
  return [
    ownerRecord,
    ...memberRows.map(
      (row) => mapWorkspaceTeamMember(
        row,
...
async function inviteWorkspaceTeamMember(user, input) {
  ...
  const existingMembership = await adminSupabase.from("workspace_team_members").select("*").eq("workspace_owner_user_id", user.id).eq("invited_email", email).maybeSingle();
  ...
  const { data, error } = await adminSupabase.from("workspace_team_members").insert({
    workspace_owner_user_id: user.id,
  ...
  return mapWorkspaceTeamMember(data);
}
async function updateWorkspaceTeamMember(user, memberId, input) {
  ...
  const { data, error } = await adminSupabase.from("workspace_team_members").update({
    ...
    role,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("id", normalizedMemberId).eq("workspace_owner_user_id", user.id).select("*").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  let profileRow = null;
  if (memberUserId) {
    const profileResult = await adminSupabase.from("app_profiles").select("user_id,email,full_name,profile_picture_url").eq("user_id", memberUserId).maybeSingle();
    if (profileResult.error && !isMissingSchemaError(profileResult.error)) {
      throw profileResult.error;
    }
...
async function removeWorkspaceTeamMember(user, memberId) {
  const normalizedMemberId = normalizeOptionalIdentifier(memberId);
  if (!normalizedMemberId || normalizedMemberId.startsWith("owner-")) {
    throw new Error("Workspace owner cannot be removed from the team.");
  }
  const { data, error } = await adminSupabase.from("workspace_team_members").delete().eq("id", normalizedMemberId).eq("workspace_owner_user_id", user.id).select("id").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
function normalizeWorkspaceOptionType(value) {
  return value === "attribute" ? "attribute" : "label";
}
function normalizeWorkspaceOptionValueType(value) {
  const normalized = normalizeOptionalString(value)?.toLowerCase();
  return normalized === "number" || normalized === "date" || normalized === "boolean" || normalized === "select" ? normalized : "text";
}
function mapWorkspaceOptionDefinition(row) {
  return {
    id: String(row.id),
    userId: String(row.user_id),
...
async function getWorkspaceOptionDefinitions(user) {
  const ownerUserId = await resolveWorkspaceOwnerUserId(user.id);
  const { data, error } = await adminSupabase.from("workspace_option_definitions").select("*").eq("user_id", ownerUserId).order("type", { ascending: true }).order("name", { ascending: true });
  if (error) {
    throw error;
  }
  return (data || []).map(mapWorkspaceOptionDefinition);
}
async function createWorkspaceOptionDefinition(user, input) {
  const ownerUserId = await resolveWorkspaceOwnerUserId(user.id);
  ...
  const { data, error } = await adminSupabase.from("workspace_option_definitions").insert({
    user_id: ownerUserId,
    type,
    name,
  ...
  return mapWorkspaceOptionDefinition(data);
}
async function deleteWorkspaceOptionDefinition(user, optionId) {
  const ownerUserId = await resolveWorkspaceOwnerUserId(user.id);
  const normalizedOptionId = normalizeOptionalIdentifier(optionId);
  if (!normalizedOptionId) {
    throw new Error("Choose an option to delete.");
  }
  const { data, error } = await adminSupabase.from("workspace_option_definitions").delete().eq("id", normalizedOptionId).eq("user_id", ownerUserId).select("id").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function getChannelWithToken(userId) {
  const { data, error } = await adminSupabase.from("meta_channels").select("*").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
...
async function getMessengerChannelWithToken(userId) {
  const { data, error } = await adminSupabase.from("messenger_channels").select("*").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data || data.status === "disconnected") {
...
async function getInstagramChannelWithToken(userId) {
  const { data, error } = await adminSupabase.from("instagram_channels").select("*").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data || data.status === "disconnected") {
  ...
  const pageAccessTokenCiphertext = normalizeOptionalString(row.page_access_token_ciphertext);
  const userAccessToken = userAccessTokenCiphertext ? decryptAccessToken(userAccessTokenCiphertext) : pageAccessTokenCiphertext ? decryptAccessToken(pageAccessTokenCiphertext) : null;
  ...
  return {
    ...
    pageAccessToken: pageAccessToken || void 0,
    userAccessToken
  };
}
function getConnectedChannelRow(row) {
  return row?.status === "disconnected" ? null : row;
}
function getDisconnectedChannelMetadata(row, disconnectedAt) {
  const metadata = isRecord(row.metadata) ? { ...row.metadata } : {};
  return {
    ...metadata,
    disconnect: {
...
async function softDisconnectChannelRow(args) {
  const rowResult = await adminSupabase.from(args.tableName).select("*").eq("user_id", args.userId).maybeSingle();
  if (rowResult.error) {
    throw rowResult.error;
  }
  const row = rowResult.data || null;
  if (!row) {
    return { ok: true };
  }
  const disconnectedAt = (/* @__PURE__ */ new Date()).toISOString();
  const { error } = await adminSupabase.from(args.tableName).update({
    status: "disconnected",
...
async function disconnectChannelStorage(args) {
  const { error } = await adminSupabase.from(args.tableName).delete().eq("user_id", args.userId);
  if (!error || isMissingSchemaError(error)) {
    return { ok: true };
  }
  ...
  return softDisconnectChannelRow({
    ...
    metadata: args.metadataBuilder ? args.metadataBuilder(row, disconnectedAt) : getDisconnectedChannelMetadata(row, disconnectedAt)
  });
}
async function subscribeMetaAppToPageMessagingWebhook(fields) {
  if (!metaAppId || !metaAppSecret) {
...
async function disconnectMetaChannel(userId) {
  const templatesResult = await adminSupabase.from("meta_templates").delete().eq("user_id", userId);
...
async function disconnectInstagramChannel(userId) {
  return disconnectChannelStorage({
...
async function disconnectMessengerChannel(userId) {
  return disconnectChannelStorage({
...
async function resolveReusableMetaAccessToken(userId, providedAccessToken, featureName) {
...
async function resolveInstagramBusinessToken(userId, longLivedToken, accessToken) {
...
async function getEmailConnectionRow(userId) {
  const { data, error } = await adminSupabase.from("email_connections").select("*").eq("user_id", userId).maybeSingle();
  if (error && !isMissingSchemaError(error)) {
    throw error;
  }
  return data || null;
}
async function getStoredEmailConnection(userId) {
  return mapEmailConnection(await getEmailConnectionRow(userId));
}
async function getEmailConnectionWithPassword(userId) {
...
async function resolveEmailHostForIpv4(host, protocol) {
  const normalizedHost = host.trim();
  const hostFamily = net.isIP(normalizedHost);
  if (hostFamily === 4) {
    return { host: normalizedHost, servername: void 0, family: 4 };
  }
  if (hostFamily === 6) {
    return { host: normalizedHost, servername: void 0, family: 6 };
  }
  try {
    const addresses = await dns.promises.resolve4(normalizedHost);
    const address = addresses.find((entry) => net.isIP(entry) === 4);
    if (address) {
      return { host: address, servername: normalizedHost, family: 4 };
    }
  } catch {
  }
  try {
    const lookupResult = await dns.promises.lookup(normalizedHost, { family: 4 });
    return { host: lookupResult.address, servername: normalizedHost, family: 4 };
  } catch {
    return { host: normalizedHost, servername: normalizedHost, family: void 0 };
  }
}
...
function getEmailVerificationErrorMessage(error, protocol) {
...
async function verifyEmailConnectionInput(input) {
...
async function verifyEmailSmtpConnectionInput(input) {
  return verifySmtpConnection(normalizeEmailSmtpVerificationInput(input));
}
async function verifyEmailImapConnectionInput(input) {
  return verifyImapConnection(normalizeEmailImapVerificationInput(input));
}
async function saveEmailConnection(userId, input) {
  const normalizedInput = normalizeEmailConnectionInput(input);
  ...
  const { data, error } = await adminSupabase.from("email_connections").upsert(payload, { onConflict: "user_id" }).select("*").single();
  if (error) {
    throw error;
  }
  return mapEmailConnection(data);
}
async function deleteEmailConnection(userId) {
...
function mapParsedAddressList(addresses) {
  return (addresses || []).map((entry) => normalizeEmailAddress(entry.address)).filter((entry) => Boolean(entry));
}
async function fetchEmailInbox(userId, options) {
  ...
  try {
    ...
    for await (const message of client.fetch(`${startSequence}:${total}`, {
      ...
      const htmlBody = typeof parsed.html === "string" ? parsed.html : parsed.html ? String(parsed.html) : null;
      ...
      messages.push({
        ...
        receivedAt: parsed.date?.toISOString() || (message.envelope?.date instanceof Date ? message.envelope.date.toISOString() : null),
        htmlBody,
        textBody,
        previewText,
...
async function getEmailTemplateById(userId, templateId) {
  const { data, error } = await adminSupabase.from("email_templates").select("*").eq("user_id", userId).eq("id", templateId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function getEmailTemplates(userId) {
  const { data, error } = await adminSupabase.from("email_templates").select("*").eq("user_id", userId).order("updated_at", { ascending: false });
  if (error && !isMissingSchemaError(error)) {
    throw error;
  }
  return (data || []).map((row) => mapEmailTemplate(row));
}
async function saveEmailTemplate(userId, input) {
  ...
  const { data, error } = await adminSupabase.from("email_templates").insert(payload).select("*").single();
  if (error) {
    throw error;
  }
  return mapEmailTemplate(data);
}
async function deleteEmailTemplate(userId, templateId) {
  const { error } = await adminSupabase.from("email_templates").delete().eq("user_id", userId).eq("id", templateId);
  if (error) {
    throw error;
  }
  return { ok: true };
}
async function getEmailCampaigns(userId) {
  const { data, error } = await adminSupabase.from("email_campaigns").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(50);
  if (error && !isMissingSchemaError(error)) {
    throw error;
  }
  return (data || []).map((row) => mapEmailCampaign(row));
}
async function insertEmailCampaign(args) {
  ...
  const { data, error } = await adminSupabase.from("email_campaigns").insert(payload).select("*").single();
  if (error) {
    throw error;
  }
  return mapEmailCampaign(data);
}
async function sendEmailCampaign(userId, input) {
  ...
  const status = deliveredCount === normalizedInput.recipients.length ? "sent" : deliveredCount > 0 ? "partial" : "failed";
  const campaign = await insertEmailCampaign({
    ...
    status,
    sentAt: deliveredCount > 0 ? (/* @__PURE__ */ new Date()).toISOString() : null
  });
  if (campaign.status === "sent") {
    await createUserNotification({
      ...
      body: `${campaign.campaignName} reached ${campaign.recipientCount} recipient${campaign.recipientCount === 1 ? "" : "s"}.`,
...
async function getConversationalAutomationConfigRow(userId) {
  const { data, error } = await adminSupabase.from("meta_conversational_automation_configs").select("*").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  return data || null;
}
async function getConversationalAutomationConfig(userId, channelRow) {
...
async function saveConversationalAutomationConfig(args) {
  ...
  const payload = {
    ...
    last_synced_at: args.lastSyncedAt !== void 0 ? args.lastSyncedAt : normalizeOptionalString(currentRow?.last_synced_at),
    last_error: args.lastError !== void 0 ? args.lastError : normalizeOptionalString(currentRow?.last_error),
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  const { data, error } = await adminSupabase.from("meta_conversational_automation_configs").upsert(payload, { onConflict: "user_id" }).select("*").single();
  if (error) {
    throw error;
  }
  return mapConversationalAutomationConfig(data, {
    userId: args.userId,
    channelRow: args.channelRow
  });
}
async function getAutomationRules(userId) {
  const { data, error } = await adminSupabase.from("automation_rules").select("*").eq("user_id", userId).order("updated_at", { ascending: false });
  if (error) {
    throw error;
  }
  return (data || []).map((row) => mapAutomationRule(row));
}
async function saveAutomationRules(args) {
  const existingResult = await adminSupabase.from("automation_rules").select("id").eq("user_id", args.userId);
  if (existingResult.error) {
    throw existingResult.error;
  }
  const existingIds = new Set((existingResult.data || []).map((row) => String(row.id)));
  const keptIds = new Set(args.rules.map((rule) => rule.id).filter((id) => Boolean(id)));
  const removedIds = [...existingIds].filter((id) => !keptIds.has(id));
  if (removedIds.length > 0) {
    const deleteResult = await adminSupabase.from("automation_rules").delete().eq("user_id", args.userId).in("id", removedIds);
  ...
  for (const rule of args.rules) {
    ...
    if (rule.id && existingIds.has(rule.id)) {
      const updateResult = await adminSupabase.from("automation_rules").update(payload).eq("user_id", args.userId).eq("id", rule.id);
      if (updateResult.error) {
        throw updateResult.error;
      }
    } else {
      const insertResult = await adminSupabase.from("automation_rules").insert(payload);
...
async function getEnabledAutomationRules(userId) {
  const { data, error } = await adminSupabase.from("automation_rules").select("*").eq("user_id", userId).eq("is_enabled", true).order("updated_at", { ascending: false });
  if (error) {
    throw error;
  }
  return (data || []).map((row) => mapAutomationRule(row));
}
function doesAutomationRuleMatchIncomingText(rule, text) {
  ...
  const isWhatsAppMessageTrigger = rule.triggerType === "incoming_message_keyword" || rule.triggerType === "whatsapp_message_received";
...
function isMinuteWithinRange(current, start, end) {
...
function evaluateAutomationRuleCondition(args) {
  ...
  switch (condition.type) {
    case "message_contains_keywords": {
      const keywords = (condition.values || []).map((value) => normalizeOptionalString(value)?.toLowerCase()).filter((value) => Boolean(value));
      if (keywords.length === 0 || !body) {
        return false;
      }
      return keywords.some(
        (keywordValue) => doesKeywordMatchText({
          text: body,
    ...
    case "contact_attribute": {
      ...
      const contactRecord = args.contact;
      const attributes = isRecord(contactRecord.attributes) ? contactRecord.attributes : {};
      const actualValue = normalizeOptionalString(contactRecord[field] ?? attributes[field]);
      if (field === "tags" || field === "labels") {
        const labels = Array.isArray(contactRecord.labels) ? contactRecord.labels.map((label) => normalizeOptionalString(label)?.toLowerCase()).filter(Boolean) : [];
...
function doesAutomationRuleFilterGroupMatch(args) {
  ...
  const results = filters.conditions.map(
    (condition) => evaluateAutomationRuleCondition({
      condition,
      body: args.body,
...
function doesAutomationRuleHaveKeywordCondition(rule) {
...
function doesAutomationRuleKeywordConditionMatch(rule, body) {
  const filters = rule.filters || rule.action.filters;
  if (!filters) {
    return doesAutomationRuleMatchIncomingText(rule, body);
  }
  return filters.conditions.some((condition) => {
    return condition.type === "message_contains_keywords" ? evaluateAutomationRuleCondition({
      condition,
      body,
...
async function markAutomationRuleTriggered(userId, rule, triggeredAt) {
  const { error } = await adminSupabase.from("automation_rules").update({
    last_triggered_at: triggeredAt,
    trigger_count: rule.triggerCount + 1,
    last_error: null,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("user_id", userId).eq("id", rule.id);
  if (error) {
    throw error;
  }
}
async function markAutomationRuleError(userId, ruleId, error) {
  const updateErrorMessage = mapDbError(error).slice(0, 1e3);
  const { error: updateError } = await adminSupabase.from("automation_rules").update({
    last_error: updateErrorMessage,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("user_id", userId).eq("id", ruleId);
  if (updateError) {
...
async function setContactMarketingOptOutFromAutomation(args) {
  const identity = normalizeOptionalString(args.recipientWaId) || normalizeOptionalString(args.thread.contactWaId) || normalizeOptionalString(args.thread.displayPhone);
  const rowsById = /* @__PURE__ */ new Map();
  if (args.thread.id) {
    rowsById.set(args.thread.id, args.thread);
  }
  if (identity) {
  ...
  const { data, error } = await adminSupabase.from("conversation_threads").update({
    marketing_opted_out: true,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("user_id", args.userId).in("id", threadIds).select("*");
  if (error) {
    throw error;
  }
  const rows = data || [];
  const activeRow = rows.find((row) => String(row.id) === args.thread.id) || rows[0] || null;
  if (!activeRow) {
...
async function processAutomationRulesForLeadCreated(args) {
  ...
  for (const rule of rules) {
    ...
    const filters = rule.filters || rule.action.filters;
    const matches = !filters || filters.conditions.length === 0 ? true : doesAutomationRuleFilterGroupMatch({
      rule,
    ...
    try {
      if (rule.action.type === "opt_out_marketing") {
        ...
        await markAutomationRuleTriggered(args.userId, rule, (/* @__PURE__ */ new Date()).toISOString());
        continue;
      }
...
async function sendAutomationRuleResponse(args) {
  ...
  if (action.type === "opt_out_marketing") {
    const nextThread2 = await setContactMarketingOptOutFromAutomation({
      userId: args.userId,
      thread: args.thread,
  ...
  if (action.type === "send_template") {
    ...
    messagePayload = await buildOutgoingTemplatePayloadWithStoredDefaults(args.userId, {
      ...
      templateName: action.templateName || "",
      language: action.templateLanguage || "",
      replyToMessageId: args.triggerMessageId || void 0
    });
...
async function processAutomationRulesForIncomingMessage(args) {
  ...
  for (const rule of rules) {
    if (rule.triggerType !== "incoming_message_keyword" && rule.triggerType !== "whatsapp_message_received") {
      continue;
    }
    if (!doesAutomationRuleFilterGroupMatch({
      rule,
      body: args.body,
      receivedAt: args.receivedAt ? new Date(args.receivedAt) : /* @__PURE__ */ new Date(),
...
async function refreshChannelSnapshot(userId, row, accessToken) {
  const [phone, waba] = await Promise.all([
  ...
  const { data, error } = await adminSupabase.from("meta_channels").update({
    ...
    last_synced_at: syncedAt,
    updated_at: syncedAt
  }).eq("user_id", userId).eq("id", row.id).select("*").single();
  if (error) {
    throw error;
  }
  return {
...
async function saveMetaChannel(args) {
  const existingChannelResult = await adminSupabase.from("meta_channels").select("phone_number_id,metadata").eq("user_id", args.userId).maybeSingle();
  if (existingChannelResult.error) {
    throw existingChannelResult.error;
  }
  const existingChannel = existingChannelResult.data || null;
  const existingMetadata = existingChannel && isRecord(existingChannel.metadata) ? existingChannel.metadata : {};
  const shouldPreserveMetadata = normalizeOptionalIdentifier(existingChannel?.phone_number_id) === args.phoneNumberId;
  const nextMetadata = {
  ...
  const { data, error } = await adminSupabase.from("meta_channels").upsert(payload, {
    onConflict: "user_id"
  }).select("*").single();
  if (error) {
    throw error;
...
async function saveInstagramChannel(args) {
  ...
  let appWebhookLastError = null;
  const webhookFields = args.pageAccessToken && args.pageId ? [...DEFAULT_INSTAGRAM_WEBHOOK_FIELDS] : [...DEFAULT_INSTAGRAM_LOGIN_WEBHOOK_FIELDS];
  try {
    ...
    webhookLastError = mapDbError(error2);
  }
  const payload = {
    user_id: args.userId,
  ...
  const { data, error } = await adminSupabase.from("instagram_channels").upsert(payload, {
    onConflict: "user_id"
  }).select("*").single();
  if (error) {
    throw error;
  }
  return mapInstagramChannel(data);
}
async function updateInstagramWebhookSubscription(args) {
  ...
  const { data, error } = await adminSupabase.from("instagram_channels").update({
    status: args.lastError ? "error" : "connected",
    metadata,
    last_synced_at: (/* @__PURE__ */ new Date()).toISOString(),
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("user_id", args.userId).eq("id", String(args.row.id)).select("*").single();
  if (error) {
    throw error;
  }
  return mapInstagramChannel(data);
}
async function activateInstagramWebhookSubscription(userId) {
  ...
  const webhookFields = pageId && pageAccessToken ? [...DEFAULT_INSTAGRAM_WEBHOOK_FIELDS] : [...DEFAULT_INSTAGRAM_LOGIN_WEBHOOK_FIELDS];
...
async function saveMessengerChannel(args) {
  ...
  const { data, error } = await adminSupabase.from("messenger_channels").upsert(payload, {
    onConflict: "user_id"
  }).select("*").single();
  if (error) {
    throw error;
  }
  return mapMessengerChannel(data);
}
async function listInstagramConnectableAccounts(longLivedToken) {
...
async function listMessengerConnectablePages(userAccessToken) {
  const pages = await fetchMessengerPages(userAccessToken);
  return pages.filter(
    (page) => normalizeOptionalIdentifier(page.id) && normalizeOptionalString(page.access_token)
  ).map((page) => {
    const pageId = normalizeOptionalIdentifier(page.id) || "";
    return {
      pageId,
...
async function syncTemplates(userId) {
  ...
  const existingTemplatesResult = await adminSupabase.from("meta_templates").select("id, template_name, language, status, raw").eq("user_id", userId);
  if (existingTemplatesResult.error) {
    throw existingTemplatesResult.error;
  }
  const existingStatusByTemplate = /* @__PURE__ */ new Map();
  const existingHeaderMediaPreviewByTemplate = /* @__PURE__ */ new Map();
  const remoteTemplateKeys = /* @__PURE__ */ new Set();
  ...
  for (const template of remoteTemplates) {
    ...
    const { error: error2 } = await adminSupabase.from("meta_templates").upsert(payload, {
    ...
    if (nextStatus && nextStatus !== previousStatus) {
      void dispatchDeveloperWebhookEvent(userId, "template.status_updated", {
        ...
        console.error("Failed to dispatch template.status_updated webhook:", error3);
      });
  ...
  const staleTemplateIds = (existingTemplatesResult.data || []).filter((templateRow) => {
    const templateName = normalizeOptionalString(templateRow.template_name);
    const language = normalizeOptionalString(templateRow.language);
    return templateName && language && !remoteTemplateKeys.has(`${templateName}:${language}`);
  }).map((templateRow) => normalizeOptionalIdentifier(templateRow.id)).filter((templateId) => Boolean(templateId));
  if (staleTemplateIds.length > 0) {
    const { error: error2 } = await adminSupabase.from("meta_templates").delete().eq("user_id", userId).in("id", staleTemplateIds);
    if (error2) {
      throw error2;
    }
  }
  ...
  const { data, error } = await adminSupabase.from("meta_templates").select("*").eq("user_id", userId).order("updated_at", { ascending: false });
  if (error) {
    throw error;
  }
  return (data || []).map((row2) => mapTemplate(row2));
}
function getTemplateComponents(raw) {
  const components = raw?.components;
  return Array.isArray(components) ? components.filter((component) => Boolean(component) && typeof component === "object" && !Array.isArray(component)) : [];
}
function normalizeTemplateSnapshot(raw, fallback) {
...
function getTemplatePreviewText(snapshot, fallbackName) {
...
async function getStoredTemplateSnapshot(userId, templateName, language) {
...
async function getStoredTemplateRecord(userId, templateName, language) {
  const { data, error } = await adminSupabase.from("meta_templates").select("template_name, language, category, status, raw").eq("user_id", userId).eq("template_name", templateName).eq("language", language).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  return {
    ...
    raw: isRecord(data.raw) ? data.raw : {}
  };
}
function getTemplateMediaHeaderType(raw) {
...
function getTemplateFlowButton(raw) {
...
function getTemplateFlowButtonIndex(raw) {
  return getTemplateFlowButton(raw)?.index || null;
}
function templateMessageComponentHasParameters(component) {
  return Array.isArray(component?.parameters) && component.parameters.filter(isRecord).length > 0;
}
function getTemplateMessageComponent(components, type, subType) {
...
function removeTemplateMessageComponents(components, type, subType) {
  const normalizedType = type.toLowerCase();
  const normalizedSubType = subType?.toLowerCase();
...
function buildTemplateMediaHeaderComponent(headerType, preview) {
  const mediaObject = {
    link: preview.url
  };
...
function getTemplateComponentByType(raw, type) {
...
function getSortedTemplateVariableIndexes(text) {
  const matches = typeof text === "string" ? text.match(/\{\{\s*(\d+)\s*\}\}/g) || [] : [];
  return Array.from(
    new Set(
      matches.map((match) => Number(match.replace(/\D/g, ""))).filter((index) => Number.isInteger(index) && index > 0)
    )
  ).sort((left, right) => left - right);
}
function normalizeTemplateExampleString(value) {
  if (typeof value === "number" && Number.isFinite(value)) {
    return String(value);
  }
  return normalizeOptionalString(value);
}
function getTemplateHeaderTextExamples(component) {
  ...
  return headerText.map((value) => normalizeTemplateExampleString(value)).filter((value) => Boolean(value));
}
function getTemplateBodyTextExamples(component) {
  const example = isRecord(component.example) ? component.example : null;
  ...
  return firstExampleSet.map((value) => normalizeTemplateExampleString(value)).filter((value) => Boolean(value));
}
function buildTextParameterComponent(type, variableIndexes, examples) {
...
async function addStoredTemplateTextParameterComponents(userId, input) {
  const components = Array.isArray(input.components) ? input.components.filter(isRecord) : [];
  ...
  const headerText = normalizeOptionalString(headerComponent?.format)?.toUpperCase() === "TEXT" ? normalizeOptionalString(headerComponent?.text) : null;
...
async function addStoredTemplateHeaderMediaComponent(userId, input) {
  const components = Array.isArray(input.components) ? input.components.filter(isRecord) : [];
  const existingHeaderComponent = getTemplateMessageComponent(components, "header");
  if (templateMessageComponentHasParameters(existingHeaderComponent)) {
    return components;
...
function buildTemplateFlowButtonComponent(index, input, defaultFlowToken) {
  const action = {
...
async function getDefaultTemplateFlowToken(userId, templateRecord) {
  if (!templateRecord) {
    return "unused";
  }
...
async function addStoredTemplateFlowButtonComponent(userId, input) {
  const components = Array.isArray(input.components) ? input.components.filter(isRecord) : [];
...
async function buildOutgoingTemplatePayloadWithStoredDefaults(userId, input) {
...
function getConversationThreadActivityTimestamp(row) {
  const timestamp = normalizeOptionalString(row.last_message_at) || normalizeOptionalString(row.updated_at) || normalizeOptionalString(row.created_at);
  if (!timestamp) {
    return 0;
  }
...
function pickConversationThreadRow(rows, canonicalPhone) {
  return [...rows].sort((left, right) => {
    ...
    const timeDiff = getConversationThreadActivityTimestamp(right) - getConversationThreadActivityTimestamp(left);
    if (timeDiff !== 0) {
      return timeDiff;
    }
...
function pickConversationThreadString(rows, key, primaryRow) {
...
async function findConversationThreadRowsByIdentity(userId, contactIdentity) {
  const variants = buildContactIdentityVariants(contactIdentity);
  if (variants.length === 0) {
    return [];
  }
  const { data, error } = await adminSupabase.from("conversation_threads").select("*").eq("user_id", userId).in("contact_wa_id", variants);
  if (error) {
    throw error;
  }
  return data || [];
...
async function findMarketingOptedOutContact(args) {
  const rows = [];
  if (args.threadId) {
    const { data, error } = await adminSupabase.from("conversation_threads").select("*").eq("user_id", args.userId).eq("id", args.threadId).maybeSingle();
    if (error) {
      throw error;
    }
    if (data) {
...
async function assertMarketingTemplateSendAllowed(args) {
  ...
  if (optedOutContact) {
    const contactLabel = normalizeOptionalString(optedOutContact.contact_name) || normalizeOptionalString(optedOutContact.display_phone) || args.to;
...
async function consolidateConversationThreadRows(args) {
  ...
  const latestMessageRow = [...uniqueRows].filter((row) => normalizeOptionalString(row.last_message_at)).sort((left, right) => {
  ...
  const mergedLabels = normalizeLabels(
    uniqueRows.flatMap((row) => Array.isArray(row.labels) ? row.labels : [])
  );
  const mergedAttributes = uniqueRows.reduce((attributes, row) => {
  ...
  const payload = {
    ...
    status: normalizeStatus(
      pickConversationThreadString(uniqueRows, "status", primaryRow) || primaryRow.status
    ),
    priority: normalizePriority(
      pickConversationThreadString(uniqueRows, "priority", primaryRow) || primaryRow.priority
    ),
    labels: mergedLabels,
  ...
  const { error: updateError } = await adminSupabase.from("conversation_threads").update(payload).eq("user_id", args.userId).eq("id", String(primaryRow.id));
  if (updateError) {
    throw updateError;
  }
  if (duplicateIds.length > 0) {
    const { error: reassignMessagesError } = await adminSupabase.from("conversation_messages").update({ thread_id: String(primaryRow.id) }).eq("user_id", args.userId).in("thread_id", duplicateIds);
    if (reassignMessagesError) {
      throw reassignMessagesError;
    }
    const { error: deleteThreadsError } = await adminSupabase.from("conversation_threads").delete().eq("user_id", args.userId).in("id", duplicateIds);
    if (deleteThreadsError) {
      throw deleteThreadsError;
    }
  }
  const { data, error } = await adminSupabase.from("conversation_threads").select("*").eq("user_id", args.userId).eq("id", String(primaryRow.id)).single();
  if (error) {
    throw error;
  }
  return data;
}
async function ensureConversationThreadPhoneConsistency(userId, rows) {
  const rowsByPhone = /* @__PURE__ */ new Map();
  for (const row of rows) {
  ...
  for (const [canonicalPhone, groupedRows] of rowsByPhone.entries()) {
    ...
    const needsConsolidation = groupedRows.length > 1 || groupedRows.some((row) => {
...
async function upsertThread(args) {
  ...
  if (canonicalPhone) {
    ...
    existingRow = matchingRows.length > 0 ? await consolidateConversationThreadRows({
      userId: args.userId,
      rows: matchingRows,
      canonicalPhone
    }) : null;
  } else {
    const existing = await adminSupabase.from("conversation_threads").select("*").eq("user_id", args.userId).eq("contact_wa_id", contactWaId).maybeSingle();
    if (existing.error) {
      throw existing.error;
    }
    existingRow = existing.data || null;
  }
  const currentUnread = existingRow?.unread_count ? Number(existingRow.unread_count) : 0;
  const explicitDisplayPhone = args.displayPhone !== void 0 ? formatContactIdentity(args.displayPhone) ?? normalizeOptionalString(args.displayPhone) : void 0;
  const payload = {
    ...
    display_phone: explicitDisplayPhone ?? (canonicalPhone ? `+${canonicalPhone}` : normalizeOptionalString(existingRow?.display_phone)) ?? null,
    ...
    status: args.status !== void 0 ? normalizeStatus(args.status) : normalizeStatus(existingRow?.status ?? null),
    priority: args.priority ?? normalizePriority(existingRow?.priority ?? null),
    labels: args.labels !== void 0 ? normalizeLabels(args.labels) : normalizeLabels(existingRow?.labels),
  ...
  const result = existingRow ? await query.update(payload).eq("user_id", args.userId).eq("id", String(existingRow.id)).select("*").single() : await query.insert({
    user_id: args.userId,
    ...payload
  }).select("*").single();
  const { data, error } = result;
  ...
  if (!existingRow) {
    void dispatchDeveloperWebhookEvent(args.userId, "conversation.created", {
      ...
      console.error("Failed to dispatch conversation.created webhook:", error2);
    });
  }
  return thread;
}
async function createContact(userId, input) {
  ...
  const existing = matchingRows.length > 0 ? await consolidateConversationThreadRows({
    userId,
    rows: matchingRows,
  ...
  const payload = {
    ...
    labels: input.labels !== void 0 ? normalizeLabels(input.labels) : existing?.labels ?? [],
    marketing_opted_out: typeof input.marketingOptedOut === "boolean" ? input.marketingOptedOut : Boolean(existing?.marketing_opted_out),
  ...
  const query = adminSupabase.from("conversation_threads");
  const result = existing ? await query.update(payload).eq("user_id", userId).eq("id", String(existing.id)).select("*").single() : await query.insert({
    user_id: userId,
    ...payload
  }).select("*").single();
  const { data, error } = result;
  ...
  if (!existing) {
    ...
    try {
      ...
      console.error("Lead-created automation rule processing failed:", error2);
    }
    void dispatchDeveloperWebhookEvent(userId, "contact.created", {
      contact
    }).catch((error2) => {
      console.error("Failed to dispatch contact.created webhook:", error2);
    });
  }
  return contact;
}
async function updateContact(userId, threadId, input) {
  const existingResult = await adminSupabase.from("conversation_threads").select("*").eq("user_id", userId).eq("id", threadId).maybeSingle();
  if (existingResult.error) {
    throw existingResult.error;
  }
  if (!existingResult.data) {
  ...
  const nextContactWaId = input.displayPhone !== void 0 ? normalizeContactIdentity(input.displayPhone) || normalizeContactIdentity(existing.contact_wa_id) : normalizeContactIdentity(existing.contact_wa_id);
  if (!nextContactWaId) {
  ...
  if (canonicalPhone) {
    ...
    targetRow = await consolidateConversationThreadRows({
      userId,
      rows: rowsToMerge,
      canonicalPhone
    }) || existing;
  ...
  const payload = {
    ...
    contact_name: input.contactName !== void 0 ? normalizeOptionalString(input.contactName) : targetRow.contact_name ?? null,
    display_phone: input.displayPhone !== void 0 ? formatContactIdentity(input.displayPhone) ?? (canonicalPhone ? `+${canonicalPhone}` : formatContactIdentity(targetRow.display_phone)) ?? normalizeOptionalString(targetRow.display_phone) ?? null : formatContactIdentity(targetRow.display_phone) ?? (canonicalPhone ? `+${canonicalPhone}` : normalizeOptionalString(targetRow.display_phone)) ?? null,
    email: input.email !== void 0 ? normalizeOptionalString(input.email) : targetRow.email ?? null,
    source: input.source !== void 0 ? normalizeOptionalString(input.source) : targetRow.source ?? null,
    remark: input.remark !== void 0 ? normalizeOptionalString(input.remark) : targetRow.remark ?? null,
    avatar_url: input.avatarUrl !== void 0 ? normalizeOptionalString(input.avatarUrl) : targetRow.avatar_url ?? null,
    status: input.status !== void 0 ? normalizeStatus(input.status) : normalizeStatus(targetRow.status),
    priority: input.priority !== void 0 ? normalizePriority(input.priority) : normalizePriority(targetRow.priority),
    labels: input.labels !== void 0 ? normalizeLabels(input.labels) : normalizeLabels(targetRow.labels),
    marketing_opted_out: typeof input.marketingOptedOut === "boolean" ? input.marketingOptedOut : Boolean(targetRow.marketing_opted_out),
    owner_name: input.ownerName !== void 0 ? normalizeOptionalString(input.ownerName) : targetRow.owner_name ?? null,
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  const { data, error } = await adminSupabase.from("conversation_threads").update(payload).eq("user_id", userId).eq("id", String(targetRow.id)).select("*").single();
  if (error) {
    throw error;
  }
...
async function insertMessage(args) {
  ...
  if (args.waMessageId) {
    const existing = await adminSupabase.from("conversation_messages").select("*").eq("user_id", args.userId).eq("wa_message_id", args.waMessageId).maybeSingle();
    if (existing.error) {
      throw existing.error;
    }
    if (existing.data) {
  ...
  const { data, error } = await adminSupabase.from("conversation_messages").insert(payload).select("*").single();
  if (error) {
    throw error;
  }
  ...
  if (message.direction === "inbound") {
    void dispatchDeveloperWebhookEvent(args.userId, "message.received", {
      ...
      console.error("Failed to dispatch message.received webhook:", error2);
    });
  }
  return message;
}
function mergeWhatsAppStatusPayloadIntoRaw(raw, statusRecord) {
  ...
  const previousHistory = Array.isArray(currentRaw.whatsapp_status_history) ? currentRaw.whatsapp_status_history.filter(isRecord) : [];
...
async function updateOutgoingWhatsAppMessageStatus(args) {
  ...
  const result = await adminSupabase.from("conversation_messages").select("*").eq("user_id", args.userId).eq("wa_message_id", messageId);
  if (result.error) {
    throw result.error;
  }
  const nextStatus = normalizeOptionalString(args.statusRecord.status) || null;
  const errors = Array.isArray(args.statusRecord.errors) ? args.statusRecord.errors.filter(isRecord) : [];
  const firstError = errors[0] || null;
  const firstErrorData = isRecord(firstError?.error_data) ? firstError.error_data : null;
  console.info("WhatsApp message status webhook:", {
    messageId,
    status: nextStatus,
    errorCode: firstError?.code || null,
    errorTitle: normalizeOptionalString(firstError?.title) || null,
    errorMessage: normalizeOptionalString(firstErrorData?.details) || normalizeOptionalString(firstError?.message) || null,
  ...
  for (const row of result.data || []) {
    ...
    const { data, error } = await adminSupabase.from("conversation_messages").update({
      status: nextStatus,
    ...
    if (nextStatus && nextStatus !== previousStatus) {
      const eventName = nextStatus === "read" ? "message.read" : nextStatus === "delivered" ? "message.delivered" : nextStatus === "failed" ? "message.failed" : null;
      if (eventName) {
        const message = mapMessage(data);
        void dispatchDeveloperWebhookEvent(args.userId, eventName, {
          ...
          console.error(`Failed to dispatch ${eventName} webhook:`, error2);
        });
      }
    }
  }
}
function getOutgoingInteractivePreviewText(payload) {
  if (payload.type !== "interactive") {
    return null;
  }
  const bodyText = normalizeOptionalString(payload.interactive.body?.text);
  const headerText = payload.interactive.header?.type === "text" ? normalizeOptionalString(payload.interactive.header.text) : null;
...
function getOutgoingContactsPreviewText(payload) {
  ...
  return formattedName ? `Contact card: ${formattedName}` : phone ? `Contact card: ${phone}` : email ? `Contact card: ${email}` : "Contact card";
}
async function describeOutgoingWhatsAppMessage(userId, payload) {
  ...
  switch (payload.type) {
    ...
    case "location": {
      const locationLabel = normalizeOptionalString(payload.location.name) || normalizeOptionalString(payload.location.address) || `${payload.location.latitude}, ${payload.location.longitude}`;
      return {
...
function getConversationThreadMessagingChannel(thread) {
...
function getThreadRecipientIdFromIdentity(thread, prefix) {
  const expectedPrefix = `${prefix}:`;
  ...
  if (displayIdentity && !displayIdentity.startsWith("@")) {
    return displayIdentity.startsWith(expectedPrefix) ? displayIdentity.slice(expectedPrefix.length) : displayIdentity;
  }
  return null;
}
async function sendThreadOutgoingWhatsAppMessage(args) {
  const payload = args.payload.type === "template" ? {
  ...
  const remote = payload.type === "template" ? await sendRemoteWhatsAppTemplateMessageForStoredCategory({
    userId: args.user.id,
    accessToken: args.accessToken,
    phoneNumberId: args.phoneNumberId,
    payload
...
async function sendThreadOutgoingMessengerTextMessage(args) {
  ...
  const createdAt = (/* @__PURE__ */ new Date()).toISOString();
  const senderLabel = normalizeOptionalString(args.channelRow.page_name) || normalizeOptionalString(args.user.user_metadata?.full_name);
  const nextThread = await upsertThread({
    userId: args.user.id,
    metaChannelId: null,
    contactWaId: args.thread.contactWaId,
...
async function sendThreadOutgoingInstagramTextMessage(args) {
  ...
  const createdAt = (/* @__PURE__ */ new Date()).toISOString();
  const senderLabel = normalizeOptionalString(args.channelRow.instagram_username) || normalizeOptionalString(args.channelRow.instagram_name) || normalizeOptionalString(args.user.user_metadata?.full_name);
  const nextThread = await upsertThread({
    userId: args.user.id,
    metaChannelId: null,
    contactWaId: args.thread.contactWaId,
...
function buildOutgoingTextPayload(input) {
  return {
    to: input.to,
    type: "text",
...
function buildCallPermissionRequestPayload(input) {
...
function buildOutgoingMediaPayload(input) {
...
function buildOutgoingTemplatePayload(input) {
  return {
    to: input.to,
    type: "template",
    context: input.replyToMessageId ? {
...
function getRemoteWhatsAppInitialMessageStatus(remote) {
...
async function buildOutgoingFlowPayload(input) {
  ...
  const flowMode = input.action.flowMode === "draft" || flow.status === "DRAFT" ? "draft" : "published";
...
async function getThreadMessages(userId, threadId, options) {
  const markRead = options?.markRead ?? true;
  const [threadResult, messagesResult] = await Promise.all([
    adminSupabase.from("conversation_threads").select("*").eq("user_id", userId).eq("id", threadId).maybeSingle(),
    adminSupabase.from("conversation_messages").select("*").eq("user_id", userId).eq("thread_id", threadId).order("created_at", { ascending: true })
  ]);
  if (threadResult.error) throw threadResult.error;
  if (messagesResult.error) throw messagesResult.error;
  if (!threadResult.data) throw new Error("Conversation not found.");
  let threadRow = threadResult.data;
  let messageRows = messagesResult.data || [];
  if (markRead && Number(threadResult.data.unread_count || 0) > 0) {
    const updatedAt = (/* @__PURE__ */ new Date()).toISOString();
    const { error: threadUpdateError } = await adminSupabase.from("conversation_threads").update({
      unread_count: 0,
      updated_at: updatedAt
    }).eq("user_id", userId).eq("id", threadId);
    ...
    const latestUnreadInboundMessage = [...messageRows].reverse().find((row) => {
      return row.direction === "inbound" && isWhatsAppCloudMessageId(row.wa_message_id) && normalizeOptionalString(row.status) !== "read";
    });
    if (latestUnreadInboundMessage) {
      try {
        ...
        if (phoneNumberId && messageId) {
          ...
          const readMessageIds = messageRows.filter((row) => {
            if (row.direction !== "inbound" || !isWhatsAppCloudMessageId(row.wa_message_id) || normalizeOptionalString(row.status) === "read") {
              return false;
            }
          ...
          if (readMessageIds.length > 0) {
            const { error: messagesUpdateError } = await adminSupabase.from("conversation_messages").update({
              status: "read"
            }).eq("user_id", userId).in("id", readMessageIds);
            if (messagesUpdateError) {
              throw messagesUpdateError;
            }
            const readMessageIdsSet = new Set(readMessageIds);
            messageRows = messageRows.map(
              (row) => readMessageIdsSet.has(String(row.id)) ? {
                ...row,
                status: "read"
              } : row
            );
...
async function getThreadById(userId, threadId) {
  const { data, error } = await adminSupabase.from("conversation_threads").select("*").eq("user_id", userId).eq("id", threadId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function ensureConversationReplyWindowOpen(userId, thread) {
  const { data, error } = await adminSupabase.from("conversation_messages").select("created_at").eq("user_id", userId).eq("thread_id", thread.id).eq("direction", "inbound").neq("message_type", "call_summary").order("created_at", { ascending: false }).limit(1);
  if (error) {
    throw error;
  }
  ...
  const channelLabel = channel === "instagram" ? "Instagram" : channel === "messenger" ? "Messenger" : "WhatsApp";
...
async function deleteContact(userId, threadId) {
  const { data, error } = await adminSupabase.from("conversation_threads").delete().eq("user_id", userId).eq("id", threadId).select("id").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function findMetaChannelForPhoneNumberNameUpdate(args) {
  ...
  const rows = data || [];
  const matchedRow = normalizedDisplayPhoneNumber ? rows.find((row) => normalizePhoneLike(row.display_phone_number) === normalizedDisplayPhoneNumber) : null;
...
async function handlePhoneNumberNameUpdateWebhook(args) {
  ...
  try {
    ...
    console.error("Failed to refresh phone number after display name webhook:", error2);
  }
  const currentMetadata = getMetaChannelMetadataRecord(channel);
  const currentDisplayNameApproval = isRecord(currentMetadata.displayNameApproval) ? currentMetadata.displayNameApproval : {};
  const currentDisplayNameRequest = isRecord(currentMetadata.displayNameRequest) ? currentMetadata.displayNameRequest : {};
  const requestedName = requestedVerifiedName || normalizeOptionalString(currentDisplayNameRequest.requestedName) || normalizeOptionalString(phoneSnapshot?.verified_name);
  const nextDisplayNameApproval = {
    ...currentDisplayNameApproval,
    status: decision,
  ...
  if (decision === "APPROVED") {
    nextDisplayNameApproval.approvedAt = normalizeOptionalString(currentDisplayNameApproval.approvedAt) || receivedAt;
  }
  if (decision.includes("REJECT") || decision.includes("DECLIN")) {
    nextDisplayNameApproval.rejectedAt = receivedAt;
  }
  const nextDisplayNameRequest = requestedName || Object.keys(currentDisplayNameRequest).length > 0 ? {
    ...currentDisplayNameRequest,
    ...
    status: decision,
    approvedAt: decision === "APPROVED" ? normalizeOptionalString(currentDisplayNameRequest.approvedAt) || receivedAt : normalizeOptionalString(currentDisplayNameRequest.approvedAt),
    rejectedAt: decision.includes("REJECT") || decision.includes("DECLIN") ? receivedAt : normalizeOptionalString(currentDisplayNameRequest.rejectedAt),
    rejectionReason: decision.includes("REJECT") || decision.includes("DECLIN") ? rejectionReason : null,
    lastCheckedAt: receivedAt,
    lastError: decision.includes("REJECT") || decision.includes("DECLIN") ? rejectionReason : null
  } : null;
  const { data: updatedChannel, error } = await adminSupabase.from("meta_channels").update({
    display_phone_number: phoneSnapshot?.display_phone_number || displayPhoneNumber || normalizeOptionalString(channel.display_phone_number),
    verified_name: phoneSnapshot?.verified_name || normalizeOptionalString(channel.verified_name),
    quality_rating: phoneSnapshot?.quality_rating || normalizeOptionalString(channel.quality_rating),
    messaging_limit_tier: getNormalizedMessagingLimitTier(phoneSnapshot || void 0) || normalizeOptionalString(channel.messaging_limit_tier),
    metadata: {
      ...currentMetadata,
  ...
  }).eq("user_id", userId).eq("id", channel.id).select("*").single();
  if (error) {
    throw error;
  }
  const nextChannel = updatedChannel;
  if (decision === "APPROVED" || isRejectedDisplayNameStatus(decision)) {
    const verifiedName2 = requestedName || normalizeOptionalString(phoneSnapshot?.verified_name) || normalizeOptionalString(nextChannel.verified_name) || "Your WhatsApp display name";
    const displayPhone2 = displayPhoneNumber || normalizeOptionalString(phoneSnapshot?.display_phone_number) || normalizeOptionalString(nextChannel.display_phone_number) || "your connected sender";
    await createDisplayNameReviewNotification({
      userId,
      phoneNumberId: String(nextChannel.phone_number_id),
      verifiedName: verifiedName2,
      displayPhoneNumber: displayPhone2,
  ...
  try {
    ...
    for (const entry of payload.entry || []) {
      for (const change of entry.changes || []) {
        ...
        const { data: channel, error: channelError } = await adminSupabase.from("meta_channels").select("*").eq("phone_number_id", phoneNumberId).maybeSingle();
        ...
        const firstContact = contacts[0] && isRecord(contacts[0]) ? contacts[0] : null;
        const contactProfile = firstContact && isRecord(firstContact.profile) ? firstContact.profile : null;
        ...
        for (const message of messages) {
          ...
          const createdAt = toIsoTimestamp(message.timestamp) || (/* @__PURE__ */ new Date()).toISOString();
          const messageRecord = message;
          const messageId = normalizeOptionalString(message.id) || null;
          const body = getMessageText(messageRecord);
          if (messageId) {
            const existingMessage = await adminSupabase.from("conversation_messages").select("id").eq("user_id", userId).eq("wa_message_id", messageId).maybeSingle();
          ...
          let thread = await upsertThread({
            ...
            contactName: contactProfile?.name || null,
          ...
          const isNewConversation = Math.abs(Date.parse(thread.createdAt) - Date.parse(createdAt)) < 2e3;
          await insertMessage({
            userId,
            threadId: thread.id,
            waMessageId: messageId,
            ...
            body,
            senderName: contactProfile?.name || null,
          ...
          if (flowReply) {
            try {
              ...
              const refreshedThread = await adminSupabase.from("conversation_threads").select("*").eq("user_id", userId).eq("id", thread.id).maybeSingle();
              if (!refreshedThread.error && refreshedThread.data) {
                thread = mapThread(refreshedThread.data);
              }
            } catch (error) {
        ...
        for (const call of calls) {
          ...
          await handleCallWebhookEntry({
            ...
            metaChannelId,
            callRecord: call,
            fallbackContactName: contactProfile?.name || null
          });
        }
...
function handleMessengerWebhookVerification(req, res) {
  const mode = req.query["hub.mode"];
  const token = req.query["hub.verify_token"];
  const challenge = req.query["hub.challenge"];
...
function handleInstagramWebhookVerification(req, res) {
  const mode = req.query["hub.mode"];
  const token = req.query["hub.verify_token"];
  const challenge = req.query["hub.challenge"];
  const validTokens = new Set(
...
async function handleMessengerWebhookEvent(req, res) {
  try {
    const payload = req.body;
...
async function processMessengerWebhookPayload(payload) {
  const pageIds = Array.from(
    new Set(
      (payload.entry || []).map((entry) => normalizeOptionalIdentifier(entry.id)).filter((pageId) => Boolean(pageId))
    )
  );
  if (pageIds.length === 0) {
    return;
  }
  const channelsResult = await adminSupabase.from("messenger_channels").select("*").in("page_id", pageIds);
  ...
  const matchedPageIds = channelRows.map((row) => normalizeOptionalIdentifier(row.page_id)).filter((pageId) => Boolean(pageId));
  if (matchedPageIds.length > 0) {
    await adminSupabase.from("messenger_channels").update({
      last_synced_at: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  const getProfile = (channelRow, senderId) => {
    ...
    if (!profileCache.has(cacheKey)) {
      ...
      let promise;
      try {
        promise = pageAccessTokenCiphertext ? fetchMessengerUserProfile(decryptAccessToken(pageAccessTokenCiphertext), senderId).catch((error) => {
  ...
  for (const entry of payload.entry || []) {
    ...
    const events = Array.isArray(entry.messaging) ? entry.messaging.filter((event) => isRecord(event)) : [];
    for (const event of events) {
      ...
      const existingMessageResult = await adminSupabase.from("conversation_messages").select("id").eq("user_id", userId).eq("wa_message_id", inboundEvent.messageId).maybeSingle();
      if (existingMessageResult.error) {
        throw existingMessageResult.error;
      }
      if (existingMessageResult.data) {
...
async function processInstagramWebhookPayload(payload) {
  ...
  const [byInstagramAccountResult, byPageResult] = await Promise.all([
    adminSupabase.from("instagram_channels").select("*").in("instagram_account_id", identifiers),
    adminSupabase.from("instagram_channels").select("*").in("page_id", identifiers)
  ]);
  ...
  const channelRowsById = /* @__PURE__ */ new Map();
  for (const row of [
    ...byInstagramAccountResult.data || [],
    ...byPageResult.data || []
  ]) {
  ...
  const channelIds = channelRows.map((row) => normalizeOptionalIdentifier(row.id)).filter((id) => Boolean(id));
  if (channelIds.length > 0) {
    await adminSupabase.from("instagram_channels").update({
      last_synced_at: (/* @__PURE__ */ new Date()).toISOString(),
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).in("id", channelIds);
  }
  ...
  const getProfile = (channelRow, senderId) => {
    ...
    if (!profileCache.has(cacheKey)) {
      const promise = fetchInstagramMessagingUserProfileForChannel(channelRow, senderId).catch((error) => {
  ...
  for (const entry of payload.entry || []) {
    ...
    for (const event of events) {
      ...
      const channelByRecipientInstagramId = recipientId ? channelByInstagramAccountId.get(recipientId) : null;
      const channelByRecipientPageId = recipientId ? channelByPageId.get(recipientId) : null;
      const channelRow = channelByEntryInstagramId || channelByRecipientInstagramId || channelByEntryPageId || channelByRecipientPageId || null;
      const userId = normalizeOptionalIdentifier(channelRow?.user_id);
      const instagramAccountId = normalizeOptionalIdentifier(channelRow?.instagram_account_id);
...
async function handleInstagramWebhookEvent(req, res) {
  try {
    const payload = req.body;
    const entries = Array.isArray(payload.entry) ? payload.entry : [];
...
app.post("/api/meta/catalog/webhook", async (req, res) => {
  try {
    ...
    const matchedAt = (/* @__PURE__ */ new Date()).toISOString();
    ...
    await Promise.all(
      rows.map(async (row) => {
        ...
        const { error } = await adminSupabase.from("meta_channels").update({
          metadata: {
            ...existingMetadata,
            catalogWebhook: {
...
app.get("/api/meta/lead-capture/webhook", async (req, res) => {
  try {
    ...
    const configResult = await adminSupabase.from("meta_lead_capture_configs").select("user_id").eq("verify_token", verifyToken).maybeSingle();
    if (configResult.error) {
      throw configResult.error;
    }
    if (!configResult.data) {
...
app.post("/api/meta/lead-capture/webhook", async (req, res) => {
  try {
    ...
    for (const entry of payload.entry || []) {
      ...
      const pageId = normalizeOptionalIdentifier(entry.id);
      if (pageId) {
        const updateResult = await adminSupabase.from("meta_lead_capture_configs").update({
          last_webhook_at: (/* @__PURE__ */ new Date()).toISOString(),
          last_error: null
      ...
      const changes = Array.isArray(entry.changes) ? entry.changes.filter((change) => isRecord(change)) : [];
...
app.post(["/api/razorpay/webhook", "/api/integrations/razorpay/webhook"], async (req, res) => {
  try {
    ...
    const result = await insertRazorpayWebhookEvent(req, req.body);
...
app.post("/api/auth/password-reset", async (req, res) => {
  try {
    ...
    const captchaToken = normalizeOptionalString(req.body?.captchaToken) || void 0;
...
app.post("/api/developer/webhooks", async (req, res) => {
  try {
    res.json(await createDeveloperWebhook(req.authedUser.id, req.body || {}));
  } catch (error) {
    sendError(res, 400, error);
  }
});
...
app.post("/api/notifications/read", async (req, res) => {
  try {
    const notificationId = req.body && typeof req.body.notificationId === "string" ? req.body.notificationId : null;
...
app.post("/api/email/connection/verify", async (req, res) => {
  try {
    res.json(await verifyEmailConnectionInput(req.body));
  } catch (error) {
    sendError(res, 400, error);
  }
});
app.post("/api/email/connection/verify/smtp", async (req, res) => {
  try {
    res.json(await verifyEmailSmtpConnectionInput(req.body));
  } catch (error) {
    sendError(res, 400, error);
  }
});
app.post("/api/email/connection/verify/imap", async (req, res) => {
  try {
    res.json(await verifyEmailImapConnectionInput(req.body));
  } catch (error) {
    sendError(res, 400, error);
  }
});
app.post("/api/email/connection", async (req, res) => {
  try {
    res.json({
      connection: await saveEmailConnection(req.authedUser.id, req.body)
    });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/email/templates", async (req, res) => {
  try {
    res.json({
      template: await saveEmailTemplate(req.authedUser.id, req.body)
    });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/email/campaigns/send", async (req, res) => {
  try {
    res.json({
      campaign: await sendEmailCampaign(req.authedUser.id, req.body)
    });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/integrations/meta-ads/options", async (req, res) => {
  try {
    const { accessToken, flowState } = req.body;
...
app.post("/api/integrations/meta-ads", async (req, res) => {
  try {
    res.json(await saveMetaAdsIntegration(req.authedUser.id, req.body));
  } catch (error) {
    sendError(res, 400, error);
  }
});
...
app.post("/api/profile/photo", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (uploadedProfilePictureUrl) {
      await deleteStoredAppProfilePhoto(uploadedProfilePictureUrl).catch(() => void 0);
    }
    sendError(res, 400, error);
  }
});
app.post("/api/profile/company-logo", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (uploadedCompanyLogoUrl) {
      await deleteStoredAppProfilePhoto(uploadedCompanyLogoUrl).catch(() => void 0);
    }
    sendError(res, 400, error);
  }
});
...
app.post("/api/team/invite", async (req, res) => {
  try {
    const member = await inviteWorkspaceTeamMember(req.authedUser, req.body);
    res.json({
      member,
      inviteSent: true
    });
...
app.post("/api/workspace/options", async (req, res) => {
  try {
    const option = await createWorkspaceOptionDefinition(req.authedUser, req.body);
    res.json({ option });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/billing/quote", async (req, res) => {
  try {
    const { quote } = await getBillingQuote(req.body);
    res.json({ quote });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/billing/subscription", async (req, res) => {
  try {
    const { planCode, billingCycle, couponCode } = req.body;
...
app.post("/api/billing/subscription/verify", async (req, res) => {
  try {
    ...
    const trialEndsAt = typeof subscription.notes?.trial_ends_at === "string" && subscription.notes.trial_ends_at || (subscription.start_at ? new Date(subscription.start_at * 1e3).toISOString() : null);
    if (!planCode) {
...
app.post("/api/wallet/topup", async (req, res) => {
  ...
  try {
    ...
    const transactionUpdateResult = await adminSupabase.from("transactions").update({
    ...
    if (pendingTransactionId) {
      const transactionErrorUpdate = await adminSupabase.from("transactions").update({
        status: "failed",
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("id", pendingTransactionId);
...
app.post("/api/wallet/topup/verify", async (req, res) => {
  try {
    ...
    const transactionResult = await adminSupabase.from("transactions").select("*").eq("id", transactionId).maybeSingle();
    ...
    const transactionRow = transactionResult.data;
    ...
    const promoteTransactionResult = await adminSupabase.from("transactions").update({
      status: "successful",
      ...
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("id", transactionId).eq("status", "pending").select("*").maybeSingle();
    if (promoteTransactionResult.error) {
      throw promoteTransactionResult.error;
    }
    if (promoteTransactionResult.data) {
      const walletUpdateResult = await adminSupabase.from("wallets").update({
...
app.post("/api/meta/connect/manual", async (req, res) => {
  try {
    const { accessToken, setupType, wabaId, phoneNumberId } = req.body;
...
app.post("/api/meta/connect/embedded", async (req, res) => {
  try {
    const { code, setupType, wabaId, phoneNumberId, redirectUri, setupContext, flowState, oauthState } = req.body;
    ...
    const mergedSetupContext = {
      ...
      flowState: normalizeOptionalString(setupContext?.flowState) || normalizeOptionalString(flowState) || "core_onboarding",
      oauthState: normalizeOptionalString(setupContext?.oauthState) || normalizeOptionalString(oauthState)
    };
    ...
    const accessToken = await exchangeEmbeddedSignupCode(code, {
      redirectUri,
      requestReferer: req.get("referer") || void 0,
      requestOrigin: req.get("origin") || void 0
    });
    const [phone, waba, reusableAssets] = await Promise.all([
      fetchPhoneNumber(accessToken, phoneNumberId),
      fetchWaba(accessToken, wabaId),
...
app.post("/api/meta/oauth/exchange", async (req, res) => {
  try {
    const { code, redirectUri, flowState, oauthState } = req.body;
    ...
    const exchangeResult = normalizedFlowState === "instagram_flow" ? await exchangeInstagramLoginCode(normalizedCode, {
...
app.post("/api/meta/channel/two-step-verification", async (req, res) => {
  try {
    const pin = normalizeMetaRegistrationPin(req.body.pin);
    const { row, accessToken } = await getChannelWithToken(req.authedUser.id);
    await updateWhatsAppBusinessPhoneNumberTwoStepVerification(
      accessToken,
    ...
    const updateTimestamp = (/* @__PURE__ */ new Date()).toISOString();
    const currentMetadata = getMetaChannelMetadataRecord(row);
    const currentTwoStepVerification = isRecord(currentMetadata.twoStepVerification) ? currentMetadata.twoStepVerification : {};
    const senderRegistration = isRecord(currentMetadata.senderRegistration) ? currentMetadata.senderRegistration : {};
    const enabledAt = normalizeOptionalString(currentTwoStepVerification.enabledAt) || normalizeOptionalString(senderRegistration.registeredAt) || updateTimestamp;
    const { data, error } = await adminSupabase.from("meta_channels").update({
      status: "connected",
      metadata: {
        ...currentMetadata,
    ...
    }).eq("user_id", req.authedUser.id).eq("id", row.id).select("*").single();
    if (error) {
      throw error;
    }
    res.json({
      channel: mapChannel(data)
    });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/meta/channel/request-code", async (req, res) => {
  try {
    ...
    res.json({ channel: mapChannel(data) });
  } catch (error) {
    sendError(res, 400, error);
  }
});
app.post("/api/meta/channel/verify-code", async (req, res) => {
  try {
    const code = normalizeMetaVerificationCode(req.body.code);
    const { row, accessToken } = await getChannelWithToken(req.authedUser.id);
    ...
    const currentMetadata = getMetaChannelMetadataRecord(row);
    const currentVerificationCodeRequest = isRecord(currentMetadata.verificationCodeRequest) ? currentMetadata.verificationCodeRequest : {};
    const verificationTimestamp = (/* @__PURE__ */ new Date()).toISOString();
    const { data, error } = await adminSupabase.from("meta_channels").update({
      metadata: {
        ...currentMetadata,
        verificationCodeRequest: {
...
app.post("/api/meta/channel/deregister", async (req, res) => {
  try {
    ...
    const deregistrationTimestamp = (/* @__PURE__ */ new Date()).toISOString();
    const currentMetadata = getMetaChannelMetadataRecord(row);
    const currentTwoStepVerification = isRecord(currentMetadata.twoStepVerification) ? currentMetadata.twoStepVerification : {};
    const { data, error } = await adminSupabase.from("meta_channels").update({
      status: "connected",
...
app.post("/api/instagram/connect/options", async (req, res) => {
  try {
    const { longLivedToken, accessToken } = req.body;
...
app.post("/api/messenger/connect/facebook-login", async (req, res) => {
  try {
    ...
    const selectedPage = (pageId ? connectablePages.find((page) => String(page.id) === pageId) : connectablePages.length === 1 ? connectablePages[0] : null) || null;
    if (!selectedPage) {
...
app.post("/api/meta/templates", async (req, res) => {
  try {
    ...
    if (normalizedHeaderType === "IMAGE" || normalizedHeaderType === "VIDEO" || normalizedHeaderType === "DOCUMENT") {
      const normalizedHeaderMediaHandle = normalizeOptionalString(headerMediaHandle) || normalizeOptionalString(headerMediaSampleUrl);
    ...
    const normalizedButtons = (await Promise.all(
      (buttons || []).map(async (button) => {
        ...
        if (button.type === "FLOW") {
          ...
          if (flowAction === "navigate") {
            normalizedButton.navigate_screen = normalizeOptionalString(button.navigateScreen ?? rawButton.navigate_screen) || entryScreen || void 0;
          }
    ...
    if (urlButtonCount > MAX_TEMPLATE_URL_BUTTONS || quickReplyButtonCount > MAX_TEMPLATE_QUICK_REPLY_BUTTONS || flowButtonCount > MAX_TEMPLATE_FLOW_BUTTONS) {
...
app.post("/api/meta/templates/:templateId/duplicate", async (req, res) => {
  try {
    ...
    const { row, accessToken } = await getChannelWithToken(req.authedUser.id);
    const templateResult = await adminSupabase.from("meta_templates").select("*").eq("user_id", req.authedUser.id).eq("id", templateId).maybeSingle();
    if (templateResult.error) throw templateResult.error;
    if (!templateResult.data) throw new Error("Template not found.");
    const raw = templateResult.data.raw;
    const sourceName = String(templateResult.data.template_name);
    const bodyComponent = Array.isArray(raw.components) ? raw.components.find((component) => component.type === "BODY") : null;
    if (!bodyComponent) {
...
app.delete("/api/meta/templates/:templateId", async (req, res) => {
  try {
    const templateResult = await adminSupabase.from("meta_templates").select("*").eq("user_id", req.authedUser.id).eq("id", req.params.templateId).maybeSingle();
    if (templateResult.error) throw templateResult.error;
    if (!templateResult.data) throw new Error("Template not found.");
    const { row, accessToken } = await getChannelWithToken(req.authedUser.id);
...
app.post("/api/meta/flows", async (req, res) => {
  try {
    const normalizedInput = normalizeFlowCreateInput(req.body);
    const { row, accessToken } = await getChannelWithToken(req.authedUser.id);
    const wabaId = normalizeOptionalIdentifier(row.waba_id);
    if (!wabaId) {
      throw new Error("The connected WhatsApp channel is missing a WABA ID.");
    ...
    const { data, error } = await adminSupabase.from("meta_flows").upsert(
      {
        user_id: req.authedUser.id,
        meta_channel_id: normalizeOptionalIdentifier(row.id),
        meta_flow_id: metaFlowId,
        flow_name: normalizedInput.name,
        status: normalizeOptionalString(remoteFlow.status)?.toUpperCase() || (normalizedInput.publish ? "PUBLISHED" : "DRAFT"),
...
app.patch("/api/meta/flows/:flowId", async (req, res) => {
  try {
    ...
    const normalizedInput = normalizeFlowUpdateInput(req.body);
    ...
    const preview = getRemoteFlowPreview(raw);
    const savedAt = (/* @__PURE__ */ new Date()).toISOString();
    const { data, error } = await adminSupabase.from("meta_flows").update({
      flow_name: nextName,
      ...
      last_error: getFlowValidationMessage(raw),
      updated_at: savedAt
    }).eq("user_id", req.authedUser.id).eq("id", req.params.flowId).select("*").single();
    if (error) {
      throw error;
    }
    res.json({
...
app.delete("/api/meta/flows/:flowId", async (req, res) => {
  try {
    ...
    const { error } = await adminSupabase.from("meta_flows").delete().eq("user_id", req.authedUser.id).eq("id", req.params.flowId);
    if (error) {
      throw error;
    }
    res.status(204).send();
...
app.post("/api/automations/rules", async (req, res) => {
  try {
    const { row } = await getChannelWithToken(req.authedUser.id);
    const rules = normalizeAutomationRulesInput(req.body.rules);
    res.json({
...
app.get("/api/meta/business-profile", async (req, res) => {
  try {
    ...
    const currentMetadata = isRecord(snapshot.channelRow.metadata) ? { ...snapshot.channelRow.metadata } : {};
    const currentDisplayNameApproval = isRecord(currentMetadata.displayNameApproval) ? { ...currentMetadata.displayNameApproval } : {};
    const currentDisplayNameRequest = isRecord(currentMetadata.displayNameRequest) ? { ...currentMetadata.displayNameRequest } : null;
    const previousDisplayNameStatus = getDisplayNameStatus(currentDisplayNameApproval.status) || getStoredDisplayNameApprovalStatus(row.metadata);
    ...
    if (nextDisplayNameStatus || nextDisplayNameRequestStatus) {
      const checkedAt = (/* @__PURE__ */ new Date()).toISOString();
      const approvedAt = nextDisplayNameStatus === "APPROVED" ? normalizeOptionalString(currentDisplayNameApproval.approvedAt) || checkedAt : null;
      const nextMetadata = {
        ...currentMetadata,
        displayNameApproval: {
          ...currentDisplayNameApproval,
        ...
        ...currentDisplayNameRequest ? {
          displayNameRequest: {
            ...currentDisplayNameRequest,
            status: nextDisplayNameRequestStatus || getDisplayNameStatus(currentDisplayNameRequest.status),
            approvedAt: nextDisplayNameRequestStatus === "APPROVED" ? normalizeOptionalString(currentDisplayNameRequest.approvedAt) || checkedAt : normalizeOptionalString(currentDisplayNameRequest.approvedAt),
            lastCheckedAt: checkedAt
          }
        } : {}
      };
      const { data: updatedChannelRow, error: updateDisplayNameMetadataError } = await adminSupabase.from("meta_channels").update({
        metadata: nextMetadata,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("user_id", req.authedUser.id).eq("id", snapshot.channelRow.id).select("*").single();
      if (updateDisplayNameMetadataError) {
        throw updateDisplayNameMetadataError;
      }
      profileChannelRow = updatedChannelRow;
      if (previousDisplayNameStatus !== "APPROVED" && nextDisplayNameStatus === "APPROVED") {
        const verifiedName2 = normalizeOptionalString(snapshot.phone.verified_name) || normalizeOptionalString(snapshot.channelRow.verified_name) || "Your WhatsApp display name";
        const displayPhoneNumber = normalizeOptionalString(snapshot.phone.display_phone_number) || normalizeOptionalString(snapshot.channelRow.display_phone_number) || "your connected sender";
        await createUserNotification({
          userId: req.authedUser.id,
          type: "display_name_approved",
          title: "WhatsApp display name approved",
          body: `${verifiedName2} is now approved for ${displayPhoneNumber}.`,
          targetPath: "/dashboard/profile",
          metadata: {
          ...
          dedupeKey: `display-name-approved:${String(snapshot.channelRow.phone_number_id)}:${verifiedName2}`
        });
      }
    }
  ...
  try {
    ...
    res.json(
      await fetchWhatsAppBusinessActivities(accessToken, wabaId, {
        limit: typeof limit === "string" || typeof limit === "number" ? Number(limit) : void 0,
        after: typeof after === "string" ? after : void 0,
        before: typeof before === "string" ? before : void 0,
        since: typeof since === "string" ? since : void 0,
        until: typeof until === "string" ? until : void 0,
        activityType: typeof activityType === "string" ? activityType.split(",") : void 0
      })
    );
  } catch (error) {
    sendError(res, 400, error);
...
app.post("/api/calls/permissions/request", async (req, res) => {
  try {
    ...
    const existingThread = payload.threadId ? await getThreadById(req.authedUser.id, payload.threadId) : null;
...
app.post("/api/calls", async (req, res) => {
  try {
    ...
    const callSession = response.callId || payload.callId ? await upsertCallSession({
      userId: req.authedUser.id,
      metaChannelId: String(row.id),
    ...
    const callLog = payload.action === "connect" && payload.to ? await insertCallLog({
      userId: req.authedUser.id,
      callId: response.callId,
      phone: payload.to,
      type: "outgoing"
    }) : callSession && isTerminalCallState(callSession.state) ? await syncCallLogFromSession(req.authedUser.id, callSession) : null;
    ...
    res.json({
      ...response,
      callLog: callLog || void 0,
      callSession: callSession || void 0
    });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/contacts", async (req, res) => {
  try {
    const contact = await createContact(req.authedUser.id, req.body);
    res.json({ contact });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.patch("/api/contacts/:threadId", async (req, res) => {
  try {
    const contact = await updateContact(req.authedUser.id, req.params.threadId, req.body);
    res.json({ contact });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.get("/api/conversations", async (req, res) => {
  try {
    ...
    const { data, error } = await adminSupabase.from("conversation_threads").select("*").eq("user_id", req.authedUser.id).order("last_message_at", { ascending: false, nullsFirst: false }).limit(limit);
    if (error) {
      throw error;
    }
    res.json({
      conversations: (data || []).map((row) => mapThread(row))
    });
  } catch (error) {
    sendError(res, 400, error);
  }
...
app.post("/api/conversations/:threadId/messages/text", async (req, res) => {
  try {
    ...
    if (threadChannel === "messenger") {
      const { row: row2, accessToken: accessToken2 } = await getMessengerChannelWithToken(req.authedUser.id);
      const result2 = await sendThreadOutgoingMessengerTextMessage({
        user: req.authedUser,
        channelRow: row2,
      ...
      res.json({ ok: true, thread: result2.thread, message: result2.message });
      return;
    }
    if (threadChannel === "instagram") {
      const { row: row2, pageAccessToken, userAccessToken } = await getInstagramChannelWithToken(req.authedUser.id);
      const result2 = await sendThreadOutgoingInstagramTextMessage({
        user: req.authedUser,
        channelRow: row2,
        userAccessToken,
        pageAccessToken,
...
app.post("/api/conversations/:threadId/messages", async (req, res) => {
  try {
    ...
    const { message, clientTempId } = req.body;
    if (!message) {
...
app.post("/api/campaigns/marketing-message", async (req, res) => {
  try {
    ...
    const productPolicy = payload.productPolicy ? normalizeMarketingMessageProductPolicy(payload.productPolicy) : null;
    const messageActivitySharing = typeof payload.messageActivitySharing === "boolean" ? payload.messageActivitySharing : void 0;
    if (!campaignName) {
    ...
    for (const recipient of recipients) {
      try {
        ...
        if (optedOutContact) {
          const contactLabel = normalizeOptionalString(optedOutContact.contact_name) || normalizeOptionalString(optedOutContact.display_phone) || recipient.contactName || recipient.to;
          throw new Error(`${contactLabel} has opted out of WhatsApp marketing campaigns.`);
        }
        const templatePayload = {
          ...
          templateName,
          language,
          components: Array.isArray(payload.components) ? payload.components : void 0
        };
        ...
        await insertMessage({
          ...
          raw: {
            ...
            marketing_message: {
              ...
              message_activity_sharing: typeof messageActivitySharing === "boolean" ? messageActivitySharing : null,
    ...
    const optedOutCount = results.filter(
      (result) => result.error?.toLowerCase().includes("opted out of whatsapp marketing campaigns")
    ).length;
    const response = {
      campaignName,
      templateName,
      audienceCount: recipients.length,
      sentCount: successResults.length - heldForQualityAssessmentCount - pausedCount,
      failedCount,
      optedOutCount,
    ...
    if (response.sentCount > 0) {
      ...
      await createUserNotification({
        ...
        body: `${campaignName} was accepted by Meta for ${response.sentCount} contact${response.sentCount === 1 ? "" : "s"}.`,
